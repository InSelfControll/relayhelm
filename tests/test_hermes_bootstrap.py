...
The bootstrap module is imported at the top of every Relayhelm entry point
(hermes, relayhelm, hermes-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Relayhelm entry point imports hermes_bootstrap as its
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Relayhelm CLI
...
class TestEntryPointsImportBootstrap:
    """Every Relayhelm entry point must import hermes_bootstrap as its
    ...
    def test_entry_point_imports_bootstrap(self, path):
        ...
        half-finished ``relayhelm update`` (git-reset landed new code but
        ...
        """
        # Resolve relative to the relayhelm repo root.  Tests live
...
class TestHardenImportPath:
    ...
    directory from shadowing Relayhelm's own top-level modules — covering both
