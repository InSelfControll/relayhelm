...
Run from the PR worktree:
    ...
    cd ~/.relayhelm/relayhelm/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.relayhelm/relayhelm``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.relayhelm/relayhelm
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a relayhelm checkout"
)
assert (PR_DIR / "tools" / "browser_tool.py").exists(), (
    f"PR_DIR={PR_DIR} doesn't look like a relayhelm checkout"
)
