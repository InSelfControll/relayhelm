"""`setup_path()` must also install a `relayhelm-acp` launcher.

ACP hosts (Zed, JetBrains, Buzz Desktop) spawn the agent by resolving a command
name against the login-shell PATH. The `relayhelm-acp` console script generated by
the packaging metadata lives inside the install's venv, which is not on that
PATH, so those hosts report Relayhelm as not installed. `setup_path()` therefore
writes a `relayhelm-acp` launcher next to the `relayhelm` one.
...
ACP_BLOCK = re.compile(
    r'(    rm -f "\$command_link_dir/relayhelm-acp".*?'
    r'log_success "Installed relayhelm-acp launcher[^\n]*\n)',
    re.S,
)
...
def _extract_acp_shim_block() -> str:
    ...
    assert match, (
        "could not locate the relayhelm-acp launcher block in scripts/install.sh — "
...
def _run_block(tmp_path: Path, use_venv: str) -> Path:
    ...
    return command_link_dir / "relayhelm-acp"
...
def test_acp_launcher_does_not_follow_a_symlink_into_the_venv(tmp_path):
    ...
    An older install could leave `relayhelm-acp` as a symlink to the venv console
    ...
    shim_path = command_link_dir / "relayhelm-acp"
    ...
    assert console_script.read_text(encoding="utf-8") == marker, (
        "venv/bin/relayhelm-acp was overwritten — symlink-stomp regression (#21454)"
    )
    assert not shim_path.is_symlink(), (
        "command_link_dir/relayhelm-acp must be replaced with a regular file"
    )


# ---------------------------------------------------------------------------
# relayhelm launcher regression (#74819)
# ---------------------------------------------------------------------------

HERMES_AGENT_BLOCK = re.compile(
    r'(rm -f "\$command_link_dir/relayhelm-agent".*?'
    r'log_success "Installed relayhelm-agent launcher[^\n]*\n)',
    re.S,
)
...
def _extract_hermes_agent_shim_block() -> str:
    ...
    assert match, (
        "could not locate the relayhelm launcher block in scripts/install.sh — "
        "if it was renamed, update this test with it"
    )
    return match.group(1)
...
def _run_hermes_agent_block(tmp_path: Path, use_venv: str) -> Path | None:
    """Execute the extracted relayhelm block with the env vars setup_path() sets."""
    if use_venv == "false":
        # --no-venv: relayhelm is NOT installed by this block (handled
    ...
    assert result.returncode == 0, (
        f"relayhelm shim block failed:\nstdout={result.stdout}\nstderr={result.stderr}"
    )
    return command_link_dir / "relayhelm-agent"


def test_venv_install_writes_executable_hermes_agent_launcher(tmp_path):
    """venv install must write a user-executable relayhelm launcher."""
    ...
    assert "unset PYTHONPATH" in text
    assert "unset PYTHONHOME" in text
    assert "run_agent.py" in text, "relayhelm must dispatch to run_agent.py"


def test_hermes_agent_launcher_cleanup_on_uninstall(tmp_path):
    """uninstall.remove_wrapper_script() must remove relayhelm alongside
    relayhelm and relayhelm-acp."""
    from hermes_cli.uninstall import remove_wrapper_script

    # Simulate a relayhelm wrapper in the user-local location
    local_shim = tmp_path / ".local" / "bin" / "relayhelm-agent"
    ...
    assert local_shim in removed, "local relayhelm wrapper must be removed"
