...
Field incident: an interrupted `relayhelm update` (pull done, restart never ran)
...
had no way to know the fix was one `relayhelm gateway restart`. The failure
...
def test_import_error_with_skew_names_shas_and_the_restart_command(monkeypatch):
    ...
    assert "relayhelm gateway restart" in msg
...
def test_import_error_without_skew_stays_a_plain_import_message(monkeypatch):
    ...
    assert "relayhelm gateway restart" not in msg
