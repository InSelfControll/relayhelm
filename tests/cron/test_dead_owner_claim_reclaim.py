...
A one-shot ``relayhelm cron run <job_id>`` used to background-dispatch the run
...
def _orphan_claimed_row(executions, job_id: str) -> str:
    ...
    Mirrors what a one-shot ``relayhelm cron run`` leaves behind: a row stuck in
...
class TestOneShotCliRunIsSynchronous:
    ...
    def test_cli_run_declares_stateless_channel_before_dispatch(self, monkeypatch):
        """`relayhelm cron run` must gate off async delivery so the run executes
