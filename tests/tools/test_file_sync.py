...
def _make_get_files(tmp_files, remote_base="/root/.relayhelm"):
...
def _make_manager(tmp_files, remote_base="/root/.relayhelm", upload=None, delete=None):
...
class TestEdgeCases:
    ...
    def test_file_disappears_between_list_and_upload(self, tmp_path):
        ...
        upload = MagicMock()
        mgr = FileSyncManager(
            get_files_fn=lambda: [(str(f), "/root/.relayhelm/ephemeral.txt")],
            upload_fn=upload,
            delete_fn=MagicMock(),
        )
...
class TestConcurrency:
    def test_sync_back_waits_for_active_sync_transaction(self, tmp_path):
        ...
        def get_files():
            return [
                (str(path), f"/root/.relayhelm/cache/images/{path.name}")
    ...
    def test_sync_back_does_not_overwrite_uploaded_credential_files(self, tmp_path, monkeypatch):
            ...
            lambda: [
                {
                    ...
                    "container_path": "/root/.relayhelm/credentials/token.json",
                }
            ],
        )
        monkeypatch.setattr(
            "tools.credential_files.iter_skills_files",
            lambda container_base="/root/.relayhelm": [
                {
                    "host_path": str(skill),
        ...
        def bulk_download(dest: Path) -> None:
            with tarfile.open(dest, "w") as tar:
                for name, data in {
                    "root/.relayhelm/credentials/token.json": b"remote-token",
                    "root/.relayhelm/skills/skill.py": b"remote-skill",
                }.items():
