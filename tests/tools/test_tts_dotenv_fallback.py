...
TTS provider tools must resolve API keys from ``~/.relayhelm/.env`` (via
...
class TestDotenvFallbackPerProvider:
    """For each affected provider, when only ``~/.relayhelm/.env`` carries the
...
class TestRegressionGuard:
    ...
    ``hermes_cli.config.load_env`` to simulate ``~/.relayhelm/.env`` carrying the
    ...
    def test_minimax_missing_when_only_in_dotenv_before_fix(self, tmp_path, monkeypatch):
        ...
        # Simulate ~/.relayhelm/.env carrying the key (load_env returns the dict
    ...
    def test_check_tts_requirements_sees_dotenv_minimax(self, monkeypatch):
        ...
        in ``~/.relayhelm/.env``, even though the dispatcher would later succeed.
        """
        from tools import tts_tool
