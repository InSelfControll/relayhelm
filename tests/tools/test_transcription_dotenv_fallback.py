...
``~/.relayhelm/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.relayhelm/.env`` would be told
...
class TestEndToEndRegressionGuard:
    ...
    ``~/.relayhelm/.env`` carrying the key while ``os.environ`` does not.
