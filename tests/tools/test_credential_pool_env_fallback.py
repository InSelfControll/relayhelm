...
- _seed_from_env reads API keys from ~/.relayhelm/.env when not in os.environ
- _resolve_api_key_provider_secret falls back to credential_pool when env vars are empty
- ~/.relayhelm/.env takes priority over os.environ for Hermes-managed credentials
...
def _write_env_file(home: Path, **kwargs) -> None:
    """Write key=value pairs to ~/.relayhelm/.env."""
    lines = [f"{k}={v}" for k, v in kwargs.items()]
    (home / ".env").write_text("\n".join(lines) + "\n")


class TestCredentialPoolSeedsFromDotEnv:
    """_seed_from_env must read keys from ~/.relayhelm/.env, not just os.environ.
    ...
    def test_dotenv_wins_over_stale_os_environ(self, isolated_hermes_home, monkeypatch):
        """Regression for #20591: a fresh key rotated into ~/.relayhelm/.env must
...
class TestAuthResolvesFromDotEnv:
    """_resolve_api_key_provider_secret must also read from ~/.relayhelm/.env."""
    ...
    def test_dotenv_wins_over_stale_os_environ_on_resolve(
        ...
        """Regression for #20591: when both ~/.relayhelm/.env and os.environ define
    ...
    def test_get_anthropic_key_prefers_dotenv_over_stale_os_environ(
        ...
        prefer ~/.relayhelm/.env over a stale shell export. This path resolves
