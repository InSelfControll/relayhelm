...
class TestProtectedInstructionFiles:
    ...
    AGENTS.md / CLAUDE.md / SOUL.md / .cursorrules / project-local .relayhelm
    ...
    def test_checkout_nested_under_hermes_dir_not_gated(self, tmp_path, approvals):
        """A repo living UNDER a .relayhelm dir (e.g. ~/.relayhelm/relayhelm)
        must not have every write gated — only files directly inside a
        .relayhelm dir count as project config."""
        repo = tmp_path / ".relayhelm" / "some-repo" / "src"
    ...
    def test_real_hermes_home_not_gated_by_this_check(
        ...
        """~/.relayhelm itself is governed by existing guards, not this gate."""
        import tools.file_tools_write_guards as ft
        fake_home = tmp_path / ".relayhelm"
