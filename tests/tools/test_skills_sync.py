...
class TestRmtreeWritableScopeGuard:
    ...
    exception — the result is a silent ``shutil.rmtree(~/.relayhelm/)``
    ...
    def test_refuses_anything_that_is_not_a_strict_child_of_skills(self, tmp_path):
        """``/``, ``~/.relayhelm`` itself, a sibling dir, and the skills root
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `relayhelm profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.relayhelm) rely on so bundled
    skills are never seeded at install time NOR re-injected by `relayhelm update`.
    """
...
class TestOptOutToggleAndRemove:
    """`relayhelm skills opt-out/opt-in` core: marker toggle + safe removal."""
