...
Under a gateway (Telegram, Discord, Slack, ...) Relayhelm passes the
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Relayhelm via Telegram AND Discord
...
class TestProfilePeerUniqueness:
    """Each Relayhelm profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `relayhelm -p partner` pinned to a different person's peer).
