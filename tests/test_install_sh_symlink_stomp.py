...
Older versions of ``install.sh`` created ``$command_link_dir/relayhelm`` as a
symlink to the pip-generated entry point at ``$HERMES_BIN`` (i.e.
``venv/bin/relayhelm``). When ``setup_path()`` later switched to writing a bash
shim with ``cat > "$command_link_dir/relayhelm" <<EOF``, the redirect followed
the existing symlink and overwrote the pip entry point with the shim. The
shim's ``exec "$HERMES_BIN" "$@"`` then self-recursed and ``relayhelm`` hung on
every invocation.

These tests pin the fix: ``setup_path()`` must remove ``$command_link_dir/relayhelm``
...
def _extract_setup_path_shim_block() -> str:
    ...
    match = re.search(
        r"(?P<block>mkdir -p \"\$command_link_dir\".*?chmod \+x \"\$command_link_dir/relayhelm\")",
        text,
        re.DOTALL,
    )
...
def test_setup_path_shim_block_removes_old_link_before_writing() -> None:
    ...
    rm_idx = block.find('rm -f "$command_link_dir/relayhelm"')
    cat_idx = block.find('cat > "$command_link_dir/relayhelm" <<EOF')
    assert rm_idx != -1, (
        "setup_path() must `rm -f` $command_link_dir/relayhelm before the "
...
def test_re_running_setup_path_block_preserves_pip_entry_point(tmp_path: Path) -> None:
    ...
    Layout mirrors a real install:
        ...
        tmp/
          venv/bin/relayhelm        <- pip entry point (the one we must preserve)
          local_bin/relayhelm       <- symlink → ../venv/bin/relayhelm  (old install)
    ...
    requires that, after the run:
      ...
      * ``venv/bin/relayhelm`` still contains its original pip-script body
      * ``local_bin/relayhelm`` is a regular file (not a symlink) holding the shim
    """
    venv_bin = tmp_path / "venv" / "bin"
    venv_bin.mkdir(parents=True)
    pip_entry = venv_bin / "relayhelm"
    ...
    shim_path = command_link_dir / "relayhelm"
    ...
    assert pip_entry.read_text() == pip_marker, (
        "venv/bin/relayhelm was overwritten by setup_path() — symlink-stomp "
        "regression (#21454)."
    )
    ...
    assert not shim_path.is_symlink(), (
        "command_link_dir/relayhelm must be replaced with a regular file, not "
