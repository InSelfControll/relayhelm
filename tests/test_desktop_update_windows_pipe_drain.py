...
last of them to close it. ``relayhelm update`` deliberately runs build steps with
...
"Updating Relayhelm" until the user kills the gateway by hand, and the stale marker
...
step writing to both pipes took 18.3s vs 0.29s. ``relayhelm update`` is exactly this
...
class TestIdleWatchdogCountsUpdateLogGrowth:
    ...
    Real updates are stdout-silent for 40+ minutes: ``relayhelm update`` captures
...
def test_update_step_survives_pipe_leak_flood_and_live_child_stall(
    ...
    *logstall* -- a step that is silent on its pipes but appends to the
    ...
    This is the shape of every real ``relayhelm update`` build: output streams to
    ...
    assert "PIPE-DRAIN SELF-TEST: PASS" in result.stdout, (
        ...
        "Relayhelm' forever) or metered a chatty step (backpressure on the running "
