...
Before the s6 migration, ``docker run inselfcontroll/relayhelm gateway
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → relayhelm gateway run) can take
    ...
    # (main-wrapper.sh → python → relayhelm gateway run) can take well over
...
def test_supervised_gateway_does_not_recurse(
    ...
    supervised ``relayhelm gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``relayhelm gateway
    ...
    # (main-wrapper.sh → python → relayhelm gateway run) can take well
    ...
    # started either — so we'd see the CMD's `relayhelm gateway run`
    ...
    # `relayhelm gateway run`. If the recursion guard fails, s6 would
    ...
    assert n <= 1, (
        f"expected at most one supervised python `relayhelm gateway run` "
