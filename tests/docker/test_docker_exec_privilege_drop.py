...
The shim (docker/hermes-exec-shim.sh, installed at /opt/hermes/bin/relayhelm)
...
failed: Relayhelm is not logged into Nous Portal" on every message.
...
def test_shim_drops_root_to_hermes_uid(sleep_container: str) -> None:
    ...
    Use `relayhelm config set` which writes config.yaml under HERMES_HOME.
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    "Relayhelm is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `relayhelm config set`-style
    ...
    the hermes UID. We use `relayhelm auth list` since that touches the
    ...
    # `relayhelm auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `relayhelm config set` first to
