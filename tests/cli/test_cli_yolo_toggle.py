...
prompt. Only ``relayhelm --yolo`` (process-start env), ``HERMES_YOLO_MODE=1``,
and ``relayhelm config set approvals.mode off`` actually bypassed.
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``relayhelm --yolo`` sets ``HERMES_YOLO_MODE`` before tool imports, so
