...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"relayhelm[termux]"' in text
    assert '"relayhelm[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"relayhelm[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
