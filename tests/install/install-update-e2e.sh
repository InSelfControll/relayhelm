...
#
# Installs a real, earlier Relayhelm the way a user does, applies ONE update route,
...
INSTALL_DIR="/home/hermes/.relayhelm/relayhelm"   # user-level layout (sandbox default)
FAKE_REMOTE="/work/repos/relayhelm.git"
...
# both together.
UPSTREAM_URL="${HERMES_DEV_SANDBOX_UPSTREAM:-https://github.com/InSelfControll/relayhelm.git}"
...
# ── helpers ────────────────────────────────────────────────────────────────
# Does the INSTALLED hermes accept FLAG on `relayhelm update`?
#
...
# that silently rots. `relayhelm update --help` is the same surface a user meets,
# and argparse prints every option it accepts.
update_supports() {
  local flag="$1"
  in_sandbox "relayhelm update --help 2>&1" | grep -qF -- "$flag"
}
...
require_hermes_works() {
  local when="$1" out
  out="$(in_sandbox "relayhelm --version" 2>&1)" \
    || { printf '%s\n' "$out" >&2; fail "relayhelm --version failed $when"; }
  printf '%s\n' "$out" | sed 's/^/    /'
  ok "hermes runs $when"
}

# ── install the earlier Relayhelm ─────────────────────────────────────────────
...
case "$ROUTE" in
  update)
    ...
    if update_supports --yes; then
      update_cmd="relayhelm update --yes"
    else
      update_cmd="relayhelm update </dev/null"
    fi
    if ! in_sandbox "cd $INSTALL_DIR && $update_cmd"; then
      collect_sandbox_logs update
      fail "relayhelm update failed ($update_cmd)"
    fi
    require_landed_on_target 'relayhelm update'
    require_hermes_works 'after relayhelm update'
    ;;
  installer)
