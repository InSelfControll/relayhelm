...
class TestEmbeddedTuiPaneClarifier:
    """When ``HERMES_DESKTOP_TERMINAL=1``, a standalone ``relayhelm --tui`` is
...
class TestContradictionGone:
    ...
    contained both ``You are running in the Relayhelm terminal UI (TUI).`` and
    ``Runtime surface: you're running inside the Relayhelm desktop GUI app.``.
