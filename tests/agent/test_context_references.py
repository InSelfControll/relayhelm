...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Relayhelm Tests")
...
def test_binary_reference_block_maps_host_attachment_to_container_path(tmp_path: Path, monkeypatch):
    ...
    assert "/root/.relayhelm/attachments/archive.zip" in result.message
...
def test_binary_reference_block_keeps_host_path_on_local_backend(tmp_path: Path, monkeypatch):
    ...
    assert "/root/.relayhelm/attachments/" not in result.message
...
async def test_blocks_canonical_read_denylist_credential_stores(tmp_path: Path, monkeypatch):
    ...
    a chat peer could otherwise attach `@file:~/.relayhelm/auth.json` and read the
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.relayhelm/auth.json and @file:.relayhelm/.anthropic_oauth.json "
        "and @file:.relayhelm/mcp-tokens/github.json and @file:project/.env",
        cwd=tmp_path,
...
async def test_canonical_guard_fails_closed_when_lookup_raises(tmp_path: Path, monkeypatch):
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.relayhelm/auth.json",
        cwd=tmp_path,
        allowed_root=tmp_path,
        context_length=100_000,
    )
...
@pytest.mark.parametrize(
    ...
    [
        "/tmp/plain.png",
        "/Users/me/Library/Application Support/Relayhelm/composer-images/a.png",
