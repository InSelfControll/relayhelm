"""Regression tests for InSelfControll/relayhelm#2765.
...
service that didn't inherit ``~/.relayhelm/.env``) used to be dropped silently.
...
def test_warns_once_and_dedupes(caplog):
    ...
    assert "relayhelm memory status" in msg
