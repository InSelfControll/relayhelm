...
def curator_env(tmp_path, monkeypatch):
    ...
    # the *next* test has configured (or the default ~/.relayhelm once monkeypatch
...
# `relayhelm model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_review_prompt_does_not_steer_terminal_writes():
    ...
    ``mkdir -p ~/.relayhelm/skills/<umbrella>/references/ && mv ...`` its
