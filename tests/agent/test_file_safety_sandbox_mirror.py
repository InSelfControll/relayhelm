...
Those paths look like ``…/sandboxes/<backend>/<task>/home/.relayhelm/…`` and
they accumulate divergent copies of authoritative profile state (SOUL.md,
config.yaml, memories/*.md) because the host Relayhelm process never reads
...
class TestClassifySandboxMirrorTarget:
    def test_docker_mirror_soul_md_classified(self, tmp_path):
        ...
        target = (
            tmp_path
            / "profiles" / "group1"
            / "sandboxes" / "docker" / "default" / "home" / ".relayhelm"
    ...
    def test_other_backends_and_inner_files_match(self, tmp_path, backend, inner):
        ...
        target = (
            tmp_path
            / "sandboxes" / backend / "task-42" / "home" / ".relayhelm"
            / Path(inner)
        )
...
class TestGetSandboxMirrorWarning:
    def test_non_mirror_returns_none(self, tmp_path):
        ...
        target = tmp_path / ".relayhelm" / "profiles" / "group1" / "SOUL.md"
        target.parent.mkdir(parents=True)
    ...
    def test_mirror_warning_names_mirror_root_and_inner_path(self, tmp_path):
        ...
        assert "sandboxes/docker/default/home/.relayhelm" in warn
    ...
    def test_warning_is_defense_in_depth_not_boundary(self, tmp_path):
        ...
        target = (
            tmp_path
            / "sandboxes" / "docker" / "t" / "home" / ".relayhelm"
            / "profiles" / "g" / "SOUL.md"
        )
        target.parent.mkdir(parents=True)
        target.write_text("x")
