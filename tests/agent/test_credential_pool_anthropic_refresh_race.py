...
Relayhelm *processes* via the cross-process ``_auth_store_lock`` flock,
...
Relayhelm's own PKCE login (``manual:hermes_pkce`` / ``hermes_pkce``) got no
...
def _fake_pool_store(monkeypatch):
    ...
    This stands in for ``~/.relayhelm/auth.json`` so the two "process-local"
...
def test_concurrent_hermes_pkce_refresh_loses_credential_despite_valid_token_on_disk(monkeypatch):
    """Two 'Relayhelm processes' race to refresh the same stale hermes_pkce
    ...
    assert loser_entry_after.last_status != STATUS_EXHAUSTED, (
        ...
        "though the account is not actually exhausted -- a sibling Relayhelm "
        "process holds a perfectly valid rotated token. This causes "
        "spurious 're-authenticate with Anthropic' failures under "
        "ordinary concurrent Relayhelm usage (fleet workers, cron jobs, "
