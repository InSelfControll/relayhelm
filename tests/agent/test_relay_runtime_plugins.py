...
def test_foreign_active_plugin_configuration_is_left_unchanged(
    ...
    try:
        ...
        assert relay.events == []
        assert "already active outside Relayhelm native ownership" in caplog.text
...
def test_initialization_failure_is_fail_open(explicit_static_config, caplog):
    ...
    try:
        ...
        assert "Relayhelm Relay plugin initialization failed" in caplog.text
    finally:
        host.shutdown()
...
def test_two_profile_hosts_initialize_once_and_clear_after_final_shutdown(
    ...
    assert (
        caplog.text.count(
            ...
            "hosted by this Relayhelm process."
        )
        == 1
    )
...
""".strip(),
    ...
    try:
        ...
        assert relay.events == []
        assert "Relayhelm [[dynamic_plugins]] records are unsupported" in caplog.text
...
agent_name = "Relayhelm Native Test"
agent_version = "test"
""".strip(),
        encoding="utf-8",
    )
