...
class TestSanitizeStructureNonAscii:
    def test_sanitizes_nested_dict_structure(self):
        payload = {
            "default_headers": {
                "X-Title": "Relayhelm │ Agent",
                "User-Agent": "Relayhelm/1.0 🤖",
            }
        }
        assert _sanitize_structure_non_ascii(payload) is True
        assert payload["default_headers"]["X-Title"] == "Relayhelm  Agent"
        assert payload["default_headers"]["User-Agent"] == "Relayhelm/1.0 "
