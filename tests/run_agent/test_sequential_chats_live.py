...
AlexKucera reported on Discord (2026-04-16) that after ``relayhelm update`` pulled
...
Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.relayhelm/.env).
"""
...
# Load ~/.relayhelm/.env so live runs pick up OPENROUTER_API_KEY without
# needing the runner to shell-source it first. Silent if the file is absent.
def _load_user_env() -> None:
    env_file = Path.home() / ".relayhelm" / ".env"
