...
Relayhelm's own top-level modules (``utils``, ``proxy``, ``ui``).  entry.py must
...
def test_guard_handles_absolute_cwd_path():
    ...
    path, ahead of the Relayhelm root.  harden_import_path must relocate the
    Relayhelm root to the front so ``from utils import ...`` resolves to Relayhelm."""
    import sys
