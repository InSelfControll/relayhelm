"""``relayhelm config set`` must parse list/mapping literals, not store them as strings.

Before this fix, ``relayhelm config set platform_toolsets.discord '["file","web"]'``
