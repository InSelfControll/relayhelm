...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Relayhelm' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Relayhelm' DB/metadata/logs on task completion.
    """
...
# spawned with `relayhelm -p <profile>` must read/write the same kanban.db
...
class TestSharedBoardPaths:
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.relayhelm/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.relayhelm/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.relayhelm/kanban.db.
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'relayhelm kanban recompute' would promote it.
    """
    with kbc.connect() as conn:
        # A is done.
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    assert "Relayhelm" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
