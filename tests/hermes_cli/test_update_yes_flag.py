"""Tests for `relayhelm update --yes / -y` — assume yes for interactive prompts.
...
class TestUnicodeDecodeErrorInUpdatePrompts:
    ...
    the exception escape and crash `relayhelm update` mid-flight.
    """
    ...
    def test_unicode_decode_error_in_tty_skips_and_prints_hint(
        ...
        out = capsys.readouterr().out
        assert "relayhelm config migrate" in out
