...
subparser, placing the flag BEFORE the subcommand (e.g. 'relayhelm --yolo chat')
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `relayhelm gateway run --accept-hooks` failed
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `relayhelm -t web chat` silently dropped the
