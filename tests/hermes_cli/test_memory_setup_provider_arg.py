"""Tests for `relayhelm memory setup [provider]` routing.
...
``relayhelm memory setup honcho``) without the interactive picker — which matters
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "relayhelm memory setup" in out
