...
HERMES_HOME=/root/.relayhelm), _apply_profile_override must still read
...
def _run_apply_profile_override(
    ...
    or None if unset.
    """
    hermes_root = tmp_path / ".relayhelm"
    ...
    monkeypatch.setattr(sys, "argv", argv or ["relayhelm", "gateway", "start"])
...
class TestApplyProfileOverrideHermesHomeGuard:
    ...
    def test_hermes_home_at_root_with_active_profile_is_redirected(
        ...
        """HERMES_HOME=/root/.relayhelm + active_profile=coder must redirect
        HERMES_HOME to .../profiles/coder.

        Bug scenario from #22502: systemd sets HERMES_HOME to the hermes root
        and the user switches to a profile via `relayhelm profile use`.
    ...
    def test_sudo_explicit_profile_resolves_invoking_users_profile(self, tmp_path, monkeypatch):
        ...
        profile_dir = user_home / ".relayhelm" / "profiles" / "elias"
        profile_dir.mkdir(parents=True, exist_ok=True)
        (root_home / ".relayhelm").mkdir(parents=True, exist_ok=True)
        ...
        assert sys.argv == ["relayhelm", "gateway", "install", "--system"]
...
class TestSupervisedChildIgnoresStickyProfile:
    ...
    bare ``relayhelm gateway run`` (no ``-p``) to mean "the root HERMES_HOME
    ...
    def test_non_supervised_run_still_follows_active_profile(
        ...
        """Without the sentinel, a normal `relayhelm gateway run` still honors
