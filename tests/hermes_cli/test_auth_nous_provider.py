...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `relayhelm model` → Nous Portal → Skip (keep current) after
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `relayhelm auth add nous`
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Relayhelm must surface an
    ...
    re-authenticate" string led users to report this as a Relayhelm persistence
    ...
    assert "relayhelm auth add nous" in message.lower()
...
def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    ...
    writing to the user's real ``~/.relayhelm/shared/`` across CI runs.
    """
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `relayhelm auth add nous
