...
def _make_gui_build(hermes_home: Path) -> None:
    ...
    desktop = hermes_home / "relayhelm" / "apps" / "desktop"
    ...
    (hermes_home / "relayhelm" / "node_modules").mkdir(parents=True)
...
def test_uninstall_removes_launcher_entry_and_refreshes_cache(tmp_path, monkeypatch):
    ...
    hermes_home = tmp_path / ".relayhelm"
    _make_agent(hermes_home)
    icon = lde.icon_path(hermes_home / "relayhelm")
    ...
    assert lde.icon_path(hermes_home / "relayhelm").exists()
    # The agent itself survives a GUI uninstall.
    assert (hermes_home / "relayhelm" / "hermes_cli").is_dir()
...
def test_uninstall_skips_cache_refresh_when_no_launcher_entry(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(tmp_path / ".relayhelm")
