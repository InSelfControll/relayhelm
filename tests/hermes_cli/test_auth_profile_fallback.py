...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.relayhelm/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.relayhelm            (has its own auth.json fixture)
    * Profile     -> tmp_path/.relayhelm/profiles/coder   (active, HERMES_HOME points here)
    ...
    global_root = tmp_path / ".relayhelm"
    global_root.mkdir()
...
# ``_load_provider_state`` itself, those helpers raise ``"Relayhelm is not
...
def classic_env(tmp_path, monkeypatch):
    """Classic single-root layout (HERMES_HOME != ~/.relayhelm, no profiles)."""
