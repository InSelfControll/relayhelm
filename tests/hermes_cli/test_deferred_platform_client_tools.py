...
loader so ``relayhelm chat`` doesn't import ~20 gateway SDKs. The a2a plugin ships
...
``relayhelm tools`` checklist. The same tools worked in gateway/web processes only
...
class TestA2AClientToolsInCliProcess:
    ...
    def test_a2a_appears_in_the_hermes_tools_checklist(self):
        ...
        # the `relayhelm tools` checklist does.
        discover_plugins()
...
class TestDeferredPlatformToolPreregistration:
    ...
    def test_tools_stay_attributed_after_materialization(
        ...
        """`relayhelm plugins list` must still credit the pre-registered tools.
    ...
    def test_partially_registered_tools_are_still_attributed(
        ...
        `relayhelm plugins list` under-report what the process is carrying, and
