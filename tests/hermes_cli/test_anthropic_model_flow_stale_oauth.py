"""Tests for Bug #12905 fix — stale OAuth token detection in relayhelm model flow.

Bug 3: `relayhelm model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.relayhelm/.env but no valid
