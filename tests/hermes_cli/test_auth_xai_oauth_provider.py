"""Tests for xAI Grok OAuth — tokens stored in Relayhelm auth store (~/.relayhelm/auth.json)."""
...
def _setup_hermes_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Relayhelm auth store at the given root."""
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``relayhelm model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``relayhelm model``
    ...
    assert "relayhelm model" not in rendered.lower()
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Relayhelm that didn't validate) must not be silently honored on the refresh
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `relayhelm model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `relayhelm auth list` reflects truth and
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``relayhelm auth remove xai-oauth 1`` for a
    ...
    # Act: the user runs `relayhelm auth remove xai-oauth 1`.
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Relayhelm invocation."
    )


def test_login_xai_oauth_relogin_clears_suppression_and_reseeds(tmp_path, monkeypatch):
    """remove -> ``relayhelm model`` re-login (``_login_xai_oauth``) must clear the
    ...
    only ``auth_add_command`` cleared it — the ``relayhelm model`` re-login path did
    not. So after remove -> re-login the seed kept skipping and ``relayhelm auth
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`relayhelm auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``relayhelm auth status``, ``relayhelm setup``, and the
