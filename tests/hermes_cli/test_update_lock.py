...
dashboard-spawned ``relayhelm update`` was mid-``npm install``, which then failed
...
class TestHandoffFromOrchestratingUpdater:
    """The Tauri updater holds the marker, then spawns ``relayhelm update``.

    The regression: the child saw its own parent's live marker and exited 2,
    so every GUI update failed with "Relayhelm is still running" and retrying
...
class TestAncestryHandoff:
    ...
    ``hermes-setup`` under ``~/.relayhelm`` is only refreshed by a full installer
