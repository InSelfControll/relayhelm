"""Tests for IRC gateway configuration via `relayhelm setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Relayhelm install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Relayhelm install."""
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `relayhelm setup gateway` experience with IRC present."""
