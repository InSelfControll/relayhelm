...
- Bare `relayhelm setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `relayhelm setup --quick` runs the narrower "fill in missing items" flow.
- `relayhelm setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
class TestExistingInstallDefault:
    """Bare `relayhelm setup` on an existing install = full reconfigure wizard."""
