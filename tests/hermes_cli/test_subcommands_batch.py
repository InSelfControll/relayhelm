...
def test_config_get_unset_subcommands_parse():
    """`relayhelm config get/unset` parse key args (and --json for get)."""
...
# `hermes_cli/auth.py`) prints a deprecation notice pointing at `relayhelm auth` /
# `relayhelm model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `relayhelm --help`.
