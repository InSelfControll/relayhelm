"""Tests for ``relayhelm dashboard --stop`` / ``--status`` flags.

These flags share the detection + kill path with the post-``relayhelm update``
...
class TestDashboardStatus:
    def test_status_no_processes(self, capsys):
        ...
        assert "No relayhelm dashboard or serve processes running" in out
    ...
    def test_status_with_processes(self, capsys):
        ...
        # couldn't see (#81564).
        processes = [
            (12345, "relayhelm dashboard --port 9119"),
            (12346, "python -m hermes_cli.main dashboard --host 0.0.0.0 --port 9120"),
            (12347, "relayhelm serve --host 100.94.65.93 --port 9119"),
        ]
        ...
        assert exc.value.code == 0
        out = capsys.readouterr().out
        assert "3 relayhelm dashboard/serve process(es) running" in out
...
class TestDashboardStop:
    ...
    def test_stop_kills_and_exits_zero_when_all_killed(self, capsys):
        ...
        # for the post-`relayhelm update` path).
...
class TestLifecycleFlagsTakePrecedence:
    ...
    who typed ``relayhelm dashboard --stop`` must not end up ALSO starting
    a new server."""
...
class TestArgparseWiring:
    ...
    ``relayhelm dashboard --stop`` / ``--status`` actually parse."""
