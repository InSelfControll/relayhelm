"""Tests for --ignore-user-config and --ignore-rules flags on `relayhelm chat`.
...
files. In Relayhelm the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.relayhelm/config.yaml`` in ``load_cli_config()``
