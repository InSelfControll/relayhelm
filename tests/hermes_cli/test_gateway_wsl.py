...
class TestGatewayCommandWSLMessages:
    ...
    def test_install_wsl_no_systemd(self, monkeypatch, capsys):
        """relayhelm gateway install on WSL without systemd shows guidance.
        ...
        assert "relayhelm gateway run" in out
        assert "tmux" in out


    @pytest.mark.linux_only
    def test_status_wsl_running_manual(self, monkeypatch, capsys):
        """relayhelm gateway status on WSL with manual process shows WSL note.
