"""Tests for the `relayhelm memory reset` CLI command.
...
def memory_env(tmp_path, monkeypatch):
    ...
    (memories / "MEMORY.md").write_text(
        "§\nHermes repo is at ~/.relayhelm/relayhelm\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
def _run_memory_reset(target="all", yes=False, monkeypatch=None, confirm_input="no"):
    ...
    Simulates what happens when `relayhelm memory reset` is run.
    """
...
class TestMemoryReset:
    """Tests for `relayhelm memory reset` subcommand."""
