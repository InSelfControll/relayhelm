"""Tests for the multi-board kanban layer (``relayhelm kanban boards …``).
...
* CLI surface: ``relayhelm kanban boards list/create/switch/rm``.
...
class TestSlugValidation:
    @pytest.mark.parametrize("good", [
        "default", "atm10-server", "relayhelm", "proj_1", "a",
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``relayhelm kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
