...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Relayhelm", folders=["/tmp/hermes"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "relayhelm"
    assert proj.name == "Relayhelm"
    ...
    # Lookup by slug too.
    assert pdb.get_project(conn, "relayhelm").id == pid
