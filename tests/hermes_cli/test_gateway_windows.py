...
class TestStableWindowsGatewayWorkingDir:
    ...
    def test_stable_gateway_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        missing = tmp_path / "missing" / ".relayhelm"
...
def test_elevated_gateway_command_uses_hidden_console_python(monkeypatch):
    ...
    monkeypatch.setattr(gateway_windows.sys, "executable", r"C:\Relayhelm\venv\Scripts\python.exe")
    ...
    assert verb == "runas"
    assert executable == r"C:\Relayhelm\venv\Scripts\python.exe"
...
# resume_pending=True) never fires when `relayhelm gateway stop` kills the
