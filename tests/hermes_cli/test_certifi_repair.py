...
1. The venv-repair import probes (early recovery + `relayhelm update`) must
...
2. ``relayhelm doctor`` must fail the certificate check in that state, and
   ``relayhelm doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `relayhelm update`'s venv repair must apply
...
# =========================================================================
# 2. relayhelm doctor: detection and --fix repair
# =========================================================================
...
class TestSslGuardRepairHint:
    def test_missing_bundle_error_mentions_doctor_fix(self, monkeypatch, tmp_path):
        ...
        assert "relayhelm doctor --fix" in message
