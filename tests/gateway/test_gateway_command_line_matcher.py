...
Regression guard for the Windows ``relayhelm gateway restart`` silent-outage bug:
...
ACCEPT = [
    ...
    "relayhelm-gateway.exe",
    "relayhelm gateway",          # bare `relayhelm gateway` defaults to run
    "relayhelm gateway run",
    # profile selector AFTER the `gateway` token (argv is profile-position
    # agnostic — _apply_profile_override strips --profile/-p anywhere)
    "relayhelm gateway --profile work run",
    "python -m hermes_cli.main gateway -p work run",
    "relayhelm gateway --profile=work run",
    # a profile literally NAMED "gateway"
    "relayhelm -p gateway gateway run",
    "python -m hermes_cli.main --profile gateway gateway run",
    # quoted Windows paths with spaces (shlex-aware tokenization)
    r'"C:\Program Files\Relayhelm\relayhelm-gateway.exe"',
    r'"C:\Program Files\Relayhelm\gateway\run.py" run',
