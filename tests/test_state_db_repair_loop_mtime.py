...
Incident (2026-08-17): a malformed-SCHEMA state.db sent Relayhelm into an
...
def test_live_connection_keeps_its_write_lock_across_a_repair_pass(tmp_path):
    ...
    POSIX advisory locks, so it is immune. DELETE mode is what Relayhelm falls
...
def test_forensic_backup_includes_the_rollback_journal(tmp_path):
    ...
    Rollback-journal mode is Relayhelm's fallback on NFS/SMB/FUSE/ZFS and on
