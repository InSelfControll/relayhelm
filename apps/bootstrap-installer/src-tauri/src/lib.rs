//! Relayhelm Setup — Tauri entrypoint.
//!
...
pub fn run() {
    ...
    // Relayhelm is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Relayhelm installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Relayhelm is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Relayhelm" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Relayhelm" shortcuts that install.ps1 creates, and a
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::hermes_home().join("relayhelm");
        ...
        .expect("error while running Relayhelm Setup");
}
