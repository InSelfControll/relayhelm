#!/usr/bin/env node
// set-exe-identity.mjs — stamp the Relayhelm icon + version metadata onto the
// built Relayhelm.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Relayhelm.exe keeps the stock Electron icon and
...
//   node scripts/set-exe-identity.mjs <path-to-Relayhelm.exe>
//
...
// Stamp the Relayhelm icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'Relayhelm',
      FileDescription: 'Relayhelm',
      CompanyName: 'Nous Research',
      LegalCopyright: 'Copyright (c) 2026 Nous Research'
    }
  })

  console.log('[set-exe-identity] done — Relayhelm icon + identity stamped')
}
