...
// no longer bundles the Relayhelm Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Relayhelm.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Relayhelm'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Relayhelm.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Relayhelm'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
const DEFAULT_HERMES_HOME = (() => {
  ...
  return path.join(os.homedir(), '.relayhelm')
})()
const VENV_ROOT = path.join(DEFAULT_HERMES_HOME, 'relayhelm', 'venv')
...
function launchFresh() {
  ...
  return { runtimeRoot: path.join(hermesHome, 'relayhelm', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Relayhelm Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  const staleFactoryMarker = path.join(APP.resourcesPath, 'relayhelm', 'hermes_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Relayhelm
