 ...
 *
 * Stamps the Relayhelm icon + identity onto the packed Windows Relayhelm.exe via
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Relayhelm')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Relayhelm'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Relayhelm.exe keeps the stock Electron icon`)
  }
}
