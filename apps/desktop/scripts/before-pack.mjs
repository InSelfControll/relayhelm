 ...
 * product name (`Relayhelm`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Relayhelm'
 *
 ...
 * (the mac-unpacked Relayhelm.app bundle) and Windows (win-unpacked), so we
 ...
 * Relayhelm.exe that Windows can't load (truncated PE from a corrupt cached
 ...
 */
export function preserveRollbackBackup(appOutDir, productExeName = 'Relayhelm.exe') {
...
export default async function beforePack(context) {
  ...
  try {
    ...
    // rename fails.
    const productExe = `${(context && context.packager?.appInfo?.productFilename) || 'Relayhelm'}.exe`
