 ...
 *   2. PortableGit under %LOCALAPPDATA%\relayhelm\git\ (install.ps1)
...
export function findGitBash(opts: GitBashOptions): string | null {
  ...
  if (localAppData) {
    candidates.push(joinWin(localAppData, 'relayhelm', 'git', 'bin', 'bash.exe'))
    candidates.push(joinWin(localAppData, 'relayhelm', 'git', 'usr', 'bin', 'bash.exe'))
  }
