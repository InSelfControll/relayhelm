...
test('does not fall back to heavyweight /api/status for transient health failures', async () => {
  ...
  await assert.rejects(
    waitForHermesReady('http://127.0.0.1:9000', {
      fetchPublicJson: async url => {
        calls.push(['public', url])
        throw new Error('Timed out connecting to Relayhelm backend after 15000ms')
      },
      fetchJson: async url => {
        calls.push(['token', url])
      },
...
test('recognizes missing-route shapes only', () => {
  ...
  assert.equal(
    isMissingHealthEndpointError(
      new Error('Expected JSON from /api/health but got HTML. The endpoint is likely missing on the Relayhelm backend.')
    ),
    true
  )
  assert.equal(isMissingHealthEndpointError(new Error('Timed out connecting to Relayhelm backend after 15000ms')), false)
...
test('unsigned OAuth is a terminal reauth failure; needsOauthLogin alone is not', () => {
  ...
  assert.equal(isReauthRequiredError(new Error('Could not reach the remote Relayhelm gateway')), false)
})
