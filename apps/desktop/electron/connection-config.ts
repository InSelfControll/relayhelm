...
// The Nous portal (NAS) does NOT use Relayhelm gateway session cookies — it is a
...
/** Which Relayhelm profile the remote SSH dashboard should actually run as.
...
// Relayhelm Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function normalizeSshConfig(entry) {
  ...
  // name used by the remote Relayhelm installation. Preserve an explicit mapping
  // when it is a valid Relayhelm profile identifier; otherwise fall back to the
 ...
 * the Relayhelm gateway session cookies, so cloud sign-in / discovery liveness
