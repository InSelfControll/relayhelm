 ...
 *
 *   macOS:   …/Relayhelm.app/Contents/MacOS/Relayhelm  → …/Relayhelm.app
 *   Windows: …\Relayhelm\Relayhelm.exe                 → …\Relayhelm  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Relayhelm.app/Contents/MacOS/Relayhelm → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Relayhelm.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Relayhelm.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Relayhelm$/i.test(dir) || /[\\/]hermes-desktop$/i.test(dir)) {
      return dir
    }
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Relayhelm install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`
