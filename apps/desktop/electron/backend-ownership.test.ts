...
function ownershipEntry(overrides: Partial<BackendIdentity> = {}) {
  return { command: 'relayhelm serve --port 0', ...identity(overrides) }
}
...
test('backend identity check matches only serve and dashboard invocation shapes', () => {
  assert.equal(backendCommandMatches('/venv/bin/relayhelm serve --port 0'), true)
  assert.equal(backendCommandMatches('python -m hermes_cli.main dashboard --no-open'), true)
  assert.equal(backendCommandMatches('/venv/bin/relayhelm --profile work serve --port 0'), true)
  assert.equal(backendCommandMatches('"C:\\Relayhelm Runtime\\relayhelm.exe" dashboard --no-open'), true)
  assert.equal(backendCommandMatches('relayhelm chat --query serve'), false)
