...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/Relayhelm.app/Contents/MacOS/Relayhelm', 'darwin'),
    '/Applications/Relayhelm.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/Relayhelm.app/Contents/MacOS/Relayhelm', 'darwin'),
    '/Users/x/Applications/Relayhelm.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\Relayhelm\\Relayhelm.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\Relayhelm'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\hermes-desktop\\Relayhelm.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\hermes-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\Relayhelm.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_HermesXXXX/hermes', 'linux', { APPIMAGE: '/home/x/Apps/Relayhelm.AppImage' }),
    '/home/x/Apps/Relayhelm.AppImage'
  )
})
...
test('resolveRemovableAppPath finds the unpacked dir on Linux', () => {
  ...
  assert.equal(resolveRemovableAppPath('/usr/bin/relayhelm', 'linux', {}), null)
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/Relayhelm.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/Relayhelm.app'), false)
...
test('buildPosixCleanupScript waits for the PID, runs the uninstall module, removes bundle', () => {
  const script = buildPosixCleanupScript({
    desktopPid: 4321,
    pythonExe: '/home/x/.relayhelm/relayhelm/venv/bin/python',
    pythonPath: null,
    agentRoot: '/home/x/.relayhelm/relayhelm',
    uninstallArgs: ['-m', 'hermes_cli.uninstall', '--mode', 'gui'],
    appPath: '/opt/hermes/linux-unpacked',
    hermesHome: '/home/x/.relayhelm'
  })
  ...
  assert.match(script, /export HERMES_HOME='\/home\/x\/\.relayhelm'/)
})
...
test('buildPosixCleanupScript exports PYTHONPATH when pythonPath is set (lite/full)', () => {
  const script = buildPosixCleanupScript({
    ...
    pythonPath: '/home/x/.relayhelm/relayhelm',
    agentRoot: '/home/x/.relayhelm/relayhelm',
  ...
  // venv is torn down.
  assert.match(script, /export PYTHONPATH='\/home\/x\/\.relayhelm\/relayhelm'/)
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\hermes',
    uninstallArgs: ['-m', 'hermes_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\Relayhelm',
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\Relayhelm" >nul 2>&1/)
