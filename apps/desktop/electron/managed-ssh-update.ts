 ...
 * wire protocol:
 *
 *   gate dials -> drain every captured scope -> detached `relayhelm update`
...
function buildPosixManagedUpdateLaunch(target: RemoteUpdateTarget, correlationId: string): string {
  ...
  const home = validateRemoteValue(target.hermesHome, 'Relayhelm home')
...
async function runManagedSshUpdate<TScope extends ManagedSshScope>(
  ...
  return {
    ...
    message:
      outcome === 'updated'
        ? 'Remote Relayhelm updated and every managed SSH profile is ready.'
        : restoreOk
