...
test('parseRegQueryValue extracts a REG_SZ value', () => {
  const out = ['', 'HKEY_CURRENT_USER\\Environment', '    HERMES_HOME    REG_SZ    F:\\Relayhelm\\data', ''].join('\r\n')
  assert.equal(parseRegQueryValue(out, 'HERMES_HOME'), 'F:\\Relayhelm\\data')
})
...
test('parseRegQueryValue preserves spaces inside the value', () => {
  const out = '    HERMES_HOME    REG_SZ    C:\\Program Files\\Relayhelm\r\n'
  assert.equal(parseRegQueryValue(out, 'HERMES_HOME'), 'C:\\Program Files\\Relayhelm')
})
...
test('expandWindowsEnvRefs leaves literal paths and unknown refs intact', () => {
  assert.equal(expandWindowsEnvRefs('F:\\Relayhelm\\data', {}), 'F:\\Relayhelm\\data')
...
test('readWindowsUserEnvVar queries HKCU\\Environment and expands the value', () => {
  ...
  const exec = (cmd, args) => {
    ...
    return 'HKEY_CURRENT_USER\\Environment\r\n    HERMES_HOME    REG_EXPAND_SZ    %DRIVE%\\Relayhelm\r\n'
  }
  ...
  assert.equal(value, 'F:\\Relayhelm')
