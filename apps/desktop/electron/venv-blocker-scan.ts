 ...
 * preview servers. Unknown Python/Relayhelm processes are deliberately ignored.
 */
...
export function formatBlockerMessage(result: VenvBlockerScanResult): string {
  const lines = [
    'Update aborted: another Relayhelm process is using this installation.',
    '',
...
export function formatProbeFailedMessage(error?: string): string {
  ...
  return (
    'Update aborted: Desktop could not verify the Relayhelm installation is free.' +
    timeoutDetail +
    '\n\n' +
    'Close other Relayhelm windows and terminals, then retry.  If the problem\n' +
    'persists, run `relayhelm update` in a terminal for detailed diagnostics.'
  )
}
