 ...
 * packaged binary) with a real `relayhelm serve` backend pointed at a mock
 * inference server. The full chain is exercised:
 *
 *   electron → relayhelm serve (python) → mock provider → renderer
 *
...
test.describe('dev-mode boot with mock backend', () => {
  test('window opens with Relayhelm title', async () => {
    const title = await fixture!.page.title()
    expect(title).toContain('Relayhelm')
  })
  ...
  test('backend boots and app becomes ready', async () => {
    ...
    // electron starts → relayhelm serve is spawned → WS connects → config
