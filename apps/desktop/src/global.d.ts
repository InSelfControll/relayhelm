...
declare global {
  interface Window {
    hermesDesktop: {
      ...
      // Resume this session in the user's own terminal emulator (`relayhelm --tui
      ...
      connections: {
        ...
        // Fan out `relayhelm update` to every eligible registered connection;
      ...
      // Relayhelm Cloud: one portal login powers discovery + silent per-agent
...
export interface DesktopUpdateApplyResult {
   ...
   *  `relayhelm update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  hermesRoot?: string
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Relayhelm Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Relayhelm Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteUrl?: string
  // For a 'cloud' connection: the selected Relayhelm Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the relayhelm gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Relayhelm Cloud agent — the trimmed DTO from NAS GET /api/agents.
