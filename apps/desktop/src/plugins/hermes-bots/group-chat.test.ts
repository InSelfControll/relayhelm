...
describe('speaker labels', () => {
  it('the default profile speaks as Relayhelm in transcripts, not @default', async () => {
    ...
    expect(line).toBe('Relayhelm: hello room')
    ...
    ).toBe('Relayhelm (you): hi')
    expect(
      rounds.formatGroupChatLine({ from: { kind: 'member', name: 'builder' }, text: 'yo' } as GroupMessage, 'research')
    ).toBe('builder: yo')
  })

  it('honor friendly identity: Bot Mode title, then display_name, never a stale Relayhelm', async () => {
    const { chat, rounds } = await loadRoom()
    const data = await import('./data')

    // A renamed default (core display_name via `relayhelm profile rename`) must
    // read as its new name — the community report was "Lucy" still showing
    // "Relayhelm is thinking…" in group rooms.
    ...
    // Untitled rows keep today's behavior: default → Relayhelm, others verbatim.
    data.$botMeta.set({})

    expect(chat.groupSpeakerLabel('default')).toBe('Relayhelm')
