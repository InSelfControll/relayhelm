...
export function displayName(bot: Partial<RosterRow>, meta?: BotMeta | null): string {
  ...
  // "Relayhelm". Annotated active rows carry sourceScoped too, and keying this
  ...
  // Core-profile display name (profile.yaml, set via `relayhelm profile rename
  ...
  // never generic "Relayhelm" or a hostname-derived label.
  if (alias) {
  ...
  // that reads like nobody bothered. Present it as Relayhelm (the agent it is)
