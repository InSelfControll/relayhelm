 ...
 *  clients (Relayhelm Go, headless backends) see rooms regardless of which
 ...
 *  renamed bot never shows up as its raw profile id or a stale "Relayhelm"
 *  (community report, Aug 21 2026: renamed default still read "Relayhelm is
 *  thinking…" in group rooms). The untitled primary profile is literally
 *  named "default" — render it as Relayhelm (matching displayName and the
...
export function groupSpeakerLabel(name?: null | string) {
  ...
  // Core profile display_name (`relayhelm profile rename …` / dashboard) from
  ...
  return trimmed.toLowerCase() === 'default' ? 'Relayhelm' : trimmed
}
