 ...
 * billing-specific toast — never the generic "Relayhelm error" — with a smart CTA
  ...
  if (event.type === 'message.complete') {
    ...
    const failure =
      payload?.status === 'error'
        ? {
            error: coerceGatewayText(payload.error).trim() || finalText || 'Relayhelm reported an error',
