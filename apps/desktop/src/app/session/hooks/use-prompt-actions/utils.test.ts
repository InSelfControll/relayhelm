...
describe('renderRpcResult', () => {
  ...
  describe('session.save', () => {
    it('echoes the saved file path', () => {
      expect(renderRpcResult({ file: '/home/user/.relayhelm/sessions/saved/x.json' }, 'save')).toBe(
        'Saved transcript to /home/user/.relayhelm/sessions/saved/x.json'
      )
    })
  })
  ...
  describe('session.status', () => {
    it('passes through the multi-line plain-text output verbatim', () => {
      const output = 'Relayhelm TUI Status\n\nSession ID: s-1\nModel: nous-hermes-3 (unknown)'
