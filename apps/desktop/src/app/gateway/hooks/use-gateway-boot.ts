...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    // composer stayed disabled forever on "Starting Relayhelm...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Relayhelm…". The probe is a no-op for a healthy or local backend.
        ...
        const conn = await withTimeout(
          ...
          RECONNECT_ATTEMPT_TIMEOUT_MS,
          'Timed out reconnecting to Relayhelm backend'
        )
        ...
        // as an opaque "Could not connect to Relayhelm gateway". resolveGatewayWsUrl
    ...
    async function boot() {
      try {
        ...
        // hanging "Starting Relayhelm…" forever.
        const wsUrl = await withTimeout(
          resolveGatewayWsUrl(desktop, conn),
          RECONNECT_ATTEMPT_TIMEOUT_MS,
        ...
        await Promise.all([
          ...
          // retry it), not brick boot behind the "Relayhelm couldn't start"
