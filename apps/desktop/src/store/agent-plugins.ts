...
/**
 * Feature store for backend (agent) plugins — the native Relayhelm plugins plus
 ...
 * primitives `relayhelm plugins` and the dashboard use, so all surfaces agree on
...
export interface AgentPluginRow {
  ...
  /** Agent Plugins v1 package (portable skills/MCP format) vs native Relayhelm. */
  portable?: boolean
}
