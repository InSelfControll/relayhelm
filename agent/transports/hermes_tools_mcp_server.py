...
Codex owns the loop and tool list there, so a curated subset of Relayhelm tools is
...
# Each name MUST match a registered Relayhelm tool ``model_tools.handle_function_call()`` can dispatch.
...
def _build_server() -> Any:
    """Create the MCP server with Relayhelm tools attached (lazy imports: importable without ``mcp``)."""
    try:
    ...
    mcp = MCPServer(
        "hermes-tools",
        instructions=(
            "Relayhelm's tool surface, exposed for use inside a Codex "
    ...
    # Authoritative Relayhelm schemas so MCP clients see the same parameter docs the model does.
    all_defs = {
    ...
    for name in EXPOSED_TOOLS:
        ...
        if spec is None:
            logger.debug("skipping %s — not registered in this Relayhelm process", name)
            continue
        description = spec.get("description") or f"Relayhelm {name} tool"
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    # Keep Relayhelm' own banners off stdout (the MCP wire).
