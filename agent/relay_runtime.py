"""Profile-scoped NeMo Relay runtimes owned by the Relayhelm agent core."""
...
@dataclass
class RelaySession:
    """One isolated Relay scope stack owned by a Relayhelm session."""
    ...
    def run_in_session(self, session: RelaySession, callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        ...
        with self._lock:
            if self._runtime is None:
                raise RuntimeError("Relayhelm Relay operation lease is released")
...
class _ProcessRelayPluginConfiguration:
        ...
        with self._lock:
            if not self._owners:
                ...
                if self._state is _RelayPluginConfigurationState.ACTIVE:
                    logger.info(
                        "Relay plugins are active process-wide and apply to all profiles hosted by this Relayhelm process."
                    )
    ...
    def _activate(self, relay: Any) -> _RelayPluginConfigurationState:
        try:
            ...
            logger.warning("Relayhelm Relay plugin initialization failed: %s", exc, exc_info=True)
    ...
    def _preflight(self, relay: Any) -> _RelayPluginConfigurationState | None:
        ...
        if self._relay is not None and not self._clear_active():
            logger.warning(
                "Relayhelm Relay plugin cleanup is still pending; refusing to replace the process-global configuration"
            )
            return _RelayPluginConfigurationState.FAILED
        try:
            ...
            logger.warning(
                "Relayhelm could not determine whether a process-global Relay plugin configuration is already "
                "active; refusing to replace it", exc_info=True,
            )
            return _RelayPluginConfigurationState.FAILED
        if existing_report is not None:
            logger.warning(
                "A process-global Relay plugin configuration is already active outside Relayhelm native ownership; "
    ...
    def _initialize(self, relay: Any) -> bool:
        ...
        if dynamic_plugins:
            try:
                ...
                raise RuntimeError("Relayhelm Relay dynamic plugin activation failed") from exc
    ...
    def _clear_active(self) -> bool:
        ...
        for what, step in (
            ...
            try:
                ...
                logger.warning("Relayhelm Relay plugin %s failed", what, exc_info=True)
                return False
...
class RelayRuntime:
    ...
    def rotate_session_scope(self, session: RelaySession, *, reason: str) -> None:
        ...
        with session.lock:
            ...
            try:
                ...
                logger.warning(
                    "Relayhelm Relay segment close failed (session=%s segment=%d); abandoning the old segment span",
            ...
            try:
                ...
                logger.warning(
                    "Relayhelm Relay segment open failed (session=%s segment=%d); keeping the prior scope handle",
    ...
    def get_session(self, session_id: str) -> RelaySession | None:
        """Return an active Relayhelm Relay session without creating one."""
        with self._sessions_lock:
    ...
    def _session_context(self, session: RelaySession, *, allow_closing: bool) -> contextvars.Context:
        ...
        with session.lock:
            if session.closing and not allow_closing:
                raise RuntimeError("Relayhelm Relay session is closing")
            if session.context is None or session.handle is None:
                raise RuntimeError("Relayhelm Relay session context is unavailable")
    ...
    def _begin_operation(self) -> None:
        ...
        with self._sessions_lock:
            if self._closing:
                raise RuntimeError("Relayhelm Relay runtime is shutting down")
    ...
    def apply_tool_request_intercepts(self, *, session_id: str, tool_name: str, args: dict[str, Any]) -> dict[str, Any]:
        """Apply Relay request rewriting before Relayhelm authorizes a tool call."""
    ...
    def _pop_with_drain(
        ...
        for _ in range(drain_limit):
            ...
            try:
                ...
                logger.warning("Relayhelm Relay orphaned scope drain failed", exc_info=True)
                break
        if drained:
            logger.warning("Relayhelm Relay drained %d orphaned scope(s) before closing %s", drained, handle)
        try:
            pop_relay_scope(self.relay, handle, output=output, metadata=metadata)
            return None
    ...
    def _close_session(self, event: dict[str, Any]) -> None:
        ...
        with self._sessions_lock:
            ...
            self._forget_subagent(session_id)
        if failure:
            logger.warning("Relayhelm Relay session %s closed with errors: %s", session_id, failure)
    ...
    def shutdown(self) -> None:
        ...
        try:
            ...
            logger.warning("Relayhelm Relay deferred shutdown could not start", exc_info=True)
    ...
    def _finish_shutdown(self) -> None:
        try:
            ...
            with self._sessions_lock:
                self._shutdown_started = False
            logger.warning("Relayhelm Relay shutdown failed", exc_info=True)
            return
        with self._sessions_lock:
...
class RelayHostRegistry:
    """Own exactly one Relay host for each canonical Relayhelm profile."""
    ...
    def for_profile(self, profile_key: str | None = None, *, create: bool = True) -> RelayHost | None:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning("Relayhelm Relay runtime initialization failed", exc_info=True)
...
class RelayTurnContext:
    """Runtime-only context for one Relayhelm turn or top-level task."""
...
# >0 while the native pipeline is mid-dispatch of a Relayhelm tool/LLM callback. Nested managed
...
def _warn_on_error(what: str, callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
    """Run fail-open telemetry work: log ``Relayhelm Relay <what> failed`` and return None on error."""
    try:
        return callback(*args, **kwargs)
    except Exception:
        logger.warning("Relayhelm Relay %s failed", what, exc_info=True)
        return None
...
class RelaySessionCoordinator:
    """Own semantic conversation and turn lifetimes for Relayhelm core."""
    ...
    def _prepare_session(self, host: RelayRuntime, context: dict[str, Any]) -> None:
        ...
        for name, callback in initializers:
            try:
                ...
                logger.warning("Relayhelm Relay session initializer failed: %s", name, exc_info=True)
    ...
    def begin_turn(self, lease: ConversationLease, *, turn_id: str, task_id: str) -> RelayTurnContext:
        if lease.released:
            raise RuntimeError("Relayhelm Relay conversation lease is released")
        ...
        with self._active_turns_lock:
            if self._active_turns.get(key):
                ...
                logger.warning(
                    "Skipping Relay instrumentation for concurrent Relayhelm turn %s in session %s",
    ...
    def _close_turn_scope(self, host: RelayRuntime, turn: RelayTurnContext, *, outcome: str) -> None:
        ...
        if failure:
            logger.warning("Relayhelm Relay turn finalization failed: %s", failure)
    ...
    def _finish_logical_calls(turn: RelayTurnContext, *, outcome: str) -> None:
        ...
        while logical_calls:
            ...
            logger.warning("Relayhelm Relay logical LLM finalization failed: %s", failure)
            break
...
def resolve_execution_context(session_id: str) -> tuple[RelayRuntime | None, RelaySession | None, Any]:
    ...
    if _MANAGED_CALLBACK_DEPTH.get() > 0 or not relay_instrumentation_enabled():
        ...
        # ``tools.execute`` pipeline is mid-dispatch of a Relayhelm tool). Nested managed execution here is
...
def apply_tool_request_intercepts(*, session_id: str, tool_name: str, args: dict[str, Any]) -> dict[str, Any]:
    """Return Relay-rewritten arguments at Relayhelm's authorization boundary."""
...
def get_runtime(*, create: bool = True, profile_key: str | None = None) -> RelayRuntime | None:
    """Return the Relay host for the active Relayhelm profile."""
...
def _configured_plugin_inputs(relay: Any) -> tuple[dict[str, Any], list[Any]] | None:
    ...
    try:
        ...
        if "dynamic_plugins" in config:
            raise ValueError("Relayhelm [[dynamic_plugins]] records are unsupported; use Relay [[plugins.dynamic]] records")
        ...
        raise _RelayPluginConfigurationLoadError(
            f"Relayhelm Relay plugin configuration could not be loaded from {config_path}; continuing without Relay plugins"
        ) from exc
...
def emit_mark(
    ...
    *,
    ...
    """Emit a fail-open Relay mark under a Relayhelm session."""
    runtime = get_runtime(create=False)
    if runtime is None:
        return False
    try:
        ...
        logger.warning("Relayhelm Relay mark failed: %s", name, exc_info=True)
        return False

def ensure_session(*, session_id: str, **context: Any) -> RelaySession | None:
    """Create or return the shared Relay session used by Relayhelm core."""
    runtime = get_runtime()
    if runtime is None:
        return None
    try:
        ...
        logger.warning("Relayhelm Relay session initialization failed", exc_info=True)
        return None
...
def run_in_session(
    ...
    **kwargs: Any,
    ...
    """Run a scope, LLM, or tool API against a shared Relayhelm session."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Relayhelm Relay runtime is unavailable")
    session = runtime.get_session(session_id)
    if session is None:
        session = runtime.ensure_session({"session_id": session_id})
    if session is None:
        raise RuntimeError("Relayhelm Relay session is unavailable")
...
async def run_in_session_async(
    ...
    **kwargs: Any,
    ...
    """Await a Relay operation inside a shared Relayhelm session context."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Relayhelm Relay runtime is unavailable")
    session = runtime.get_session(session_id)
