# Contributing to Relayhelm

Thank you for contributing to Relayhelm! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Relayhelm to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo InSelfControll/relayhelm "<your terms>"
  gh search prs --repo InSelfControll/relayhelm --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/InSelfControll/relayhelm/issues?q=) · [PRs (all states)](https://github.com/InSelfControll/relayhelm/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Relayhelm itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Relayhelm install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `relayhelm skills browse` (labeled "official") and install it with `relayhelm skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `relayhelm skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.relayhelm/plugins/` (or via a pip entry point).
...
- Integrate with `relayhelm memory setup` via `post_setup()` — no need to touch core code
...
The reason is maintenance load, not quality. Every external product absorbed into the core tree becomes ours to keep working against a fast-moving codebase, for a backend we don't own and can't control. Relayhelm ships a lot and the core moves quickly; coupling third-party products into it creates an open-ended burden on the maintainers.

Publish these as a **standalone plugin repo** instead:

- Implement the relevant ABC and use the existing plugin discovery path (`~/.relayhelm/plugins/`, project `.relayhelm/plugins/`, or a pip entry point) — see [Build a Relayhelm Plugin](https://hermes-agent.nousresearch.com/docs/guides/build-a-hermes-plugin)
...
The installer creates the Relayhelm venv, wires the `hermes` command, stamps the
install method for `relayhelm update`, and clones the full git project into
`$HERMES_HOME/relayhelm` (usually `~/.relayhelm/relayhelm`). That keeps your
development environment on the same layout the CLI, updater, lazy dependency
installer, gateway, and docs assume.

```bash
curl -fsSL https://raw.githubusercontent.com/InSelfControll/relayhelm/main/scripts/install.sh | bash
cd "${HERMES_HOME:-$HOME/.relayhelm}/relayhelm"
...
Use this only if you intentionally do not want Relayhelm' managed install layout
...
uv venv ~/.relayhelm/venvs/hermes-dev --python 3.11
...
```bash
mkdir -p ~/.relayhelm/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.relayhelm/config.yaml
touch ~/.relayhelm/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.relayhelm/.env
```
...
```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/relayhelm" ~/.local/bin/relayhelm
```
...
│
├── skills/                   # Bundled skills (copied to ~/.relayhelm/skills/ on install)
├── optional-skills/          # Official optional skills (discoverable via hub, not activated by default)
├── tests/                    # Test suite
├── website/                  # Documentation site (hermes-agent.nousresearch.com)
│
├── cli-config.yaml.example   # Example configuration (copied to ~/.relayhelm/config.yaml)
└── AGENTS.md                 # Development guide for AI coding assistants
```

### User configuration (stored in `~/.relayhelm/`)

| Path | Purpose |
|------|---------|
| `~/.relayhelm/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.relayhelm/.env` | API keys and secrets |
| `~/.relayhelm/auth.json` | OAuth credentials (Nous Portal) |
| `~/.relayhelm/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.relayhelm/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.relayhelm/state.db` | SQLite session database |
| `~/.relayhelm/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.relayhelm/cron/` | Scheduled job data |
| `~/.relayhelm/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`hermes_state.py`) with full-text search and unique session titles. Per-session JSON snapshots in `~/.relayhelm/sessions/` were superseded by the SQLite store and are off by default; opt back in with `sessions.write_json_snapshots: true` if you have external tooling that consumes the JSON files directly.
...
The user may skip setup and keep loading the skill. Relayhelm only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `relayhelm setup` or update `~/.relayhelm/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Relayhelm tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Relayhelm" is the secondary collaborator. If the contributor's commit shows "Relayhelm" as author because they used Relayhelm to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Relayhelm tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `relayhelm --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Relayhelm uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.relayhelm/skins/<name>.yaml`:
...
Relayhelm runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
Relayhelm has terminal access. Security matters.
...
- Use [GitHub Issues](https://github.com/InSelfControll/relayhelm/issues)
- Include: OS, Python version, Relayhelm version (`relayhelm --version`), full error traceback
