# Relayhelm Security Policy

This document describes Relayhelm's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/InSelfControll/relayhelm/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **Relayhelm does not operate a bug
bounty program.**
...
- Environment details (`relayhelm --version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Relayhelm access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Relayhelm's documentation
...
Relayhelm supports two OS-level isolation postures. They address
...
Relayhelm supports this in two ways:

- **Relayhelm's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Relayhelm's in-process heuristics
...
Relayhelm filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Relayhelm
itself. Bugs in Relayhelm's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Relayhelm does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Relayhelm's own documentation, or reasonable
  operator expectations would predict — including cases where
  Relayhelm has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Relayhelm's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Relayhelm. Bugs in Relayhelm
...
- Relayhelm includes supply-chain guards for MCP server
