...
// ~/.relayhelm matches get_hermes_home()'s default.
const bootFilePath = () => join(process.env.HERMES_HOME ?? join(homedir(), '.relayhelm'), 'tui-theme-boot.json')

// Never touch the user's real ~/.relayhelm from test runs (the TS suite has no
