...
describe('createGatewayEventHandler', () => {
  ...
  it('annotates gateway.start_timeout with stderr tail lines so users can diagnose without /logs', () => {
    ...
    onEvent({
      payload: {
        ...
        stderr_tail:
          '[startup] timed out\nModuleNotFoundError: No module named openai\nFileNotFoundError: ~/.relayhelm/config.yaml'
      },
  ...
  it('shows setup panel for missing provider startup error', () => {
    ...
    onEvent({
      payload: {
        message:
          'agent init failed: No LLM provider configured. Run `relayhelm model` to select a provider, or run `relayhelm setup` for first-time configuration.'
      },
      type: 'error'
    } as any)
