...
/**
 * User widget apps — Relayhelm authors its own TUI widgets, mirroring the
 ...
 * React.createElement.
 *
 * Trust model matches `~/.relayhelm/plugins/`: files under HERMES_HOME execute
...
const widgetsDir = () => join(process.env.HERMES_HOME?.trim() || join(homedir(), '.relayhelm'), 'tui-widgets')
 ...
 *  every change, so a widget Relayhelm writes appears within ~a second — no
...
export function watchUserWidgets(dir = widgetsDir()): void {
  ...
  if (!attach()) {
    ...
    // user (or Relayhelm) ever writes must hot-load too — a 10s poll here read
