...
React + Ink terminal UI for Relayhelm. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
- `src/app/setupHandoff.ts` — launches external `relayhelm setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.relayhelm/.hermes_history` or under `HERMES_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `relayhelm setup` wizard, suspends Ink while it runs
...
ui-tui/
  ...
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external relayhelm setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.relayhelm/tui-parent.log
