...
There are two ways to add a platform to the Relayhelm gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.relayhelm/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Relayhelm code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `relayhelm gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
Checklist for integrating a platform directly into the Relayhelm core.
...
And add it to the `relayhelm-gateway` composite:
