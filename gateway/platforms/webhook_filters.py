...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.relayhelm to the active profile home."""
    ...
    if raw == "~/.relayhelm" or raw.startswith("~/.relayhelm/"):
        return hermes_home / raw[len("~/.relayhelm/"):]
...
def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    ...
    if raw_text == "~/.relayhelm" or raw_text.startswith("~/.relayhelm/"):
