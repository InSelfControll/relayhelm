# Relayhelm Observer Hooks

Relayhelm observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Relayhelm' planner, model providers, memory, tool registry,
...
Relayhelm also has a first-party NeMo Relay shared-metrics path. It uses these
...
Hook callbacks are fail-open. Relayhelm catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Relayhelm only builds
...
The native NeMo Relay SDK integration maps Relayhelm session, turn, LLM, and tool
