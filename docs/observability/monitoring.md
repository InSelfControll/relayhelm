...
Relayhelm gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Relayhelm's native NeMo Relay SDK
...
The OpenTelemetry SDK is an optional extra (`pip install 'relayhelm[otlp]'`),
...
a plain `relayhelm gateway run`: the exporter lives in the gateway process, so
...
Relayhelm-owned Relay transport health remains in scope. A separate gateway
...
release covers only Relayhelm-owned service-health and operational-diagnostic
signals, including Relayhelm-owned Relay transport health. Team Gateway's
