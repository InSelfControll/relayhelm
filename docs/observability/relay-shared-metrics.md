...
Relayhelm includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Relayhelm and does not require a Relayhelm observability plugin. Relayhelm remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Relayhelm execution remains available, while Relay scopes, middleware, plugins,
and subscribers are unavailable. The `relayhelm[nemo-relay]` extra remains
as a no-op compatibility alias for existing installation commands.

> [!WARNING]
> This removes the Relayhelm `observability/nemo_relay` plugin. Existing users
...
> longer activate exporters. Without the new variable, Relayhelm does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Relayhelm requires NeMo Relay 0.8.3 or later within the 0.8 release line. That
line provides the provider-codec and canonical tool-result contracts Relayhelm
uses for managed provider and tool calls.

## Runtime Dependency and Data Boundary

Relayhelm installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Relayhelm process so configured interceptors can
...
Collection remains off unless Relayhelm policy enables it:
...
variable is unset, Relayhelm does not invoke Relay's plugin initializer, so Relay
...
selected file cannot be loaded, Relayhelm reports the error and does not invoke
...
Relay plugin configuration is a process-level deployment choice, not a Relayhelm
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Relayhelm process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Relayhelm logs:
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Relayhelm
process and share the same policy boundary.

Run profiles in separate Relayhelm processes when they require different trust
...
Relayhelm core owns one Relay host and one isolated Relay session scope per Relayhelm
...
marks do not require Relayhelm plugin registration. Shared-metrics marks must
...
```text
Relayhelm turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Relayhelm shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Relayhelm sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Relayhelm used for the logical call, such as
...
counter in any rolling 24-hour window. The metric has no dimensions; Relayhelm
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Relayhelm processes share the SQLite latch, so simultaneous
...
the owning Relayhelm session. The start counter contains only bounded execution
...
provider attempts for the same Relayhelm API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Relayhelm tool-call
...
cancellations. Active task ownership follows the task ID if Relayhelm rotates its
...
outcome, approval outcome, latency, and explicit retry-count buckets. Relayhelm
...
tool or plugin names. Relayhelm does not infer retries from repeated tool names or
...
state, and a use-count bucket. Relayhelm derives reuse and patch-generation
...
are written with atomic replacement. Each package records the Relayhelm version,
...
The install identity is scoped to one `HERMES_HOME`. To reset it, stop Relayhelm
...
Run a real Relayhelm CLI turn against the deterministic local model server:
...
`$HERMES_HOME/telemetry/shared_metrics/outbox/` to the Relayhelm telemetry ingest
