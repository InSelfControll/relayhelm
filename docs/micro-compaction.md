...
has to be thrown away or summarized. Relayhelm has always done this in one batch:
...
Relayhelm folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Relayhelm's normal session flush is
...
With `micro_compact` unset or `false` Relayhelm behaves exactly as it always has:
