...
a provider run fails, or delivery starts failing. `relayhelm cron list` shows some of
...
Add `relayhelm cron doctor` as a read-only diagnostic command that summarizes cron
...
- `relayhelm cron doctor` returns `0` and prints a healthy message when active jobs
