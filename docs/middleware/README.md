# Relayhelm Middleware

Relayhelm middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Relayhelm'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Relayhelm calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Relayhelm retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Relayhelm guardrails,
...
Relayhelm stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Relayhelm runs
them as a nested chain in registration order. Middleware failures are fail-open:
Relayhelm logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Relayhelm applies middleware in this order:
...
For each tool call, Relayhelm applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Relayhelm pre-execution path against the effective arguments:
...
mkdir -p "$HERMES_HOME"
relayhelm plugins enable <plugin-name>
relayhelm chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run relayhelm plugins enable <plugin-name>
uv run relayhelm chat --query 'Reply exactly ok'
```
...
Return the same response shape Relayhelm expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Relayhelm treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Relayhelm preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Relayhelm does not convert downstream
