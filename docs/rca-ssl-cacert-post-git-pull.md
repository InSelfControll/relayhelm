# RCA: SSL CA cert bundle corruption after `relayhelm update`
...
When the venv is partially refreshed, or when one of those env vars points at a file that no longer exists, provider client construction can fail before Relayhelm has enough context to produce a useful message.
...
For a normal corrupted Relayhelm venv, reinstall the affected client dependencies:

```bash
python -m pip install --force-reinstall certifi openai httpx
```

For a custom/corporate CA setup, fix the env var so it points at a real PEM bundle, or unset it if Relayhelm should use the bundled `certifi` store.
