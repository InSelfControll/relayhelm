...
Supersedes: PR #31781 (prompt_toolkit `relayhelm profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `relayhelm profile wizard` in the terminal.
...
A profile = a full `~/.relayhelm/profiles/<name>/` directory with its own:
...
5. spawn `relayhelm -p <profile> skills install <id>` per hub skill (async, returns PIDs)
