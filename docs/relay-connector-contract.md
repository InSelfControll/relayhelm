...
This document is the formal interface between the **Relayhelm gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `relayhelm gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`relayhelm gateway enroll` → connector `/relay/enroll`, or
