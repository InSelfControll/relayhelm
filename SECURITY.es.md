# Política de Seguridad de Relayhelm

Este documento describe el modelo de confianza de Relayhelm, identifica el
...
Reporta de forma privada a través de [GitHub Security Advisories](https://github.com/InSelfControll/relayhelm/security/advisories/new)
o **security@nousresearch.com**. No abras issues públicos para
vulnerabilidades de seguridad. **Relayhelm no opera un programa de
...
- Detalles del entorno (`relayhelm --version`, SHA del commit, SO, versión de Python).
...
Relayhelm es un agente personal de un solo inquilino. Su postura es
...
- **Proceso del agente.** El intérprete Python que ejecuta Relayhelm,
...
- **Envolvente de confianza.** El conjunto de recursos a los que un operador ha otorgado
  implícitamente acceso a Relayhelm al ejecutarlo — típicamente, todo lo que
  la propia cuenta de usuario del operador puede alcanzar en el host.
- **Postura.** Una declaración explícita en la documentación o código de Relayhelm
...
Relayhelm admite dos posturas de aislamiento a nivel de SO. Abordan
...
Relayhelm admite esto de dos maneras:

- **La propia imagen Docker de Relayhelm y la configuración de Compose.** Más
...
Bajo una envoltura de proceso completo, las heurísticas en proceso de Relayhelm
...
Relayhelm filtra el entorno que pasa a sus componentes en proceso de
...
Un plugin malicioso o con errores no es una vulnerabilidad en Relayhelm
en sí mismo. Los errores en la ruta de instalación o descubrimiento de plugins de Relayhelm
...
4. **Dentro del conjunto autorizado, todos los llamadores tienen la misma confianza.**
   Relayhelm no modela capacidades por llamador dentro de un único adaptador.
...
- Violaciones de la documentación del modelo de confianza: código que se comporta
  contrariamente a lo que esta política, la propia documentación de Relayhelm o
  las expectativas razonables del operador predecirían — incluyendo casos donde
  Relayhelm ha documentado una postura sobre cómo su salida debe ser
...
- **Habilidades y plugins contribuidos por la comunidad.** Las habilidades de terceros
  ...
  están en la superficie de revisión del operador, no en la superficie de confianza de Relayhelm
  (§2.4, §2.5). Una habilidad o plugin que haga algo
  malicioso es el modo de falla esperado de uno que no fue
  revisado, no una vulnerabilidad en Relayhelm. Los errores en la ruta de
  instalación de habilidades o plugins de Relayhelm que impidan al
...
- Relayhelm incluye guardias de cadena de suministro para lanzamientos de servidores
