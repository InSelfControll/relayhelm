...
the `cronjob` tool, the `relayhelm cron` CLI (`list`, `add`, `edit`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  ...
  zero tokens.
- **Telemetry:** sidecar at `~/.relayhelm/skills/.usage.json` holds
...
Users drive it via `relayhelm kanban <verb>`; dispatcher-spawned workers
