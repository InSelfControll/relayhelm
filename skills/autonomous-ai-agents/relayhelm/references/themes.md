# Themes / Skins — Author a Relayhelm Color Theme

Author a Relayhelm **skin** — one YAML file that themes the CLI, the TUI, and the
...
`~/.relayhelm/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Relayhelm itself.
...
- Write access to the Relayhelm home dir — `~/.relayhelm` by default, or `$HERMES_HOME`
  / the active profile's dir. Skins live in `<hermes-home>/skins/`.
- Native tools: `write_file` (create the YAML), `read_file` / `search_files`
  (inspect existing skins), `terminal` (activate via `relayhelm config set`).
...
3. **Apply it yourself — never hand-edit `config.yaml`.** Run the safe writer via
   `terminal`:
   ```
   relayhelm config set display.skin <name>
   ```
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `relayhelm config set display.skin default` (or they can `/skin default`).
...
- **Don't hardcode `~/.relayhelm`** when a profile is active — resolve the real home
  from `$HERMES_HOME` first, falling back to `~/.relayhelm`.
...
- **Never hand-edit `config.yaml` to activate.** Use `relayhelm config set
...
- **You apply it, not the user.** `relayhelm config set display.skin <name>` is
...
- Run `relayhelm config get display.skin` and confirm it reports `<name>`.
