...
Edit with `relayhelm config edit` or `relayhelm config set section.key value`.
...
`relayhelm config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `relayhelm tools` (interactive) or `relayhelm tools enable/disable NAME`.
