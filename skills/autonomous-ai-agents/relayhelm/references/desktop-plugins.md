...
Write plugins for the Relayhelm desktop app: statusbar items, layout panes,
...
system (`~/.relayhelm/plugins/`) is otherwise documented separately.
...
- `ctx.os` — the curated OS door, attributed to your plugin:
  ...
  posts a native OS notification. Fires only while the user is away from Relayhelm
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.relayhelm/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
