...
Config: `~/.relayhelm/config.yaml` (settings), `~/.relayhelm/.env` (API keys) — both under `$HERMES_HOME` when it is set.
...
never hardcode `~/.relayhelm`. For custom/local-only tools, write a plugin in
`~/.relayhelm/plugins/` instead of editing core — see the developer docs.
...
- Tests auto-redirect `HERMES_HOME` to temp dirs — never touch real `~/.relayhelm/`.
