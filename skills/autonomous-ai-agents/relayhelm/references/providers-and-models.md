...
Set via `relayhelm model` (picker) or `relayhelm setup`. 35+ provider profiles ship as
...
| nous | OAuth device code | `relayhelm auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `relayhelm auth add openai-codex` |
| qwen-oauth | OAuth | `relayhelm auth add qwen-oauth` |
| minimax-oauth | OAuth | `relayhelm auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`relayhelm auth`).
...
#   relayhelm config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
