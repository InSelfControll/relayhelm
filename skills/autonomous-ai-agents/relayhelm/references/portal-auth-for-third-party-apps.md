...
## Layer 1 — Is this thing a Relayhelm plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Relayhelm process**. Its LLM calls go through Relayhelm's already-configured provider. | Already uses Portal if user's Relayhelm is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Relayhelm. That LLM call is already authenticated
via Relayhelm's provider config. The plugin's own server auth (e.g.
...
What Relayhelm has that *feels* like OAuth — `hermes login --provider nous`
opening a browser, user signs in, token lands in `~/.relayhelm/auth.json` — is a
**Hermes-specific browser flow**. Under the hood it produces a credential
Relayhelm uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Relayhelm's existing Portal credential out of `~/.relayhelm/auth.json`.
...
rides on the credential Relayhelm already holds.

Where this could live in Relayhelm:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Relayhelm itself.
...
1. First decide: Relayhelm plugin (runs inside Relayhelm) or separate app? If plugin,
   it already uses Portal via Relayhelm's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Relayhelm agent itself") is fully supported. Relayhelm treats agent
...
pitch in the Tool Gateway announcement is specifically about Relayhelm's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Relayhelm is configured to use Portal as its provider. It is
