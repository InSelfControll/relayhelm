...
Author widget apps for the Relayhelm TUI (`relayhelm --tui`): glanceable ambient
...
- The TUI must be in use (`relayhelm --tui`). Widgets do not render in the
...
1. Use `write_file` to create `~/.relayhelm/tui-widgets/<name>.mjs` (see
...
- A thrown `register()` is logged and skipped; check
  `~/.relayhelm/logs/tui_gateway_crash.log` if a widget never appears.
