...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Relayhelm agent runs by POSTing events to a URL.
...
Add to `${HERMES_HOME:-~/.relayhelm}/.env`:
```bash
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.relayhelm/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.relayhelm/scripts/` (path traversal is blocked).
...
- Subscriptions persist to `~/.relayhelm/webhook_subscriptions.json`

## How It Works

1. `hermes webhook subscribe` writes to `~/.relayhelm/webhook_subscriptions.json`
...
1. **Is the gateway running?** Check with `systemctl --user status relayhelm-gateway` or `ps aux | grep gateway`
2. **Is the webhook server listening?** `curl http://localhost:8644/health` should return `{"status": "ok"}`
3. **Check gateway logs:** `grep webhook ~/.relayhelm/logs/gateway.log | tail -20`
