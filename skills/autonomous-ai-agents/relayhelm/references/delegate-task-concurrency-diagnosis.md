...
**three** code paths in Relayhelm that cap a batch. If none of them fired, the
cap came from the **model itself** — not from Relayhelm — and the user's
narration of "the runtime caps at N" is the model rationalising its own
choice.

## The three real caps in Relayhelm
...
3. **Cost-warning** — same `_get_max_concurrent_children()`. When the
   ...
   log line** — it does not cap anything. Easy to mis-read as "Relayhelm is
   refusing my value."
...
relayhelm config get delegation.max_concurrent_children

# 2. Did Relayhelm' truncator or rejector actually fire?
grep -E "Truncated.*delegate_task|Too many tasks" ~/.relayhelm/logs/agent.log | tail
...
appears, **the cap is the model**, not Relayhelm.
