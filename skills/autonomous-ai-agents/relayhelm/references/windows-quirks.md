...
Relayhelm runs natively on Windows (PowerShell, cmd, Windows Terminal, git-bash
...
`relayhelm config edit` writes correctly.

### `execute_code` / Sandbox

**WinError 10106** from the sandbox child process — it can't create an
`AF_INET` socket. Root cause is usually Relayhelm's env scrubber dropping
...
every Relayhelm tool and most Windows APIs. Prefer forward slashes in code
