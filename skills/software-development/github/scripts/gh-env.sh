#!/usr/bin/env bash
# GitHub environment detection helper for Relayhelm skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _hermes_env="${HERMES_HOME:-$HOME/.relayhelm}/.env"; [ -f "$_hermes_env" ] && grep -q "^GITHUB_TOKEN=" "$_hermes_env" 2>/dev/null; then
