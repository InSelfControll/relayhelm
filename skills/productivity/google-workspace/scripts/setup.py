#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Relayhelm.
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Relayhelm profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Relayhelm Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Relayhelm Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  relayhelm setup")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Relayhelm home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Relayhelm")
