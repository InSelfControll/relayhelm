...
This tutorial walks you through setting up a Telegram bot powered by Relayhelm that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Relayhelm installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.relayhelm/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Relayhelm itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Relayhelm Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Relayhelm. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.relayhelm/.env`:
...
```
[Gateway] Starting Relayhelm Gateway...
...
# View live logs
journalctl --user -u relayhelm-gateway -f
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `relayhelm gateway install` to update the plist.
:::
...
```bash
relayhelm gateway stop && relayhelm gateway start
```
...
**Option 2:** Set it manually in `~/.relayhelm/.env`:
...
Control how much detail the bot shows when using tools. In `~/.relayhelm/config.yaml`:
...
Customize how the bot communicates by editing `~/.relayhelm/SOUL.md`:

For a full guide, see [Use SOUL.md with Relayhelm](/guides/use-soul-with-hermes).
...
```bash
# From the CLI
relayhelm cron list          # View all scheduled jobs
relayhelm cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u relayhelm-gateway` (Linux) or `~/.relayhelm/logs/gateway.log` (macOS) |
| Cron job output | `~/.relayhelm/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.relayhelm/cron/jobs.json` |
| Pairing data | `~/.relayhelm/pairing/` |
| Session history | `~/.relayhelm/sessions/` |
