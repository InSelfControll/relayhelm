...
[Download the Relayhelm Desktop installer](https://hermes-agent.nousresearch.com/) for macOS or Windows, then open it. On first launch it finishes setting itself up (usually under a minute).

### 2. Connect Nous Portal

When the app opens, you'll see a "Let's get you set up" screen. Click **Nous Portal** (marked **Recommended**). Your browser opens — create a [Nous Portal](https://portal.nousresearch.com) account (or sign in), choose the **Free** plan, and authorize Relayhelm. The app connects automatically.
...
Prefer to review first? Download [`install.sh`](https://raw.githubusercontent.com/InSelfControll/relayhelm/main/scripts/install.sh), inspect it, then run it.
...
Select **Quick Setup**. Relayhelm opens a browser tab and waits for you to finish the next steps.
...
When prompted to connect your account to Relayhelm, click **Connect**. You'll see a confirmation once it's linked.
...
- **[Run Relayhelm with Nous Portal](/guides/run-hermes-with-nous-portal)** — Full Portal walkthrough: models, Tool Gateway, and verification
