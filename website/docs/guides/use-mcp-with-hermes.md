---
sidebar_position: 6
title: "Use MCP with Relayhelm"
description: "A practical guide to connecting MCP servers to Relayhelm, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Relayhelm

This guide shows how to actually use MCP with Relayhelm in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Relayhelm tool
- you want Relayhelm to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Relayhelm to internal APIs, databases, or company systems without modifying Relayhelm core

Do not use MCP when:
- a built-in Relayhelm tool already solves the job well
...
- Relayhelm remains the agent
- MCP servers contribute tools
- Relayhelm discovers those tools at startup or reload time
...
If you installed Relayhelm with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Relayhelm banner/status should show MCP integration when configured
- ask Relayhelm what tools it has available
...
## WSL2: bridge Relayhelm in WSL to Windows Chrome

This is the practical setup when:

- Relayhelm runs inside WSL2
...
In this setup, Relayhelm does **not** connect to Chrome directly. Instead:

- Relayhelm runs in WSL
- Relayhelm starts a local stdio MCP server
...
```text
Relayhelm (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Relayhelm stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Relayhelm core browser transport
...
Then start a fresh Relayhelm session or run:
...
Once loaded, Relayhelm can use the MCP-prefixed browser tools directly. For example:
...
If Relayhelm runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Relayhelm from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Relayhelm from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Relayhelm:
...
So Relayhelm will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Relayhelm to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Relayhelm to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Relayhelm remains the agent; Open Scaffold remains the repo-local record.
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Relayhelm to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Relayhelm to merge, publish, deploy, or spawn runtimes.
...
Now Relayhelm can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Relayhelm core.
...
Because Relayhelm now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
