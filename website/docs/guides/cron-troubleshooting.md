...
description: "Diagnose and fix common Relayhelm cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`relayhelm gateway` for foreground, or `relayhelm gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `relayhelm cron tick`.
...
```bash
date
relayhelm cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.relayhelm/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.relayhelm/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.relayhelm/.env` |
...
| `local` | Write access to `~/.relayhelm/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `relayhelm cron list` for updated `last_error` field (if available).
...
### Check 5: Relay-fronted platforms (Relayhelm Cloud / Team Gateway)
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `relayhelm skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `relayhelm skills list`.
...
2. `~/.relayhelm/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `relayhelm cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Relayhelm config directory). Verify:
```bash
ls ~/.relayhelm/scripts/your-script.py   # Must exist
relayhelm cron edit <job_id> --script ~/.relayhelm/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `relayhelm skills install <skill-name>`.
...
Jobs are stored in `~/.relayhelm/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.relayhelm/cron/jobs.json
chmod 600 ~/.relayhelm/cron/jobs.json   # Your user should own it
```
...
```bash
relayhelm cron list                    # Show all jobs, states, next_run times
relayhelm cron run <job_id>            # Schedule for next tick (for testing)
relayhelm cron edit <job_id>           # Fix configuration issues
hermes logs                         # View recent Relayhelm logs
relayhelm skills list                  # Verify installed skills
```
...
1. Run the job with `relayhelm cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.relayhelm/logs/agent.log` for scheduler messages and `~/.relayhelm/logs/errors.log` for warnings
3. Open an issue at [github.com/InSelfControll/relayhelm](https://github.com/InSelfControll/relayhelm) with:
