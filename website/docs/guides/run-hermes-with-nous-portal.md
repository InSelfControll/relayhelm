...
3. Sets `model.provider: nous` in `~/.relayhelm/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Relayhelm is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
relayhelm setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
relayhelm auth add nous --type oauth
# Then re-run `relayhelm setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Relayhelm release notes" and summarize the top 3 hits.
```

You should see Relayhelm call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`relayhelm setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
These rows appear in `relayhelm tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Relayhelm runs the Portal login inline (without changing your inference provider or your other tools).
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Relayhelm will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
If you use [Relayhelm profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.relayhelm/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `hermes portal info` shows "not logged in" after `relayhelm setup --portal`
...
```bash
relayhelm config set model.provider nous
```
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Relayhelm doesn't replay it endlessly. Just log in again:

```bash
relayhelm auth add nous
```
...
If a model is genuinely unavailable, [open an issue](https://github.com/InSelfControll/relayhelm/issues) — most gaps are routing config we can update.
...
- Multiple Relayhelm profiles where you're using the wrong one (check `relayhelm profile list`)

### Want to revoke and start clean

```bash
relayhelm auth logout nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Relayhelm tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Relayhelm configurations
