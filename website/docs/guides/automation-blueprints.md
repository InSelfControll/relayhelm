...
Copy-paste blueprints for common automation patterns. Each blueprint uses Relayhelm's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
```bash
relayhelm cron create "0 2 * * *" \
  "You are a project manager triaging the InSelfControll/relayhelm GitHub repo.

1. Run: gh issue list --repo InSelfControll/relayhelm --state open --json number,title,labels,author,createdAt --limit 30
...
relayhelm cron create "0 9 * * 1" \
  "Scan the InSelfControll/relayhelm repo for documentation drift.

1. Run: gh pr list --repo InSelfControll/relayhelm --state merged --json number,title,files,mergedAt --limit 30
...
```bash
relayhelm cron create "0 6 * * *" \
  "Run a dependency security audit on the relayhelm project.

1. cd ~/.relayhelm/relayhelm && source .venv/bin/activate
...
```python title="~/.relayhelm/scripts/check-uptime.py"
...
```bash
relayhelm cron create "every 30m" \
  ...
  --script ~/.relayhelm/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
relayhelm cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Relayhelm development." \
...
```bash
relayhelm cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the relayhelm codebase.
