...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Relayhelm  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Relayhelm instantly when PRs are opened or updated.
:::
...
- **Relayhelm installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  relayhelm gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.relayhelm/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Relayhelm can access GitHub. Start a chat:
...
```
Run: gh pr list --repo InSelfControll/relayhelm --state open --limit 3
```
...
Still in the chat, ask Relayhelm to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo InSelfControll/relayhelm
```
...
A skill gives Relayhelm consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.relayhelm/skills/code-review
```

Create `~/.relayhelm/skills/code-review/SKILL.md`:
...
This is what makes the reviewer actually useful. Start a session and teach Relayhelm your team's standards:
...
2. Teach Relayhelm your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
relayhelm gateway status    # Is the gateway running?
relayhelm cron list         # Is the job enabled?
```
