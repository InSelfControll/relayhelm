...
description: "Reference for Relayhelm core, composite, platform, and dynamic toolsets"
---
...
```bash
relayhelm chat --toolsets web,file,terminal
relayhelm chat --toolsets debugging        # composite — expands to file + terminal + web
relayhelm chat --toolsets all              # everything
```
...
```bash
relayhelm tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Relayhelm tools programmatically. |
...
| `desktop_ui` | `annotate_preview`, `close_preview`, `close_terminal`, `drive_preview`, `focus_pane`, `open_preview`, `react_to_message`, `read_preview`, `read_terminal`, `read_window_below`, `tour` | Affordances that act on the Relayhelm desktop app itself — read/close the embedded terminal pane, open, read, close, interact with, and annotate the in-app browser, identify the OS window behind the app, reveal a pane, react to a message, run a guided tour (highlight + narrate UI elements in the app or the preview pane). Enabled for sessions whose source is the desktop app, whichever backend it's connected to (local, SSH, URL, or Relayhelm Cloud). Never present on CLI, TUI, messaging, or cron sessions. |
...
| `x_search` | `x_search` | Read-only public X discovery via xAI's built-in `x_search` Responses tool. Use the `xurl` skill for authenticated X API reads and account actions. Off by default; opt in via `relayhelm tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
...
| `relayhelm-gateway` | Internal gateway orchestrator toolset — union of every `hermes-<platform>` toolset; used when the gateway needs to accept any message source. |
...
## Relationship to `relayhelm tools`

The `relayhelm tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
