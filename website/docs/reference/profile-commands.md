...
This page covers all commands related to [Relayhelm profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `relayhelm profile` without a subcommand shows help.
...
## `relayhelm profile use`

```bash
relayhelm profile use <name>
```
...
relayhelm profile use default
```

## `relayhelm profile create`

```bash
relayhelm profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `relayhelm profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `relayhelm update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.relayhelm`), use `relayhelm skills opt-out` / `relayhelm skills opt-in`. |
...
relayhelm profile create work2 --clone-from work

# Clone everything from a specific profile
relayhelm profile create work2-backup --clone-from work --clone-all
```

## `relayhelm profile describe`

```bash
relayhelm profile describe [<name>] [options]
```
...
relayhelm profile describe researcher

# Set it explicitly
relayhelm profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
relayhelm profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
relayhelm profile describe --all --auto
```

## `relayhelm profile delete`

```bash
relayhelm profile delete <name> [options]
```
...
```bash
relayhelm profile delete mybot
relayhelm profile delete mybot --yes
```

:::warning
This permanently deletes the profile's entire directory including all config, memories, sessions, and skills. The `default` profile (`~/.relayhelm`) cannot be deleted — use `hermes uninstall` to remove everything.
:::

## `relayhelm profile show`

```bash
relayhelm profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Relayhelm home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ relayhelm profile show work
Profile: work
Path:    ~/.relayhelm/profiles/work
...
## `relayhelm profile alias`

```bash
relayhelm profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Relayhelm installation.
...
```bash
relayhelm profile alias work
# Creates/updates ~/.local/bin/work

relayhelm profile alias work --name mywork
# Creates ~/.local/bin/mywork

relayhelm profile alias work --remove
# Removes the wrapper script
```

## `relayhelm profile rename`

```bash
relayhelm profile rename <old-name> <new-name>
```
...
```bash
relayhelm profile rename mybot assistant
# ~/.relayhelm/profiles/mybot → ~/.relayhelm/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `relayhelm profile export`

```bash
relayhelm profile export <name> [options]
```
...
```bash
relayhelm profile export work
# Creates work.tar.gz in the current directory

relayhelm profile export work -o ./work-2026-03-29.tar.gz
```
...
## `relayhelm profile import`

```bash
relayhelm profile import <archive> [options]
```
...
```bash
relayhelm profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

relayhelm profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `relayhelm -p telemetry`). |
...
relayhelm profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
relayhelm profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
relayhelm profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
relayhelm profile install ./telemetry/
```

### `relayhelm profile update`

```bash
relayhelm profile update <name> [--force-config] [--yes]
```
...
### `relayhelm profile info`

```bash
relayhelm profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Relayhelm version, author, env var requirements, the source URL/path, and
...
`relayhelm profile list` also shows the distribution name and version in a
`Distribution` column, and `relayhelm profile show <name>` / `delete <name>`
...
relayhelm profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
relayhelm profile install https://github.com/your-org/internal-assistant.git
```
...
Relayhelm version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Relayhelm can clone from.
3. Tell recipients to run `relayhelm profile install <your-repo-url>`.
...
## `relayhelm -p` / `relayhelm --profile`

```bash
relayhelm -p <name> <command> [options]
relayhelm --profile <name> <command> [options]
```

Global flag to run any Relayhelm command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
relayhelm -p work chat -q "Check the server status"
relayhelm --profile dev gateway start
relayhelm -p personal skills list
relayhelm -p work config edit
```
...
- `relayhelm profile <TAB>` — subcommands (list, use, create, etc.)
- `relayhelm profile use <TAB>` — profile names
- `relayhelm -p <TAB>` — profile names
