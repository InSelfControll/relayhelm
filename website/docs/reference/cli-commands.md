...
description: "Authoritative reference for Relayhelm terminal commands and command families"
---
...
| `--profile <name>`, `-p <name>` | Select which Relayhelm profile to use for this invocation. Overrides the sticky default set by `relayhelm profile use`. |
...
| `--ignore-user-config` | Ignore `~/.relayhelm/config.yaml` and fall back to built-in defaults. Credentials in `.env` are still loaded. |
...
| Command | Purpose |
|---------|---------|
| `relayhelm chat` | Interactive or one-shot chat with the agent. |
| `relayhelm model` | Interactively choose the default provider and model. |
| `hermes moa` | Configure named Mixture of Agents presets selectable from the model picker. |
| `hermes fallback` | Manage fallback providers tried when the primary model errors. |
| `relayhelm gateway` | Run or manage the messaging gateway service. |
...
| `relayhelm setup` | Interactive setup wizard for all or part of the configuration. |
...
| `relayhelm auth` | Manage credentials — add, list, remove, reset, status, logout. Handles OAuth flows for Codex/Nous/Anthropic. |
| `hermes login` / `logout` | **Deprecated** — use `relayhelm auth` instead. |
...
| `hermes peer` | Register peer Relayhelm gateways on other machines and DM their agents' canonical Bot Chats (`hermes peer dm <peer>[/<agent>] "…"`). The transport behind cross-machine bot-to-bot messaging. |
| `hermes secrets` | Manage external secret sources (currently Bitwarden Secrets Manager) for pulling API keys at process startup instead of from `~/.relayhelm/.env`. |
...
| `relayhelm status` | Show agent, auth, and platform status. |
| `relayhelm cron` | Inspect and tick the cron scheduler. |
| `relayhelm kanban` | Multi-profile collaboration board (tasks, links, dispatcher). |
...
| `relayhelm doctor` | Diagnose config and dependency issues. |
...
| `relayhelm backup` | Back up Relayhelm home directory to a zip file. |
| `hermes checkpoints` | Inspect / prune / clear `~/.relayhelm/checkpoints/` (the shadow store used by `/rollback`). Run with no args for a status overview. |
| `relayhelm import` | Restore a Relayhelm backup from a zip file. |
| `hermes logs` | View, tail, and filter agent/gateway/error log files. |
| `relayhelm config` | Show, edit, migrate, and query configuration files. |
| `hermes skin` | List, switch, and tweak display skins. |
| `hermes console` | Open the safe Relayhelm command console. |
| `hermes pairing` | Approve or revoke messaging pairing codes. |
| `relayhelm skills` | Browse, install, publish, audit, and configure skills. |
...
| `relayhelm memory` | Configure external memory provider. Plugin-specific subcommands (e.g. `hermes honcho`) register automatically when their provider is active. |
| `relayhelm acp` | Run Relayhelm as an ACP server for editor integration. |
| `hermes mcp` | Manage MCP server configurations and run Relayhelm as an MCP server. |
| `relayhelm plugins` | Manage Relayhelm plugins (install, enable, disable, remove). |
...
| `relayhelm tools` | Configure enabled tools per platform. |
...
| `relayhelm import-agent` | Import a Claude Code (`~/.claude`) or Codex CLI (`~/.codex`) setup. |
| `relayhelm dashboard` | Launch the web dashboard for managing config, API keys, and sessions. |
| `relayhelm serve` | Start the Relayhelm backend server (headless; powers the desktop app and remote backends). |
| `hermes desktop` (alias `gui`) | Build and launch the native Electron desktop app. |
| `relayhelm profile` | Manage profiles — multiple isolated Relayhelm instances. |
| `hermes completion` | Print shell completion scripts (bash/zsh/fish). |
| `relayhelm --version` | Show version information. |
| `relayhelm update` | Pull latest code and reinstall dependencies. `--check` previews without installing; `--backup` takes a pre-pull `HERMES_HOME` snapshot. |
| `hermes uninstall` | Remove Relayhelm from the system. |

## `relayhelm chat`

```bash
relayhelm chat [options]
```
...
| `--ignore-user-config` | Ignore `~/.relayhelm/config.yaml` and use built-in defaults. Credentials in `.env` are still loaded. Useful for isolated CI runs, reproducible bug reports, and third-party integrations. |
| `--ignore-rules` | Skip auto-injection of `AGENTS.md`, `SOUL.md`, `.cursorrules`, persistent memory, and preloaded skills. Combine with `--ignore-user-config` for a fully isolated run. |
| `--safe-mode` | Troubleshooting mode: disable ALL customizations — user config, rules/memory injection, plugins, shell hooks, and MCP servers (implies `--ignore-user-config` and `--ignore-rules`). Use to isolate whether a problem comes from your setup or from Relayhelm itself. |
...
```bash
hermes
relayhelm chat -q "Summarize the latest PRs"          # seeds an interactive session
relayhelm chat --oneshot -q "Summarize the latest PRs"  # answer and exit
relayhelm chat --provider openrouter --model anthropic/claude-sonnet-4.6
relayhelm chat --toolsets web,terminal,skills
relayhelm chat --quiet -q "Return only JSON"
relayhelm chat --worktree -q "Review this repo and open a PR"
relayhelm chat --ignore-user-config --ignore-rules -q "Repro without my personal setup"
relayhelm chat --safe-mode -q "Is this bug mine or Relayhelm'?"
```

### `relayhelm -z <prompt>` — scripted one-shot

For programmatic callers (shell scripts, CI, cron, parent processes piping in a prompt), `relayhelm -z` is the purest one-shot entry point: **single prompt in, final response text out, nothing else on stdout or stderr.** No banner, no spinner, no tool previews, no `Session:` line — just the agent's final reply as plain text.

```bash
relayhelm -z "What's the capital of France?"
# → Paris.

# Parent scripts can cleanly capture the response:
answer=$(relayhelm -z "summarize this" < /path/to/file.txt)
```

Per-run overrides (no mutation to `~/.relayhelm/config.yaml`):
...
```bash
relayhelm -z "…" --provider openrouter --model openai/gpt-5.5
# or:
HERMES_INFERENCE_MODEL=anthropic/claude-sonnet-4.6 relayhelm -z "…"
```

Same agent, same tools, same skills — just strips every interactive / cosmetic layer. If you need tool output in the transcript too, use `relayhelm chat --oneshot -q` instead; `-z` is explicitly for "I only want the final answer".

#### `--usage-file` — JSON usage report for pipelines

`relayhelm -z "…" --usage-file /path/report.json` writes a machine-readable usage report after the run: `estimated_cost_usd`, `input_tokens` / `output_tokens` / `cache_read_tokens` / `cache_write_tokens` / `reasoning_tokens` / `total_tokens`, `api_calls`, `model`, `provider`, `session_id`, `service_tier`, and `completed` / `failed` flags. The report is written **even when the run fails**, so batch pipelines can always account for spend. It has no effect outside `-z`/`--oneshot`, and a broken usage write never masks the run's own outcome.

```bash
relayhelm -z "summarize this repo" --usage-file /tmp/usage.json
jq .estimated_cost_usd /tmp/usage.json
```

## `relayhelm model`

Interactive provider + model selector. **This is the command for adding new providers, setting up API keys, and running OAuth flows.** Run it from your terminal — not from inside an active Relayhelm chat session.
...
:::warning relayhelm model vs /model — know the difference
**`relayhelm model`** (run from your terminal, outside any Relayhelm session) is the **full provider setup wizard**. It can add new providers, run OAuth flows, prompt for API keys, and configure endpoints.

**`/model`** (typed inside an active Relayhelm chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys.

**If you need to add a new provider:** Exit your Relayhelm session first (`Ctrl+C` or `/quit`), then run `relayhelm model` from your terminal prompt.
:::
...
If you've only configured OpenRouter, `/model` will only show OpenRouter models. To add another provider (Anthropic, DeepSeek, Copilot, etc.), exit your session and run `relayhelm model` from the terminal.
:::
...
| `migrate-legacy` | Remove legacy `hermes.service` units left over from pre-rename installs. Profile units (`relayhelm-gateway-<profile>.service`) and unrelated services are never touched. Flags: `--dry-run`, `-y`/`--yes`. |
| `enroll` | Experimental: enroll this gateway with a relay connector and save relay credentials for connector-backed platforms. See [Relayhelm Relay](/user-guide/messaging/relay). |

Options:

| Option | Description |
|--------|-------------|
| `--all` | On `start` / `restart` / `stop`: act on **every profile's** gateway, not just the active `HERMES_HOME`. Useful if you run multiple profiles side-by-side and want to restart them all after `relayhelm update`. |
...
`relayhelm gateway enroll` accepts `--token`, `--connector-url`, `--gateway-id`, and `--wake-url`. It exchanges the enrollment token with the connector and writes the resulting `GATEWAY_RELAY_ID`, `GATEWAY_RELAY_SECRET`, `GATEWAY_RELAY_DELIVERY_KEY`, optional `GATEWAY_RELAY_URL`, and (when `--wake-url` is given) `GATEWAY_RELAY_WAKE_URL` values to the active profile's `.env`.

:::tip WSL users
Use `relayhelm gateway run` instead of `relayhelm gateway start` — WSL's systemd support is unreliable. Wrap it in tmux for persistence: `tmux new -s hermes 'relayhelm gateway run'`. See [WSL FAQ](/reference/faq#wsl-gateway-keeps-disconnecting-or-relayhelm-gateway-start-fails) for details.
:::
...
```bash
relayhelm setup [model|tts|terminal|gateway|tools|agent] [--non-interactive] [--reset] [--quick] [--reconfigure] [--portal]
```

**Easiest path:** `relayhelm setup --portal` — OAuth into Nous Portal and opt into the [Tool Gateway](../user-guide/features/tool-gateway.md) in one shot.
...
| `--reconfigure` | Backwards-compat alias — bare `relayhelm setup` on an existing install now does this by default. |
...
For configuration of the gateway itself, see [Tool Gateway](../user-guide/features/tool-gateway.md). For the one-shot setup path, see `relayhelm setup --portal` above.
...
hermes slack manifest --write      # write to ~/.relayhelm/slack-manifest.json
...
| `--name NAME` | `Relayhelm` | Bot display name in Slack. |
...
Run `hermes slack manifest --write` again after `relayhelm update` to pick
up any new commands.
...
Send a one-shot message to a configured messaging platform without spinning up an agent or gateway loop. Reuses the gateway's already-configured credentials (`~/.relayhelm/.env` + `~/.relayhelm/config.yaml`) so ops scripts, cron jobs, CI hooks, and monitoring daemons can post status updates without reimplementing each platform's REST client.
...
Bot-to-bot DMs across machines. Register another Relayhelm gateway (any machine
...
`relayhelm -p <bot> chat --in ~ -c "Bot Chat" …` bot-messaging command.
...
| Subcommand | Description |
|--------|-------------|
| `add <name> --url <URL> [--key <KEY>] [--note TEXT]` | Register or update a peer. The URL goes to `config.yaml` (`bot_peers`); the key is stored as `HERMES_PEER_<NAME>_KEY` in `~/.relayhelm/.env`. |
...
Pull API keys from an external secret manager at process startup instead of storing them in `~/.relayhelm/.env`. Currently supports **Bitwarden Secrets Manager**. See the full guide: [Bitwarden integration](../user-guide/secrets/bitwarden.md).
...
> Not to be confused with `hermes claw migrate` (one-shot import of OpenClaw configuration into Relayhelm) — `hermes migrate` is the top-level config-rewrite command.
...
On-demand vulnerability scan against [OSV.dev](https://osv.dev). Covers the Relayhelm venv (installed PyPI distributions), Python dependencies declared by plugins under `~/.relayhelm/plugins/`, and pinned `npx`/`uvx` MCP servers in `config.yaml`. Does NOT scan globally-installed packages or editor/browser extensions.
...
| `--skip-venv` | off | Skip scanning the Relayhelm Python venv. |
...
:::caution
`hermes login` has been removed. Use `relayhelm auth` to manage OAuth credentials, `relayhelm model` to select a provider, or `relayhelm setup` for full interactive setup.
:::
...
```bash
relayhelm auth                                              # Interactive wizard
relayhelm auth list                                         # Show all pools
relayhelm auth list openrouter                              # Show specific provider
relayhelm auth add openrouter --api-key sk-or-v1-xxx        # Add API key
relayhelm auth add anthropic --type oauth                   # Add OAuth credential
relayhelm auth remove openrouter 2                          # Remove by index
relayhelm auth reset openrouter                             # Clear cooldowns
relayhelm auth status anthropic                             # Show auth status for a provider
relayhelm auth logout anthropic                             # Log out and clear stored auth state
relayhelm auth spotify                                      # Authenticate Relayhelm with Spotify via PKCE
```
...
```bash
relayhelm cron <list|create|edit|pause|resume|run|remove|status|runs|incidents|doctor|tick>
```
...
| `boards switch <slug>` / `boards use` | Persist `<slug>` as the active board (writes `~/.relayhelm/kanban/current`). |
...
relayhelm kanban boards create atm10-server --name "ATM10 Server" --icon 🎮
relayhelm kanban --board atm10-server create "Restart server" --assignee ops

# Switch the active board for subsequent calls.
relayhelm kanban boards switch atm10-server
relayhelm kanban list                  # shows atm10-server tasks

# Archive a board (recoverable) or hard-delete it.
relayhelm kanban boards rm atm10-server
relayhelm kanban boards rm atm10-server --delete
```

Board resolution order (highest precedence first): `--board <slug>` flag → `HERMES_KANBAN_BOARD` env var → `~/.relayhelm/kanban/current` file → `default`.
...
relayhelm config set terminal.backend docker   # if not already
...
# Edit ~/.relayhelm/config.yaml proxy.extra_allowed_hosts: [api.example.com]
hermes egress setup
...
cat ~/.relayhelm/proxy/proxy.yaml           # the rendered iron-proxy config
tail -20 ~/.relayhelm/proxy/iron-proxy.log  # daemon-level diagnostics
tail -f ~/.relayhelm/proxy/iron-proxy.log | jq  # daemon + per-request log (line-delimited JSON; v0.39 combines both streams)
```
...
| `--script` | Filter/transform script under `~/.relayhelm/scripts/`. The webhook payload is passed as JSON on stdin; JSON stdout replaces the payload, and empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. See [Script Filters and Transforms](../user-guide/messaging/webhooks.md#script-filters-and-transforms). |

Subscriptions persist to `~/.relayhelm/webhook_subscriptions.json` and are hot-reloaded by the webhook adapter without a gateway restart.

## `relayhelm doctor`

```bash
relayhelm doctor [--fix]
```
...
Outputs a compact, plain-text summary of your entire Relayhelm setup. Designed to be copy-pasted into Discord, GitHub issues, or Telegram when asking for support — no ANSI colors, no special formatting, just data.
...
| Section | Details |
|---------|---------|
| **Header** | Relayhelm version, release date, git commit hash |
...
:::tip
`hermes dump` is specifically designed for sharing. For interactive diagnostics, use `relayhelm doctor`. For a visual overview, use `relayhelm status`.
:::
...
The report includes system info (OS, Python version, Relayhelm version), recent agent, gateway, GUI/dashboard, and desktop logs (512 KB limit per file), and redacted API key status. By default, uploads are redacted so secrets are not included.
...
```bash
relayhelm backup [options]
```

Create a zip archive of your Relayhelm configuration, skills, sessions, and data. The backup excludes the relayhelm codebase itself, and it does not nest earlier backup artifacts (`backups/`, `state-snapshots/`) — each of those already contains its own copy of `state.db`.
...
The backup uses SQLite's `backup()` API for safe copying, so it works correctly even when Relayhelm is running (WAL-mode safe).

**What's excluded from the zip:**

- `*.db-wal`, `*.db-shm`, `*.db-journal` — SQLite's WAL / shared-memory / journal sidecars. The `*.db` file already got a consistent snapshot via `sqlite3.backup()`; shipping the live sidecars alongside it would let a restore see a half-committed state.
- `checkpoints/` — per-session trajectory caches. Hash-keyed and regenerated per session; wouldn't port cleanly to another install anyway.
- The `relayhelm` code itself (this is a user-data backup, not a repo snapshot).

### Examples

```bash
relayhelm backup                           # Full backup to ~/hermes-backup-*.zip
relayhelm backup -o /tmp/hermes.zip        # Full backup to specific path
relayhelm backup --quick                   # Quick state-only snapshot
relayhelm backup --quick --label "pre-upgrade"  # Quick snapshot with label
```
...
Inspect and manage the shadow git store at `~/.relayhelm/checkpoints/` — the storage layer behind the in-session `/rollback` command. Safe to run any time; does not require the agent to be running.
...
```bash
relayhelm import <zipfile> [options]
```

Restore a previously created Relayhelm backup into your Relayhelm home directory. All files in the archive overwrite existing files in your Relayhelm home; `--force` only skips the confirmation prompt that fires when the target already has a Relayhelm installation.
...
### Examples
```bash
relayhelm import ~/hermes-backup-20260423.zip           # Prompts before overwriting existing config
relayhelm import ~/hermes-backup-20260423.zip --force   # Overwrite without prompting
```
...
View, tail, and filter Relayhelm log files. All logs are stored in `~/.relayhelm/logs/` (or `<profile>/logs/` for non-default profiles).
...
Relayhelm uses Python's `RotatingFileHandler`. Old logs are rotated automatically — look for `agent.log.1`, `agent.log.2`, etc. The `hermes logs list` subcommand shows all log files including rotated ones.
...
- **Prompt tiers** — stable / context / volatile, matching how Relayhelm layers
...
enabled. To shrink the prompt, disable unused toolsets (`relayhelm tools`) or
uninstall skills you don't need (`relayhelm skills`). Context files (AGENTS.md,
...
| `get <key> [--json]` | Print a single config value by dotted key (e.g. `relayhelm config get model.default`). `--json` emits machine-readable output. |
...
`relayhelm config set/get/unset` use `.` as the nesting separator, but many real
...
- **Existing keys just work.** When navigating an existing mapping, an
  ...
  splitting. `relayhelm config set providers.p.models.grok-4.6.supports_vision true`
...
- **Creating a new dotted key requires escaping.** Escape literal dots with a
  backslash: `relayhelm config set 'providers.p.models.grok-4\.7.context_length' 128000`
...
```bash
relayhelm skills browse
relayhelm skills browse --source official
relayhelm skills search react --source skills-sh
relayhelm skills search https://mintlify.com/docs --source well-known
relayhelm skills inspect official/security/1password
relayhelm skills inspect skills-sh/vercel-labs/json-render/json-render-react
relayhelm skills install official/migration/openclaw-migration
relayhelm skills install skills-sh/anthropics/skills/pdf --force
relayhelm skills install https://sharethis.chat/SKILL.md                     # Direct URL (+ referenced support files)
relayhelm skills install https://example.com/SKILL.md --name my-skill        # Override name when frontmatter has none
relayhelm skills check
relayhelm skills update
relayhelm skills config
relayhelm skills reset google-workspace
relayhelm skills reset google-workspace --restore --yes
relayhelm skills opt-out                  # stop future bundled-skill seeding (nothing deleted)
relayhelm skills opt-out --remove --yes   # also delete UNMODIFIED bundled skills
relayhelm skills opt-in --sync            # undo: remove marker and re-seed now
```
...
- `--source well-known` lets you point Relayhelm at a site exposing `/.well-known/skills/index.json`.
...
Skill bundles group several skills under one `/<bundle-name>` slash command. Invoking the bundle loads every referenced skill into a single combined user message. Storage: `~/.relayhelm/skill-bundles/<slug>.yaml`. See [Skill Bundles](../user-guide/features/skills.md#skill-bundles) for the YAML schema and behavior.
...
| `reload` | Re-scan `~/.relayhelm/skill-bundles/` and report added/removed bundles |
...
| `backup` | Take a manual tar.gz snapshot of `~/.relayhelm/skills/` (curator also snapshots automatically before every real run) |
| `rollback` | Restore `~/.relayhelm/skills/` from a snapshot (defaults to newest) |
...
On a fresh install the first scheduled pass is deferred by one full `interval_hours` (7 days by default) — the gateway will not curate immediately on the first tick after `relayhelm update`. Use `hermes curator run --dry-run` to preview before that happens.
...
`hermes moa configure` reuses Relayhelm' provider → model picker for each reference model and the aggregator. A preset is an execution-mode configuration, not a primary model or provider.
...
| `add` | Pick a provider + model (same picker as `relayhelm model`) and append to the chain |
...
Inspect shell-script hooks declared in `~/.relayhelm/config.yaml`, test them against synthetic payloads, and manage the first-use consent allowlist at `~/.relayhelm/shell-hooks-allowlist.json`.
...
When an external memory provider is active, it may register its own top-level `hermes <provider>` command for provider-specific management (e.g. `hermes honcho` when Honcho is active). Inactive providers do not expose their subcommands. Run `relayhelm --help` to see what's currently wired in.
:::
...
Starts Relayhelm as an ACP (Agent Client Protocol) stdio server for editor integration.
...
Manage MCP (Model Context Protocol) server configurations and run Relayhelm as an MCP server.
...
| `serve [-v\|--verbose]` | Run Relayhelm as an MCP server — expose conversations to other agents. |
...
See [MCP Config Reference](./mcp-config-reference.md), [Use MCP with Relayhelm](../guides/use-mcp-with-hermes.md), and [MCP Server Mode](../user-guide/features/mcp.md#running-hermes-as-an-mcp-server).

## `relayhelm plugins`

```bash
relayhelm plugins [subcommand]
```

Unified plugin management — general plugins, memory providers, and context engines in one place. Running `relayhelm plugins` with no subcommand opens a composite interactive screen with two sections:
...
See [Plugins](../user-guide/features/plugins.md) and [Build a Relayhelm Plugin](../developer-guide/plugins/index.md).

## `relayhelm tools`

```bash
relayhelm tools [--summary]
```
...
`relayhelm tools` invokes when you first enable Computer Use, so it's safe
to use for re-running the install if the toolset toggle didn't trigger
it (for example, on returning-user setups).

If cua-driver is already present, Relayhelm checks its version and runtime
...
installer. Relayhelm never replaces a custom binary selected through
...
The built-in `computer_use` toolset is the recommended Relayhelm integration.
Registering raw Cua MCP tools is an alternative when you need Cua's low-level
tool vocabulary. `cua-driver skills install` detects Relayhelm and links Cua's
skill pack into the Relayhelm skills directory automatically.

Permission mode and capability-manifest approval
belong to runtime launch. In bounded mode Relayhelm passes Cua's canonical
...
`relayhelm update` automatically re-runs the upstream installer at the end
of the update if cua-driver is on PATH, so most users will not need to
call `--upgrade` manually. Use it when upstream ships a fix you want
right now without waiting for the next Relayhelm update.
...
[Petdex](https://github.com/crafter-station/petdex) is a public gallery of animated sprite pets for coding agents. Install one and Relayhelm shows it reacting to agent activity across the CLI, TUI, and desktop app.
...
Migrate your OpenClaw setup to Relayhelm. Reads from `~/.openclaw` (or a custom path) and writes to `~/.relayhelm`. Automatically detects legacy directory names (`~/.clawdbot`, `~/.moltbot`) and config filenames (`clawdbot.json`, `moltbot.json`).

| Option | Description |
|--------|-------------|
| `--dry-run` | Preview what would be migrated without writing anything. |
| `--preset <name>` | Migration preset: `full` (all compatible settings) or `user-data` (excludes infrastructure config). Neither preset imports secrets — pass `--migrate-secrets` explicitly. |
| `--overwrite` | Overwrite existing Relayhelm files on conflicts (default: refuse to apply when the plan has conflicts). |
| `--migrate-secrets` | Include API keys in migration. Required even under `--preset full`. |
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.relayhelm/` (by default a single restore-point archive is written to `~/.relayhelm/backups/pre-migration-*.zip` before apply; restorable with `relayhelm import`). |
...
The migration covers 30+ categories across persona, memory, skills, model providers, messaging platforms, agent behavior, session policies, MCP servers, TTS, and more. Items are either **directly imported** into Relayhelm equivalents or **archived** for manual review.
...
## `relayhelm import-agent`

```bash
relayhelm import-agent [claude-code|codex] [options]
```

Import a **Claude Code** (`~/.claude`) or **OpenAI Codex CLI** (`~/.codex`) setup into Relayhelm. Maps `CLAUDE.md`/`AGENTS.md` instructions to memory entries, `Bash(...)` permission allow/deny rules to `command_allowlist`/`approvals.deny`, MCP servers to `mcp_servers` in `config.yaml`, and skill directories into `~/.relayhelm/skills/`. Always previews before applying; API keys and credentials are never imported.
...
```bash
relayhelm serve [options]
```

Start the Relayhelm **backend server** — the JSON-RPC/WebSocket gateway the [desktop app](/user-guide/desktop) and remote clients connect to. It is the same server `relayhelm dashboard` runs, but **headless**: it never opens a browser UI. The desktop app launches its own `relayhelm serve` backend; use this command directly when you want a headless backend on a remote host. Accepts the same `--host` / `--port` / `--insecure` / `--skip-build` / `--stop` / `--status` options as `relayhelm dashboard` below (a non-loopback bind engages the same auth gate). Requires the `[web]` extra; the embedded Chat socket additionally needs `[pty]` on a POSIX host.

**Port conflicts:** if the requested port (default `9119`) is already held by another process (e.g. a second `relayhelm serve` or the gateway), the command prints a machine-readable sentinel line `BACKEND_PORT_IN_USE port=<port>` to stdout, a human hint naming the likely holder, and exits with code **75** (`EX_TEMPFAIL`) instead of a generic error — so scripts and the desktop app can tell "port occupied" apart from "backend broken". Pass `--port 0` to bind a free ephemeral port (the successful boot announces the chosen port via `HERMES_BACKEND_READY port=<port>`).

## `relayhelm dashboard`

```bash
relayhelm dashboard [options]
```

Launch the web dashboard — a browser-based UI for managing configuration, API keys, and monitoring sessions. (For a headless backend with no browser UI — e.g. what the desktop app spawns — use [`relayhelm serve`](#hermes-serve) above.) Requires `cd ~/.relayhelm/relayhelm && uv pip install -e ".[web]"` (FastAPI + Uvicorn). The embedded browser Chat tab is always available and additionally needs the `pty` extra (`cd ~/.relayhelm/relayhelm && uv pip install -e ".[web,pty]"`) plus a POSIX PTY environment such as Linux, macOS, or WSL2. See [Web Dashboard](/user-guide/features/web-dashboard) for full documentation.
...
| `--stop` | — | Stop running `relayhelm dashboard` processes and exit. |
| `--status` | — | List running `relayhelm dashboard` processes and exit. |

### `relayhelm dashboard register`

Register this install as a self-hosted dashboard with your Nous Portal account. Creates an OAuth client, writes `HERMES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.relayhelm/.env`, and prints how to engage the login gate. Requires being logged in (`relayhelm setup`).
...
relayhelm dashboard --port 8080 --no-open
...
```bash
relayhelm profile <subcommand>
```

Manage profiles — multiple isolated Relayhelm instances, each with its own config, sessions, skills, and home directory.
...
```bash
relayhelm profile list
relayhelm profile create work --clone
relayhelm profile use work
relayhelm profile alias work --name h-work
relayhelm profile export work -o work-backup.tar.gz
relayhelm profile import work-backup.tar.gz --name restored
relayhelm profile install github.com/user/my-distro --alias
relayhelm profile update work
relayhelm -p work chat -q "Hello from work profile"
```
...
Print a shell completion script to stdout. Source the output in your shell profile for tab-completion of Relayhelm commands, subcommands, and profile names.
...
```bash
relayhelm update [--gateway] [--check] [--plan] [--no-backup] [--backup] [--yes]
```

Pulls the latest `relayhelm` code and reinstalls dependencies in the managed venv, then re-runs the post-install hooks (MCP servers, skills sync, completion install). Safe to run on a live install. Use `--check` to see whether your checkout is behind `origin/main` without installing.

`relayhelm update` pulls the configured update branch (default: `main`). If your checkout is on another branch, Relayhelm may check out the update branch before pulling. Commit branch work before updating when you want to keep it outside the update autostash flow.
...
| `--yes`, `-y` | Assume yes for interactive prompts such as config migration and stash restore. API-key entry is skipped; run `relayhelm config migrate` separately for those. |

Additional behavior:

- **Gateway restart.** After a successful update, Relayhelm attempts to restart all running gateway profiles automatically so they pick up the new code. Use `relayhelm gateway restart` when you want to restart a gateway without applying an update.
...
- **Update receipts + fleet version check.** Every run writes a machine-readable receipt to `~/.relayhelm/logs/update_receipts/` (pre-update fleet plan, steps, skips with reasons, restart outcome; `latest.json` points at the newest). After the restart phase the updater verifies each live gateway's running code against the updated checkout and prints a per-profile version matrix; a gateway still on pre-update code fails the update (exit 1) with the exact restart command.
...
- **npm lockfile churn.** Before stashing or switching branches, Relayhelm makes a best-effort cleanup of tracked `package-lock.json` diffs produced by npm install/build steps. Commit or manually stash intentional lockfile edits before running `relayhelm update`.
- **Pairing data snapshot.** Even when `--backup` is off, `relayhelm update` takes a lightweight snapshot of `~/.relayhelm/pairing/` and the Feishu comment rules before `git pull`. You can roll it back with `relayhelm backup restore --state pre-update` if a pull rewrites a file you were editing.
- **Legacy `hermes.service` warning.** If Relayhelm detects a pre-rename `hermes.service` systemd unit (instead of the current `relayhelm-gateway.service`), it prints a one-time migration hint so you can avoid flap-loop issues.
...
| Command | Description |
|---------|-------------|
| `relayhelm --version` | Print version information. |
| `relayhelm update` | Pull latest changes and reinstall dependencies. |

| `hermes uninstall [--full] [--gui] [--dry-run] [--yes]` | Remove Relayhelm, optionally deleting all config/data. `--gui` removes only the desktop Chat GUI, leaving the agent intact; `--full` also deletes config/data; `--dry-run` prints what would be removed without changing anything; `--yes` skips prompts. |
