...
Pull API keys from [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) at process startup instead of storing them in plaintext inside `~/.relayhelm/.env`. One bootstrap secret (a machine-account access token) replaces N per-provider keys, and rotating a credential becomes a single change in the Bitwarden web app.
...
2. Relayhelm stores that single token in `~/.relayhelm/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `hermes` (or the gateway, or a cron job) starts, after `~/.relayhelm/.env` has loaded, Relayhelm calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
4. By default Relayhelm **overrides** values already in your environment, so Bitwarden is the source of truth — rotate a key once in the web app and every Relayhelm process picks it up on next start. Flip `override_existing: false` in config if you want `.env` to win instead.

The `bws` binary is auto-downloaded into `~/.relayhelm/bin/` on first use — no `apt`, no `brew`, no `sudo`.
...
2. Create or pick a **Project** (e.g. "Relayhelm keys").
...
4. **Machine accounts → New machine account → My Relayhelm machine** → **Projects** tab → grant Read access to your project.
...
1. Download and verify `bws v2.0.0` into `~/.relayhelm/bin/bws`.
2. Prompt you for the access token (input is hidden). Stored in `~/.relayhelm/.env` as `BWS_ACCESS_TOKEN`.
...
| `encrypted_cache.enabled` | `false` | Store the last successful fetch in an AES-GCM encrypted cache at `~/.relayhelm/cache/bws_cache.enc.json`. |
...
| `auto_install` | `true` | When true, `bws` is auto-downloaded into `~/.relayhelm/bin/` on first use. |

## Failure modes

Bitwarden never blocks Relayhelm startup. If anything goes wrong, you'll see a one-line warning in stderr and Relayhelm continues with whatever credentials `.env` already had:
...
- Relayhelm will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — Relayhelm does not auto-upgrade `bws` to "latest" because upstream release shapes can change.

## When NOT to use this

- **Single-machine personal setups** where `~/.relayhelm/.env` is fine. You're trading one credential for another and adding a network dependency at startup.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple Relayhelm installations.
