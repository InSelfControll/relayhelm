...
Relayhelm is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Relayhelm checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.relayhelm/config.yaml`:
...
| `timeout` | `300` | Seconds Relayhelm waits for an approval reply before timing out. |
...
1. **CLI flag**: Start a session with `relayhelm --yolo` or `relayhelm chat --yolo`
...
When YOLO is active, Relayhelm shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Relayhelm refuses to run them **regardless** of:
...
Configure the timeout in `~/.relayhelm/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.relayhelm/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.relayhelm/.env` | Overwrite sensitive file via redirection |
...
Commands approved with "always" are saved to `~/.relayhelm/config.yaml`:
...
:::tip
Use `relayhelm config edit` to review or remove patterns from your permanent allowlist.
:::
...
The command scans the session database (`~/.relayhelm/state.db`) for
...
| Relayhelm credential stores | `auth.json`, `.env`, `.anthropic_oauth.json`, `mcp-tokens/`, `pairing/` under HERMES_HOME (active profile and global root) |
...
- Set automatically in the [official Docker image](https://github.com/InSelfControll/relayhelm) (`HERMES_WRITE_SAFE_ROOT=/opt/data`)
- Supports multiple roots separated by `:` on Unix or `;` on Windows
- **Do not add to `~/.relayhelm/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.relayhelm/cron/jobs.json`, profile skills, or other Relayhelm state outside that prefix

To allow both a workspace and Relayhelm home:
...
### Cron and other Relayhelm state

Do not ask the agent to `patch` `~/.relayhelm/cron/jobs.json` directly. Use the `cronjob` tool, [`relayhelm cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Relayhelm control files when write safety blocks direct edits.
...
When running the messaging gateway, Relayhelm controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.relayhelm/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.relayhelm/.env to allow open access,
...
For more flexible authorization, Relayhelm includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.
...
Control how unauthorized direct messages are handled in `~/.relayhelm/config.yaml`:
...
```bash
docker exec -u hermes relayhelm hermes pairing approve telegram ABC12DEF
```
...
[i10270]: https://github.com/InSelfControll/relayhelm/issues/10270
:::

**Storage:** Pairing data is stored in `~/.relayhelm/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Relayhelm applies strict security hardening to every container.
...
Container resources are configurable in `~/.relayhelm/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.relayhelm/sandboxes/docker/<task_id>/`
...
When loaded, Relayhelm checks if these files exist in the active profile's `HERMES_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Relayhelm infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Relayhelm infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Relayhelm integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Relayhelm under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.relayhelm/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.relayhelm/logs/` for unauthorized access attempts
10. **Keep updated** — run `relayhelm update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.relayhelm/.env`:
...
Relayhelm ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `hermes_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `relayhelm doctor` for the full remediation.
- **`relayhelm doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
```bash
relayhelm doctor --ack <advisory-id>
```
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Relayhelm installs these **lazily** on first use rather than eagerly under `relayhelm[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `relayhelm tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `relayhelm tools`.
