...
title: "Git Worktrees"
description: "Run multiple Relayhelm agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Relayhelm is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Relayhelm so each session has a clean, isolated working directory.

## Why Use Worktrees with Relayhelm?

Relayhelm treats the **current working directory** as the project root:

- CLI: the directory where you run `hermes` or `relayhelm chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.relayhelm/config.yaml`
...
Relayhelm creates `.worktrees/my-experiment/` inside the repo (branch
...
Now you can `cd` into the new worktree and run Relayhelm there:

```bash
cd ../repo-feature

# Start Relayhelm in the worktree
hermes
```
...
- Relayhelm checkpoint data under `~/.relayhelm/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Relayhelm experiment**
...
- **Avoid running Relayhelm from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `relayhelm -w` (Automatic Worktree Mode)

Relayhelm has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
```bash
relayhelm -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `relayhelm -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Relayhelm session its own clean checkout.
