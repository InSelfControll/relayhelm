...
Run multiple independent Relayhelm agents on the same machine — each with its own config, API keys, memory, sessions, skills, and gateway state.

## What are profiles?

A profile is a separate Relayhelm home directory. Each profile gets its own directory containing its own `config.yaml`, `.env`, `SOUL.md`, memories, sessions, skills, cron jobs, and state database. Profiles let you run separate agents for different purposes — a coding assistant, a personal bot, a research agent — without mixing up Relayhelm state.

:::caution Give every agent its own profile
Never point two agent processes at the same profile (the same Relayhelm home). Both write memory automatically, and each loads the other's writes into its system prompt at session start — so two writers on one home compound each other's state until it stops being anything you configured. Profiles exist exactly to prevent this; agents that need shared memory should use an [external memory provider](/user-guide/features/memory-providers) instead.
:::
...
```bash
relayhelm profile create coder       # creates profile + "coder" command alias
coder setup                       # configure API keys and model
coder chat                        # start chatting
```

That's it. `coder` is now its own Relayhelm profile with its own config, memory, and state.

## Creating a profile

:::tip
Quickest setup: run `relayhelm setup --portal` inside the new profile to wire up models + tools at once. See [Nous Portal](/integrations/nous-portal).
:::

### Blank profile

```bash
relayhelm profile create mybot
```
...
```bash
relayhelm profile create researcher --description "Reads source code and external docs, writes findings."
```

You can also set or auto-generate the description later with `relayhelm profile describe` — see the [Kanban guide](./features/kanban#auto-vs-manual-orchestration) for the full routing model.
...
Copies your current profile's `config.yaml`, `.env`, `SOUL.md`, and skills into the new profile. Same API keys, model, and capabilities, but fresh sessions and memory. Edit `~/.relayhelm/profiles/work/.env` for different API keys, or `~/.relayhelm/profiles/work/SOUL.md` for a different personality.

### Clone everything (`--clone-all`)

```bash
relayhelm profile create backup --clone-all
```

Copies **everything** — config, API keys, personality, all memories, skills, cron jobs, plugins. A complete working snapshot. Per-profile history is excluded (session history, `state.db`, `backups/`, `state-snapshots/`, `checkpoints/`) — these belong to the source profile and can reach tens of GB. For a full backup including history, use `relayhelm profile export` or `relayhelm backup` instead.

:::note OAuth logins are shared, not copied
Anthropic (Claude Pro/Max), OpenAI Codex, and xAI OAuth logins use **single-use refresh tokens** — a copy of one is not a second credential, it is the same credential with two owners, and the first profile to refresh it revokes it for every other copy. `--clone-all` (and the dashboard's credential mirroring) therefore drops those OAuth rows from the clone. The new profile keeps reading the login from the root `~/.relayhelm/auth.json`, and a token refresh performed inside any profile is written back to root, so all profiles stay signed in. Static API keys are copied as usual. To give a profile its own separate OAuth login, run `relayhelm -p <name> auth add <provider>` inside it.
:::

### Clone from a specific profile

```bash
relayhelm profile create work --clone-from coder
```
...
```bash
relayhelm profile create work-backup --clone-from coder --clone-all
```
...
The alias works with every hermes subcommand — it's just `relayhelm -p <name>` under the hood.
...
```bash
relayhelm -p coder chat
relayhelm --profile=coder doctor
relayhelm chat -p coder -q "hello"    # works in any position
```

### Sticky default (`relayhelm profile use`)

```bash
relayhelm profile use coder
relayhelm chat                   # now targets coder
relayhelm tools                  # configures coder's tools
relayhelm profile use default    # switch back
```
...
- **`relayhelm profile`**: Shows current profile name, path, model, gateway status

## Profiles vs workspaces vs sandboxing

Profiles are often confused with workspaces or sandboxes, but they are different things:

- A **profile** gives Relayhelm its own state directory: `config.yaml`, `.env`, `SOUL.md`, sessions, memory, logs, cron jobs, and gateway state.
...
Using `cwd: "."` on the local backend means "the directory Relayhelm was launched from", not "the profile directory".
...
```bash
# Edit coder's tokens
nano ~/.relayhelm/profiles/coder/.env

# Edit assistant's tokens
nano ~/.relayhelm/profiles/assistant/.env
```
...
```bash
coder gateway install         # creates relayhelm-gateway-coder systemd/launchd service
assistant gateway install     # creates relayhelm-gateway-assistant service
```

Each profile gets its own service name. They run independently.

:::note Inside the official Docker image
Per-profile gateways are supervised by [s6-overlay](https://github.com/just-containers/s6-overlay) (PID 1 in the container), so `relayhelm profile create <name>` automatically registers an s6 service slot at `/run/service/gateway-<name>/`. `relayhelm -p <name> gateway start/stop/restart` dispatches to `s6-svc` instead of spawning a bare process — crashes are auto-restarted and `docker restart` preserves the previously-running set of gateways. See [Per-profile gateway supervision](/user-guide/docker#per-profile-gateway-supervision) for details.
:::
...
```bash
coder config set model.default anthropic/claude-sonnet-4
echo "You are a focused coding assistant." > ~/.relayhelm/profiles/coder/SOUL.md
```
...
default for **future CLI/gateway runs** (same as `relayhelm profile use`) —
to edit a profile from the dashboard, use the switcher instead.

## Updating

`relayhelm update` pulls code once (shared) and syncs new bundled skills to **all** profiles automatically:
...
```bash
relayhelm profile list           # show all profiles with status
relayhelm profile show coder     # detailed info for one profile
relayhelm profile rename coder dev-bot   # rename (updates alias + service)
relayhelm profile export coder   # pack into coder.tar.gz (shareable; keys stripped)
relayhelm profile import coder.tar.gz   # install an archive as a new profile
```
...
renamed because `~/.relayhelm` is the installation root. Renaming it instead
sets a **display name**, which UI surfaces show in place of the bare ID:

```bash
relayhelm profile rename default Harumesu   # Unicode fine: 小助手
```

The display name appears in `relayhelm profile list`/`show`, the `/profile`
...
stored as `display_name` in `~/.relayhelm/profile.yaml`; remove that line to
...
```bash
relayhelm profile delete coder
```

This stops the gateway, removes the systemd/launchd service, removes the command alias, and deletes all profile data. You'll be asked to type the profile name to confirm.

Use `--yes` to skip confirmation: `relayhelm profile delete coder --yes`

:::note
You cannot delete the default profile (`~/.relayhelm`). To remove everything, use `hermes uninstall`.
:::
...
Profiles use the `HERMES_HOME` environment variable. When you run `coder chat`, the wrapper script sets `HERMES_HOME=~/.relayhelm/profiles/coder` before launching hermes. Since 119+ files in the codebase resolve paths via `get_hermes_home()`, Relayhelm state automatically scopes to the profile's directory — config, sessions, memory, skills, state database, gateway PID, logs, and cron jobs.
...
- `HERMES_HOME` is the profile boundary. It controls Relayhelm config, `.env`,
  memory, sessions, skills, logs, cron jobs, gateway state, and other Relayhelm
  data.
- `HOME` is the operating-system/user home that external CLIs expect. On host
  installs, Relayhelm keeps it as the real user home by default so tools like
...
profile` in that profile's `config.yaml`. In that mode Relayhelm launches tool
...
Relayhelm also exposes `HERMES_REAL_HOME` to subprocesses so scripts can still find
the actual account home when `home_mode: profile` is active.

The default profile is simply `~/.relayhelm` itself. No migration needed — existing installs work identically.
...
relayhelm profile export coder
relayhelm profile import ./coder.tar.gz --name coder
```
