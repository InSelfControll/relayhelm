...
If you only run one Relayhelm agent, you don't need this page — see
...
[Connecting Desktop to Many Relayhelm Instances](./multi-connection-desktop.md).

## When to use this

You want this setup when you have two or more Relayhelm agents that should all
...
(`io.github.inselfcontroll.relayhelm.gateway-<name>.plist`) or systemd user service
(`relayhelm-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
relayhelm profile create coder
relayhelm profile create personal-bot
relayhelm profile create research
...
```bash
relayhelm config set gateway.multiplex_profiles true
relayhelm gateway restart
```

Equivalently, in the default profile's `~/.relayhelm/config.yaml`:
...
You do **not** run `relayhelm gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `relayhelm gateway start` / `run` is a
...
- `/p/coder/...` API-server requests must use `API_SERVER_KEY` from
  `~/.relayhelm/profiles/coder/.env`; the default listener key is rejected.
...
home). `relayhelm status` reports the multiplexer and the profiles it serves;
`relayhelm status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/relayhelm-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: relayhelm-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    relayhelm gateway "$action"
  else
    relayhelm -p "$profile" gateway "$action"
  fi
}
...
```bash
relayhelm-gateways start      # start every configured profile
relayhelm-gateways stop       # stop every configured profile
relayhelm-gateways restart    # restart all
relayhelm-gateways status     # status across all
relayhelm-gateways list       # delegates to `relayhelm gateway list`
```

:::tip
The `default` profile is targeted with `relayhelm gateway <action>` (no `-p`),
not `relayhelm -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `relayhelm -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/io.github.inselfcontroll.relayhelm.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/relayhelm-gateway-<profile>.service`         |

The default profile keeps the historical names: `io.github.inselfcontroll.relayhelm.gateway.plist` /
...
```bash
# Default profile
tail -f ~/.relayhelm/logs/gateway.log
tail -f ~/.relayhelm/logs/gateway.error.log

# Named profile
tail -f ~/.relayhelm/profiles/<name>/logs/gateway.log
tail -f ~/.relayhelm/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.relayhelm/logs/gateway.log ~/.relayhelm/profiles/*/logs/gateway.log
```
...
relayhelm -p coder logs -f         # follow one profile
...
```bash
relayhelm profile list             # profiles + model + gateway state
relayhelm-gateways status          # full status across every profile
launchctl list | grep hermes    # macOS — PIDs and labels
systemctl --user list-units 'relayhelm-gateway-*'   # Linux — units
```
...
```
~/.relayhelm/profiles/<name>/
├── .env              # API keys, bot tokens (chmod 600)
├── config.yaml       # model, provider, toolsets, gateway settings
└── SOUL.md           # personality / system prompt
```

The default profile uses `~/.relayhelm/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
relayhelm config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.relayhelm/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=relayhelm --why="gateways running" \
  sleep infinity &
...
`relayhelm-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.relayhelm/.env ~/.relayhelm/profiles/*/.env
```

## Updating the code

`relayhelm update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `relayhelm gateway start` after a previous `relayhelm gateway stop`. The
...
cat ~/.relayhelm/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/io.github.inselfcontroll.relayhelm.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/io.github.inselfcontroll.relayhelm.gateway-<profile>.plist

# Linux
systemctl --user restart relayhelm-gateway-<profile>.service
```

### Health check

```bash
relayhelm doctor                  # default profile
relayhelm -p <profile> doctor     # one profile
```
