---
sidebar_position: 30
title: "Relayhelm Relay"
description: "Connect Relayhelm to messaging platforms through a relay connector that owns the platform credentials — enrollment, capabilities, config, and troubleshooting"
---

# Relayhelm Relay (Connector)
...
Relayhelm Relay is not a chat platform itself — it is a **connector system** that
...
1. Resolves a fresh Nous Portal access token from your existing login
   (`~/.relayhelm/auth.json`) — this proves which Nous org (tenant) you own. If
...
3. The connector verifies the token (signature, single-use, tenant match),
   ...
   once.
4. Persists the credentials into `~/.relayhelm/.env`:
...
`relayhelm gateway enroll` refuses to run in managed/hosted installs — there the
...
|---------|-------|---------|
| `GATEWAY_RELAY_URL` | env (`~/.relayhelm/.env`) | Connector relay WebSocket URL. Presence enables the relay platform. |
...
token. Re-login with `relayhelm auth add nous` (or `relayhelm setup`) and retry.
...
startup. Restart it (`relayhelm gateway restart`).
