...
description: "Chat with Relayhelm from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Relayhelm from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Relayhelm](/guides/use-voice-mode-with-hermes).
...
```mermaid
flowchart TB
    subgraph Gateway["Relayhelm Gateway"]
...
For group chats, hooks, and automation flows, Relayhelm supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Relayhelm keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Relayhelm does not hide failures just because the text resembles a silence token.
...
```bash
relayhelm gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
relayhelm gateway              # Run in foreground
relayhelm gateway setup        # Configure messaging platforms interactively
relayhelm gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo relayhelm gateway install --system   # Linux only: install a boot-time system service
relayhelm gateway start        # Start the default service
relayhelm gateway stop         # Stop the default service
relayhelm gateway status       # Check default service status
relayhelm gateway status --system         # Linux only: inspect the system service explicitly
```
...
```yaml title="~/.relayhelm/config.yaml"
gateway:
...
```bash
relayhelm gateway install --force
```

A positive value makes the generated unit use `Type=notify`,
`NotifyAccess=main`, and the matching `WatchdogSec`. Relayhelm sends heartbeats
...
| `/update` | Update Relayhelm to the latest version |
...
with the `session_reset` section in `~/.relayhelm/config.yaml`:
...
Configure per-platform overrides in `~/.relayhelm/gateway.json`:
...
Different channels can run different models and personas from a **single gateway** — e.g. a cheap fast model in `#daily` and a frontier model with a specialist prompt in `#dev`. Configure `channel_overrides` under the platform in `~/.relayhelm/gateway-config.yaml`:
...
The first time you message a busy agent on any platform, Relayhelm appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.relayhelm/config.yaml`:
...
Setting `display.tool_progress: log` sends **no** progress bubbles to chat. Instead, each tool call is appended as a line to `~/.relayhelm/logs/tool_calls.log` — a rotating audit file (5 MB × 3 backups) run through the same secret-redacting formatter as regular logs, so credentials never land on disk. Use it when you want a full tool-call trail without any chat noise.
...
- `~/.relayhelm/status_phrases.yaml` or any `*.yaml` in `~/.relayhelm/status_phrases/` (conventional paths, auto-loaded), or
...
Off by default. When enabled, Relayhelm prepends a human-readable timestamp
...
Relayhelm confirms immediately:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.relayhelm/config.yaml`:
...
relayhelm gateway start                 # Start the service
relayhelm gateway stop                  # Stop the service
relayhelm gateway status                # Check status
journalctl --user -u relayhelm-gateway -f  # View logs

# Enable lingering (keeps running after logout)
sudo loginctl enable-linger $USER

# Or install a boot-time system service that still runs as your user
sudo relayhelm gateway install --system
sudo relayhelm gateway start --system
sudo relayhelm gateway status --system
journalctl -u relayhelm-gateway -f
```

Use the user service on laptops and dev boxes. Use the system service on VPS or headless hosts that should come back at boot without relying on systemd linger.

:::danger Don't add a custom `ExecStopPost` kill drop-in
The unit Relayhelm installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit relayhelm-gateway` (or `sudo systemctl edit relayhelm-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `relayhelm update`. When `relayhelm update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart relayhelm-gateway` command (it never blocks on an interactive password prompt).
...
```bash
relayhelm gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `relayhelm update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo relayhelm update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/relayhelm-gateway`:

```
hermes ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed relayhelm-gateway*, /usr/bin/systemctl --no-ask-password start relayhelm-gateway*, /usr/bin/systemctl --no-ask-password restart relayhelm-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Relayhelm will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Relayhelm installations on the same machine (with different `HERMES_HOME` directories), each gets its own systemd service name. The default `~/.relayhelm` uses `relayhelm-gateway`; other installations use `relayhelm-gateway-<hash>`. The `relayhelm gateway` commands automatically target the correct service for your current `HERMES_HOME`.
:::

### macOS (launchd)

```bash
relayhelm gateway install               # Install as launchd agent
relayhelm gateway start                 # Start the service
relayhelm gateway stop                  # Stop the service
relayhelm gateway status                # Check status
tail -f ~/.relayhelm/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/io.github.inselfcontroll.relayhelm.gateway.plist`. It includes three environment variables:
...
- **HERMES_HOME** — scopes the gateway to your Relayhelm installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `relayhelm gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `HERMES_HOME` directory gets its own launchd label. The default `~/.relayhelm` uses `io.github.inselfcontroll.relayhelm.gateway`; other installations use `io.github.inselfcontroll.relayhelm.gateway-<suffix>`.
:::
...
`platforms.<name>.enabled: false` in `~/.relayhelm/config.yaml` is authoritative.
...
1. **Gateway log** (`~/.relayhelm/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
