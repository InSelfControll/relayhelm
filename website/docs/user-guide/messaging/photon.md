...
Connect Relayhelm to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Relayhelm — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Relayhelm runs it in a
...
2. **Finds or creates** the `Relayhelm` project on your account.
...
6. **Runs `npm install`** inside the plugin's sidecar directory. On
   ...
   `~/.relayhelm/photon/sidecar`; set `PHOTON_SIDECAR_DIR` to pin an
   explicit location.

Runtime credentials are written to `~/.relayhelm/.env`
...
(device token, dashboard project id) lives in `~/.relayhelm/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Relayhelm
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Relayhelm replies with a pairing code. Approve it with:
...
**Pre-authorize specific numbers** (in `~/.relayhelm/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.relayhelm/.env`):
...
By default Relayhelm responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Relayhelm` and
`@Relayhelm agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Relayhelm will reply.
...
assigned iMessage line Relayhelm uses. When a Photon token and dashboard project
...
- **Outbound attachments are supported.** Relayhelm sends images, voice
  ...
  media.
- **Native polls are supported.** Relayhelm sends poll content through
...
- **Read receipts are supported.** The sidecar marks an inbound iMessage
  read after forwarding it to Relayhelm, so the sender sees `Read` without
...
- **Message effects are supported.** Relayhelm sends text with native iMessage
...
- **Shared/free-tier lines can't initiate conversations with new
  ...
  brand-new recipient will be rejected by Photon even when Relayhelm is set
...
| `PHOTON_MENTION_PATTERNS` | Relayhelm wake words  | JSON list / comma / newline regex patterns for group mentions |
