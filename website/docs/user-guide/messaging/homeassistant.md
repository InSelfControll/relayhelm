...
description: Control your smart home with Relayhelm via Home Assistant integration.
...
Relayhelm integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Relayhelm"
5. Copy the token
...
Relayhelm registers four tools for smart home control:
...
Configure which events the agent sees in `~/.relayhelm/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Relayhelm".
...
The adapter reads credentials from `~/.relayhelm/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Relayhelm profile
...
running Relayhelm — `curl -H "Authorization: Bearer <token>" <url>/api/` should
