...
description: "Set up Relayhelm as a WhatsApp bot via Meta's official Business Cloud API"
---

# WhatsApp Business Cloud API Setup

Relayhelm can connect to WhatsApp through Meta's **official** WhatsApp Business Cloud API. This is the production-grade path: no Node.js bridge subprocess, no QR codes, no account-ban risk.
...
- The Relayhelm gateway needs a **public HTTPS URL** so Meta can deliver inbound messages via webhook.
- Replies more than 24 hours after the user's last message require a pre-approved **template** (this is Meta's "customer service window" rule, not a Relayhelm limit).
...
4. **Optional but recommended**: ffmpeg on `PATH` so outbound voice messages render as native WhatsApp voice-note bubbles (green waveform) instead of MP3 audio attachments. Relayhelm degrades gracefully if absent.
...
6. Copy the token → update `WHATSAPP_CLOUD_ACCESS_TOKEN` in `~/.relayhelm/.env` → restart the gateway.

System User tokens don't expire unless you explicitly revoke them.

---

## Exposing Relayhelm to the internet

The Cloud API delivers inbound messages by HTTPS POST to your webhook URL — that means the Relayhelm gateway has to be reachable from Meta's servers.  Three common ways:
...
2. Generate a **Verify Token** — the wizard does this for you with `secrets.token_urlsafe(32)`; if you're configuring manually, run:
   ...
   ```
   Save it as `WHATSAPP_CLOUD_VERIFY_TOKEN` in `~/.relayhelm/.env`.
3. Start the Relayhelm gateway: `relayhelm gateway`.
...
In addition to Meta's recipient whitelist, Relayhelm has its own per-platform allowlist that controls **which incoming messages the agent processes**.  Add to `~/.relayhelm/.env`:
...
All settings live in `~/.relayhelm/.env`.  Required values are in **bold**.
...
When the agent invokes any of these flows, Relayhelm uses WhatsApp's native interactive messages — tap-to-answer buttons instead of "reply with the number" prompts:
...
Relayhelm acknowledges inbound messages immediately:
...
Relayhelm TTS produces MP3. Two paths:
...
Relayhelm warns the agent about this window in its system prompt, so the model knows to mention it when scheduling delayed messages.

Message-template support (the workaround for outside-window sends) is not yet implemented in Relayhelm. If you need it, please [open an issue](https://github.com/InSelfControll/relayhelm/issues) — it's planned but waiting on a clear demand signal.

### Group chats

The Cloud API has limited group support (capability-tier gated by Meta).  Relayhelm's `whatsapp_cloud` adapter currently handles **direct messages only** in v1.  If you need group chats, use the Baileys bridge.

### Outbound rate limit

Meta's default throughput is **80 messages/second per business phone number**, with upgrades available.  Relayhelm doesn't currently enforce this client-side — extremely high-volume sends could hit Meta's limit.
...
- **Verify token mismatch** — the token in `~/.relayhelm/.env`'s `WHATSAPP_CLOUD_VERIFY_TOKEN` must match exactly what you typed into Meta's dashboard.  Run the curl probe above to confirm the gateway's verify handshake works locally first.
- **Gateway not running** — check `relayhelm gateway` is up.
- **App Secret not set** — without it, Relayhelm refuses inbound POSTs with 503.  Meta interprets that as "can't validate."
...
- Wait for template support to land in Relayhelm.
...
Common cause: the toolset configured for `whatsapp_cloud` is missing the tools the agent wants to call.  Check `relayhelm tools list` and verify the platform is using `hermes-whatsapp` (the default Cloud adapter toolset, same as Baileys).
...
The default `stt.provider: local` requires `pip install faster-whisper`.  If you're a Nous subscriber, you can route STT through the managed gateway instead — select **Nous Subscription** for speech-to-text in `relayhelm tools`, or set it directly:

```bash
relayhelm config set stt.provider nous
...
- **Treat the App Secret like a password** — anyone with it can forge webhook payloads that Relayhelm will accept as authentic.
...
Most users running Relayhelm for personal projects prefer Baileys. Most users running customer-facing bots prefer Cloud API.
