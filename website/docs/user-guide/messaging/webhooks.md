...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Relayhelm — Webhooks Tutorial"
...
1. Enable via `relayhelm gateway setup` or environment variables
...
| `script` | No | Filter/transform script under `~/.relayhelm/scripts/`. The webhook payload is passed as JSON on stdin. JSON object stdout replaces the payload before templating; text stdout is exposed as `script_output`; empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. |
          ...
          filters:
            ...
            - any:
                ...
                - field: "payload.project_id"
                  in_file: "~/.relayhelm/data/todoist/watchlist.json"
...
Use `script` when declarative filters are not enough. Scripts must live under `~/.relayhelm/scripts/` for the active profile; relative paths resolve there, and path traversal outside that directory is blocked. `.sh` and `.bash` scripts run with bash, and all other extensions run with the current Python interpreter.

The route payload is sent to stdin as JSON:

```python
# ~/.relayhelm/scripts/todoist-hermes-label.py
import json
import sys
...
Add the `github-pr` route to your `~/.relayhelm/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Relayhelm processes the event, and posts a review comment on the PR.
...
- Subscriptions are stored in `~/.relayhelm/webhook_subscriptions.json`
...
For dynamic subscriptions, add the `toolsets` key by editing `~/.relayhelm/webhook_subscriptions.json` directly:
...
This is the same trust model that applies to everything the agent reads: web pages, files, and tool output are all untrusted input. Relayhelm does not — and cannot reliably — sanitize untrusted text with a blocklist; phrasing, encoding, and translation make that trivially bypassable. **The trust boundary is the agent's capability surface, not the input channel.** Harden there:
...
- Run the gateway in foreground to see logs: `relayhelm gateway run`
