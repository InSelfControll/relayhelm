...
Connect Relayhelm to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Relayhelm supports two WeCom integration modes:
...
> Run `relayhelm gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Relayhelm decrypts the message, queues it for the agent
...
(Use `relayhelm gateway start` only after `relayhelm gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Relayhelm and the token the
...
in `~/.relayhelm/.env` values around `=` will also break signature checks. After
fixing, restart `relayhelm gateway run`.
...
Check `relayhelm gateway run` logs for the bound host/port. If the adapter bound to
