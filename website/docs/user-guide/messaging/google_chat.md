...
sidebar_position: 12
title: "Google Chat"
description: "Set up Relayhelm as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Relayhelm to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Relayhelm
...
> Run `relayhelm gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Relayhelm can read (e.g.,
`~/.relayhelm/google-chat-sa.json`, `chmod 600`).
...
Also grant `Pub/Sub Viewer` on the same subscription — Relayhelm calls
...
- **App name**: whatever you want users to see ("Relayhelm" is reasonable).
...
`ADDED_TO_SPACE` event that Relayhelm uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Relayhelm

Add the Google Chat section to `~/.relayhelm/.env`:
...
Send "hola" in the test DM. The bot posts a "Relayhelm is thinking…" marker, then
...
`~/.relayhelm/config.yaml` — e.g. a kitten assistant named Ada:

```yaml
platforms:
  google_chat:
    # Custom working-state marker text (default: "Relayhelm is thinking…").
...
Thread support: when a user replies inside a thread, Relayhelm detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Relayhelm session.
...
3. Download the JSON. Move it onto the host that runs Relayhelm.
4. Register the client with Relayhelm (run under the profile you want it scoped to):
...
relayhelm -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Relayhelm home (e.g.
`~/.relayhelm/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.relayhelm/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.relayhelm/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Relayhelm isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `relayhelm gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Relayhelm will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.relayhelm/google_chat_user_tokens/<sanitized_email>.json` (filesystem
