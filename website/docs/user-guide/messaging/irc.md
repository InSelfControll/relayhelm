...
> Run `relayhelm gateway setup` and pick **IRC** for a guided walk-through.
...
You can configure IRC two ways — environment variables (for a quick env-only setup) or the `gateway` block in `~/.relayhelm/gateway-config.yaml`.
...
By default, only nicks listed in `allowed_users` (or `IRC_ALLOWED_USERS`) may talk to the bot. Leave the list empty **and** set `IRC_ALLOW_ALL_USERS=true` to let anyone in the channel chat with Relayhelm — useful for testing, but not recommended on public networks since IRC nicks are not authenticated unless the network enforces NickServ.
...
Check status with `relayhelm gateway status` — IRC connection state is reported there, including for env-only setups.
...
- The adapter acquires a scoped credential lock per server+nick, so two Relayhelm profiles won't fight over the same IRC identity.
