...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Relayhelm terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Relayhelm's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `relayhelm setup --portal` — and you're ready to `relayhelm chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Relayhelm also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `relayhelm --tui` — see the [TUI](tui.md) guide.
:::
...
relayhelm chat --query-file prompt.txt
relayhelm chat --query-file - < prompt.txt

# With a specific model
relayhelm chat --model "anthropic/claude-sonnet-4"

# With a specific provider
relayhelm chat --provider nous        # Use Nous Portal
relayhelm chat --provider openrouter  # Force OpenRouter
...
relayhelm chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
relayhelm --continue             # Resume the most recent CLI session (-c)
relayhelm --resume <session_id>  # Resume a specific session by ID (-r)
relayhelm --resume latest        # Resume the most recent session (same as -c)
relayhelm --resume latest --in ./dir  # Resume ./dir's latest session, staying in ./dir

# Verbose mode (debug output)
relayhelm chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
relayhelm -w                         # Interactive mode in worktree
relayhelm -w -z "Fix issue #123"     # Single query in worktree
```

### Worktree cleanup

`relayhelm -w` sessions create disposable worktrees under `<repo>/.worktrees/`.
...
- **Untracked-only scratch** (PR body drafts, notes) is archived to
  `~/.relayhelm/archive/worktree-prune/` before its tree is removed — never
  destroyed.
...
launches `relayhelm -w` for days — no longer accumulate merged scratch trees
...
The `relayhelm plugins` commands manage native Relayhelm plugins and portable Agent
...
relayhelm plugins disable <plugin-name>
relayhelm plugins update <plugin-name>
relayhelm plugins remove <plugin-name>
```

Portable packages remain disabled until explicitly enabled. Relayhelm currently
...
<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Relayhelm CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Relayhelm CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HERMES_YOLO_MODE` is on (either `relayhelm --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Relayhelm to background (Unix only). Run `fg` in the shell to resume. |
...
- **Approvals still apply.** A dangerous command (`rm -rf`, writes to `~/.relayhelm/config.yaml`, etc.) goes through the same approval prompt the agent's `terminal` tool uses. `!` is a cost/latency shortcut, not a security bypass.
...
Type `/` to see the autocomplete dropdown. Relayhelm supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Relayhelm replies |
...
```bash
relayhelm -s hermes-agent-dev,github-auth
relayhelm chat -s github-pr-workflow -s github-auth
```

Relayhelm loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.relayhelm/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.relayhelm/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Relayhelm..."
  # Add your own!
```
...
`Ctrl+J` and backslash continuation are enabled by default, matching Claude Code / Codex / OpenCode multiline shortcuts. On supported terminals such as iTerm2, Relayhelm also requests extended key reporting so `Shift+Enter` arrives as a distinct newline key. If your terminal sends LF for plain `Enter` and you need the legacy `Ctrl+J`-as-submit fallback, opt out:
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Relayhelm recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work by default. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Relayhelm — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The first time you press Enter while Relayhelm is working, Relayhelm prints a one-line reminder explaining the `/busy` knob. It only fires once per install; `onboarding.seen.busy_input_prompt` in `config.yaml` records that it was shown. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Relayhelm to the background — just like any terminal process. The shell prints a confirmation:

```
Relayhelm has been suspended. Run `fg` to bring Relayhelm back.
```
...
Resume this session with:
  relayhelm --resume 20260225_143052_a1b2c3
...
```bash
relayhelm --continue                          # Resume the most recent CLI session
relayhelm -c                                  # Short form
relayhelm -c "my project"                     # Resume a named session (latest in lineage)
relayhelm --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
relayhelm --resume "refactoring auth"         # Resume by title
relayhelm --resume latest                     # Resume the most recent session (same as -c)
relayhelm --resume latest --in ./my-project   # Latest session for ./my-project's workspace
relayhelm -r 20260225_143052_a1b2c3           # Short form
```
...
CLI sessions are stored in Relayhelm's SQLite state database under `~/.relayhelm/state.db`. The database keeps:
...
Relayhelm immediately confirms the task and gives you back the prompt:
