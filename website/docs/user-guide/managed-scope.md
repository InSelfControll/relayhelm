...
`~/.relayhelm/config.yaml`, `~/.relayhelm/.env`, and even the shell environment — for
...
```text
/etc/hermes/
├── config.yaml     # managed config layer (wins over ~/.relayhelm/config.yaml)
└── .env            # managed env layer (wins over ~/.relayhelm/.env + shell)
```
...
files. It is **never persisted** to any `.env` by Relayhelm.
...
left user-settable. `relayhelm doctor` reports the *resolved* managed directory so
...
| 2 | `~/.relayhelm/config.yaml` (user) | `~/.relayhelm/.env` (user) |
...
```bash
relayhelm config        # shows a header naming the managed source + the pinned keys
relayhelm doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Relayhelm refuses and names the source:

```bash
$ relayhelm config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/hermes/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `relayhelm config set` / setup will not write
...
Changes take effect on the next Relayhelm start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`relayhelm doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Relayhelm as `root`), managed scope is advisory.
