...
- A long-running process (relayhelm gateway, tui_gateway) misbehaves and you can't restart it
...
For long-lived processes: Relayhelm gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Relayhelm you have two practical options:
...
### `tui_gateway` subprocess (spawned by `relayhelm --tui`)
...
debugpy.wait_for_client()
```
Start `relayhelm --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Relayhelm subagents, debug one process at a time.
