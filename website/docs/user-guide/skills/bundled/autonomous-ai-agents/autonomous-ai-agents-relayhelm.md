---
title: "Relayhelm — Use, configure, theme, extend, and orchestrate Relayhelm"
sidebar_label: "Relayhelm"
description: "Use, configure, theme, extend, and orchestrate Relayhelm"
---
...
Use, configure, theme, extend, and orchestrate Relayhelm.
...
| Path | `skills/autonomous-ai-agents\relayhelm` |
| Version | `3.2.0` |
...
Relayhelm is an open-source AI agent framework by Nous Research that runs in your terminal, a native desktop app, messaging platforms, and IDEs. It's in the same category as Claude Code (Anthropic), Codex (OpenAI), and OpenClaw — autonomous coding and task-execution agents that use tool calling to interact with your system. Relayhelm works with any LLM provider (OpenRouter, Anthropic, OpenAI, Google, DeepSeek, xAI, local models, and 20+ others) and runs on Linux, macOS, Windows, and WSL.

What makes Relayhelm different:

- **Self-improving through skills** — Relayhelm learns from experience by saving reusable procedures as skills that load into future sessions.
...
- **Profiles** — run multiple independent Relayhelm instances with isolated configs, sessions, skills, and memory.
...
This skill is a concise operating guide, not the complete source of truth for every Relayhelm feature. If a Relayhelm feature, command, or setting is not mentioned here or in a reference, do not treat that absence as evidence that it does not exist. Check the live repository and official docs before giving a negative answer.

Good verification targets, cheapest first:

- **Every shipped feature, one line each: https://hermes-agent.nousresearch.com/docs/llms.txt.** Start here for any "can Relayhelm do X?" or "how do I do X?" — it indexes the entire documentation set with a link to the page that answers. It is generated from the docs tree on every build, so it is never behind the product. Fetch it with `web_extract`, or `curl -s https://hermes-agent.nousresearch.com/docs/llms.txt` when web tools are off. The whole documentation set in one file is at `/docs/llms-full.txt`.
- CLI commands: `relayhelm --help`, `hermes <command> --help`, and `hermes_cli/main.py`
- Source tree: https://github.com/InSelfControll/relayhelm

Never answer "Relayhelm can't do that" from memory. Relayhelm ships far more than this skill body describes, and the index exists so a negative answer is always checkable.
...
relayhelm dashboard               # web admin panel + embedded chat
...
```
~/.relayhelm/config.yaml       Main configuration (settings — never secrets)
~/.relayhelm/.env              API keys and secrets ONLY (under $HERMES_HOME if set)
$HERMES_HOME/skills/        Installed skills
~/.relayhelm/skins/            Custom themes (see references/themes.md)
~/.relayhelm/desktop-plugins/  Desktop app UI plugins (see references/desktop-plugins.md)
~/.relayhelm/tui-widgets/      TUI widget apps (see references/tui-widgets.md)
~/.relayhelm/pets/             Installed pet mascots (see references/petdex.md)
~/.relayhelm/state.db          Canonical session store (SQLite + FTS5)
~/.relayhelm/sessions/         Gateway routing index, request dumps, *.jsonl transcripts
...
Profiles use `~/.relayhelm/profiles/<name>/` with the same layout. When a profile is active, resolve the real home from `$HERMES_HOME` — never hardcode `~/.relayhelm`.

## Routing Table — load the reference for the task

| User wants... | Load |
|---|---|
| **Anything not listed below — "can Relayhelm do X?", "how do I set up X?"** | **https://hermes-agent.nousresearch.com/docs/llms.txt** |
...
need more than their docs page. For everything else Relayhelm ships, fetch
`llms.txt` and it maps the question to the page that answers it.

Two theming rules that hold even without loading the reference: **you apply skins yourself** (`relayhelm config set display.skin <name>` — every surface repaints live within ~a second; don't tell the user to run `/skin`), and **to tweak one color, edit the ACTIVE skin** (`hermes skin set <key> <hex>`) — never fork `default`, which drops the palette and resets the background.

## Spawning Additional Relayhelm Instances

Run additional Relayhelm processes as fully independent subprocesses — separate sessions, tools, and environments.
...
Relayhelm uses prompt_toolkit, which requires a real terminal. Use tmux for interactive spawning:
...
- **Use `relayhelm chat -q` for fire-and-forget** — no PTY needed
- **Use tmux for interactive sessions** — raw PTY mode has `\r` vs `\n` issues with prompt_toolkit
- **For scheduled tasks**, use the `cronjob` tool instead of spawning — handles delivery and retry
- **"delegate_task is capped at N" reports** — see `references/delegate-task-concurrency-diagnosis.md`. Three real cap paths in Relayhelm; if none fired, the model is self-limiting and rationalising it as "the runtime caps."
...
- **Web dashboard** (`relayhelm dashboard`) — full admin panel: messaging channels, MCP catalog, webhooks, memory, profile builder, plus an embedded `relayhelm --tui` chat. Secured behind an OAuth/token gate.
- **Ink TUI** (`relayhelm --tui` or `display.interface: tui`) — terminal UI with docked widget apps — `references/tui-widgets.md`.
...
- **Never hand-edit `config.yaml` for the user** — use `relayhelm config set KEY VAL`; a stray indent can corrupt the file and break the live gateway.
