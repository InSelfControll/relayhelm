...
| Related skills | [`claude-code`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-claude-code), [`relayhelm`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-relayhelm) |
...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Relayhelm terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Relayhelm itself, `model.provider: openai-codex` uses Hermes-managed Codex
OAuth from `~/.relayhelm/auth.json` after `relayhelm auth add openai-codex`. For the
...
## Relayhelm Gateway Caveat

When invoking the Codex CLI from a Relayhelm gateway/service context (for example,
