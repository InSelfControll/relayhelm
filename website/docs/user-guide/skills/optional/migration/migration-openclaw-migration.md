---
title: "Openclaw Migration — Import an OpenClaw setup (memories, skills) into Relayhelm"
sidebar_label: "Openclaw Migration"
description: "Import an OpenClaw setup (memories, skills) into Relayhelm"
---
...
Import an OpenClaw setup (memories, skills) into Relayhelm.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `relayhelm skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration\openclaw-migration` |
| Version | `1.0.0` |
| Author | Relayhelm (Nous Research) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Relayhelm`, `Memory`, `Persona`, `Import` |
| Related skills | [`relayhelm`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-relayhelm) |
...
# OpenClaw -> Relayhelm Migration

Use this skill when a user wants to move their OpenClaw setup into Relayhelm with minimal manual cleanup.
...
**First-time setup:** The `relayhelm setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the Relayhelm home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Relayhelm memory entries
- merge OpenClaw command approval patterns into Relayhelm `command_allowlist`
- migrate Hermes-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Relayhelm working-directory configuration
- copy OpenClaw skills into `~/.relayhelm/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Relayhelm workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.relayhelm/tts/`
- archive non-secret docs that do not have a direct Relayhelm destination
...
- `~/.relayhelm/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `~/.relayhelm/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.relayhelm/skills/migration/openclaw-migration/`.
...
Relayhelm CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Relayhelm. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Relayhelm workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Relayhelm skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.relayhelm/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python /home/USER/.relayhelm/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python ~/.relayhelm/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.relayhelm/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.relayhelm/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.relayhelm/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Relayhelm unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Relayhelm targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Relayhelm destination. Unsupported auth blobs must still be reported as skipped.
...
- Relayhelm persona state imported
- Relayhelm memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.relayhelm/skills/openclaw-imports/`
