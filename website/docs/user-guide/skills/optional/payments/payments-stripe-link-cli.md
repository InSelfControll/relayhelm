...
| | |
|---|---|
| Source | Optional — install with `relayhelm skills install official/payments/stripe-link-cli` |
...
Wraps [@stripe/link-cli](https://github.com/stripe/link-cli) so Relayhelm can complete purchases on the user's behalf using one-time-use virtual cards or Shared Payment Tokens (SPT). Every spend is gated by an in-app approval in the Link mobile/web app — Relayhelm cannot self-approve.
...
The Link account, payment method, and spend-approval app do NOT need to be set up before Relayhelm attempts to pay — the CLI walks the user through them on first run:
...
```
link-cli auth login --client-name "Relayhelm" --interval 5 --timeout 300
```
...
`@stripe/link-cli --mcp` exposes the same commands as MCP tools over stdio. To register it with Relayhelm' native MCP:
