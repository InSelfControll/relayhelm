...
| | |
|---|---|
| Source | Optional — install with `relayhelm skills install official/security/1password` |
| Path | `optional-skills/security\1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Relayhelm |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Relayhelm terminal calls (desktop app flow only)
...
### Service Account (recommended for Relayhelm)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${HERMES_HOME:-~/.relayhelm}/.env` (the skill will prompt for this on first load).
...
## Relayhelm Execution Pattern (desktop app flow)

Relayhelm terminal commands are non-interactive by default and can lose auth context between calls.
