...
| | |
|---|---|
| Source | Optional — install with `relayhelm skills install official/creative/pixel-art` |
...
import sys
import os
sys.path.insert(0, os.path.expanduser("~/.relayhelm/skills/creative/pixel-art/scripts"))
...
```bash
cd ~/.relayhelm/skills/creative/pixel-art/scripts
...
- ffmpeg on PATH (only needed for video — Relayhelm installs package this)
