...
| | |
|---|---|
| Source | Optional — install with `relayhelm skills install official/creative/pretext` |
...
This skill exists so Relayhelm can make **cool demos** with it — the kind people post to X. See `pretext.cool` and `chenglou.me/pretext` for the community demo corpus.
