...
| | |
|---|---|
| Source | Optional — install with `relayhelm skills install official/creative/touchdesigner-mcp` |
...
```
Relayhelm -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${HERMES_HOME:-$HOME/.relayhelm}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Relayhelm config (if missing)
...
3. **Restart Relayhelm session** to pick up the new MCP server
