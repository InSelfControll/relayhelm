...
| | |
|---|---|
| Source | Optional — install with `relayhelm skills install official/creative/unreal-mcp` |
...
Companion skill for the `unreal-engine` entry in the Relayhelm MCP catalog. The
...
Two halves, in this order: the editor side must be up before Relayhelm connects.
...
`~/.relayhelm/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Relayhelm — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Relayhelm connects
...
2. Start the Relayhelm session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Relayhelm
they appear as:

| Relayhelm tool | Purpose |
|---|---|
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Relayhelm runs batched calls
...
- **Start order matters.** Editor + server up first, then the Relayhelm
...
- **Timeouts on long operations.** Relayhelm' per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.relayhelm/config.yaml` for
