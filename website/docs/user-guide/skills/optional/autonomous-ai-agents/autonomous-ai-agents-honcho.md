---
title: "Honcho — Configure and troubleshoot Honcho memory for Relayhelm"
sidebar_label: "Honcho"
description: "Configure and troubleshoot Honcho memory for Relayhelm"
---
...
Configure and troubleshoot Honcho memory for Relayhelm.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `relayhelm skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Relayhelm

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Relayhelm profile its own peer identity while sharing a unified view of the user.
...
3. **AI peer card** -- the identity card for this Relayhelm profile's AI peer
...
Honcho models conversations as interactions between **peers**. Relayhelm creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Relayhelm instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Relayhelm run |
...
Each Relayhelm profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
Guidelines for Relayhelm when Honcho memory is active.
...
Run `hermes honcho setup`. Ensure `memory.provider: honcho` is in `~/.relayhelm/config.yaml`.
...
Use `--clone` when creating: `relayhelm profile create <name> --clone`. For existing profiles: `hermes honcho sync`.
...
| `hermes honcho sync` | Create host blocks for all Relayhelm profiles that don't have one yet |
| `hermes honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Relayhelm + Honcho |
| `relayhelm memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `relayhelm memory status` | Show active memory provider and config |
| `relayhelm memory off` | Disable external memory provider |
