---
title: "Relayhelm S6 Container Supervision — Modify or debug s6 services in the Relayhelm Docker image"
sidebar_label: "Relayhelm S6 Container Supervision"
description: "Modify or debug s6 services in the Relayhelm Docker image"
---

{/* This page is auto-generated from the skill's SKILL.md by website/scripts/generate-skill-docs.py. Edit the source SKILL.md, not this page. */}

# Relayhelm S6 Container Supervision

Modify or debug s6 services in the Relayhelm Docker image.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `relayhelm skills install official/devops/hermes-s6-container-supervision` |
...
# Relayhelm s6-overlay Container Supervision

## When to use this skill

Load this skill when you're working on:
- Adding or removing a static service in the Relayhelm Docker image (something that should be supervised at every container start, like the dashboard)
...
If you're just running the Relayhelm and want to use Docker, see `website/docs/user-guide/docker.md` instead.
...
│   └── dashboard/run                  ← if HERMES_DASHBOARD=1, runs `relayhelm dashboard`
│
├── /run/service (s6-svscan watches; tmpfs)
│   ├── gateway-coder/                 ← runtime-registered per-profile
│   │   ├── type        ("longrun")
│   │   ├── run         ("#!/command/with-contenv sh ... exec s6-setuidgid hermes relayhelm -p coder gateway run")
...
| `hermes_cli/gateway.py::_dispatch_via_service_manager_if_s6` | Intercepts `relayhelm gateway start/stop/restart` and routes to s6 when running in a container. |
...
Most likely the profile has no model or auth configured. The service slot is correct — the gateway itself is unconfigured. Run `relayhelm -p <profile> setup` first. The s6 supervisor will keep restarting it; that's the desired behavior (when you fix the config, the next attempt succeeds and stays up).

### Reconciler skipped a profile

The reconciler keys on the **presence of `SOUL.md`** as the "real profile" marker. `relayhelm profile create` always seeds it. If a profile dir is missing SOUL.md (stray directory, partial restore, backup-in-progress), the reconciler skips it intentionally. Add a `SOUL.md` (even empty) to opt back in.
...
- `hermes-agent-dev`: General relayhelm codebase navigation
