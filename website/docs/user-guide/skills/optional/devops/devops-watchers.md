...
| | |
|---|---|
| Source | Optional — install with `relayhelm skills install official/devops/watchers` |
...
Watch a GitHub repo (set `GITHUB_TOKEN` in `${HERMES_HOME:-~/.relayhelm}/.env` to avoid the 60 req/hr anonymous rate limit):
