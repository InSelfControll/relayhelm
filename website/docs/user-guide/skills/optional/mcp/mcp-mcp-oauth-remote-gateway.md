...
| | |
|---|---|
| Source | Optional — install with `relayhelm skills install official/mcp/mcp-oauth-remote-gateway` |
| Path | `optional-skills/mcp\mcp-oauth-remote-gateway` |
| Version | `1.0.0` |
| Author | Ben Barclay (benbarclay), Relayhelm |
| License | MIT |
| Platforms | linux, macos |
| Tags | `MCP`, `OAuth`, `PKCE`, `Remote-Deployment` |
| Related skills | [`relayhelm`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-relayhelm), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter), [`fastmcp`](/docs/user-guide/skills/optional/mcp/mcp-fastmcp) |
...
# MCP OAuth on a Remote Relayhelm Gateway

## Overview

Relayhelm' built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the Relayhelm process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when Relayhelm runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches Relayhelm.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files Relayhelm' token storage expects, so a subsequent `/reload-mcp` finds
...
2. Relayhelm is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI Relayhelm** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
Relayhelm' native MCP OAuth client (`tools/mcp_oauth.py`):

1. Picks a free local port `P`.
2. Registers a dynamic OAuth client with the AS, sending `redirect_uri = http://127.0.0.1:P/callback`.
3. Starts an HTTP server on `127.0.0.1:P` **inside the Relayhelm process**.
4. Prints the authorize URL and waits for the code at its local endpoint.

When Relayhelm runs remotely, the `127.0.0.1` in the `redirect_uri` is the remote
...
laptop and fails to connect. The callback never reaches the Relayhelm process, the
...
already cover the deployment. When Relayhelm detects a remote session it prints two
...
Both require an interactive terminal to the Relayhelm host. The rest of this skill
is for when there is NO interactive TTY — Relayhelm running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the Relayhelm Dashboard (try this BEFORE manual token surgery)

A remote Relayhelm gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `relayhelm dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'relayhelm dashboard'`). It exposes a connector/MCP console —
...
Relayhelm' `HermesTokenStorage` would have written, so on `/reload-mcp` Relayhelm finds
cached tokens and skips the browser flow entirely.
...
uses these same env vars, so if Relayhelm' own auto-detect says "headless", the
...
```json
{
  "client_name": "Relayhelm (manual OAuth)",
...
### 8. Write tokens in Relayhelm' exact schema
...
UA** — Cloudflare will 403 you even though Relayhelm (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, Relayhelm sees `auth: oauth`, calls `HermesTokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the Relayhelm process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** Relayhelm needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (Relayhelm does natively).
...
- **Don't claim Relayhelm doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in Relayhelm. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of Relayhelm' config.
