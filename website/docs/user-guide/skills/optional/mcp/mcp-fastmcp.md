...
| | |
|---|---|
| Source | Optional — install with `relayhelm skills install official/mcp/fastmcp` |
...
| Related skills | [`relayhelm`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-relayhelm), [`mcporter`](/docs/user-guide/skills/optional/mcp/mcp-mcporter) |
...
- smoke-test a server with the FastMCP CLI before wiring it into Relayhelm or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Relayhelm. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.relayhelm/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.relayhelm/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Relayhelm integration, either:

- configure the server in `~/.relayhelm/config.yaml` using the `native-mcp` skill, or
...
### Relayhelm cannot see the deployed server

The server-building part may be correct while the Relayhelm config is not. Load the `native-mcp` skill and configure the server in `~/.relayhelm/config.yaml`, then restart Relayhelm.
