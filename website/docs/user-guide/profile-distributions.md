...
A **profile distribution** packages a complete Relayhelm agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## Two ways to share a profile

Relayhelm has two sharing paths, and they answer different questions. Distributions are the durable one; export files are the quick one.

| | **Distribution** (git repo) | **Export file** (`.tar.gz`) |
|---|---|---|
| Ship it by | `relayhelm profile install <repo>` | Send a file — chat, AirDrop, USB, email |
| Recipient needs | git, and access to the repo | The file |
| Updates | `relayhelm profile update` pulls new versions | Re-send the file |
...
| Made with | `relayhelm profile install` / `update` | `/export` and `/import`, or `relayhelm profile export` / `import` |
...
Before distributions, sharing a Relayhelm agent meant sending someone:
...
```bash
relayhelm profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `relayhelm profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Relayhelm in 2026, that's already true.
...
:::caution
**Relayhelm does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `relayhelm profile install` or `relayhelm profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
relayhelm profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.relayhelm/profiles/research-bot/SOUL.md
...
Create `~/.relayhelm/profiles/research-bot/distribution.yaml`:
...
Create `~/.relayhelm/profiles/research-bot/.gitignore` with at minimum:
...
```bash
cd ~/.relayhelm/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
Recipients who run `relayhelm profile update research-bot` will pull the latest.
...
```bash
relayhelm profile install github.com/you/research-bot --alias
```
...
5. Copies distribution-owned files into `~/.relayhelm/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
relayhelm profile install github.com/you/research-bot

# Full HTTPS
relayhelm profile install https://github.com/you/research-bot.git

# SSH
relayhelm profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
relayhelm profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
relayhelm profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
relayhelm profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
relayhelm profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
relayhelm profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Relayhelm distribution.
...
```bash
cp ~/.relayhelm/profiles/research-bot/.env.EXAMPLE ~/.relayhelm/profiles/research-bot/.env
...
```bash
relayhelm profile info research-bot
```
...
`relayhelm profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
relayhelm profile update research-bot
```
...
```bash
relayhelm profile delete research-bot
```
...
Path:    ~/.relayhelm/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `relayhelm profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
cd ~/.relayhelm/profiles/pr-reviewer
# ... build and tune ...
git init && git add . && git status   # confirm no secrets staged
...
# Each engineer
relayhelm profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `relayhelm profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.relayhelm/profiles/polymarket-trader
...
# Anyone
relayhelm profile install github.com/you/hermes-polymarket-trader --alias
```
...
# Each on-call
relayhelm profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
relayhelm profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `relayhelm profile info <name>` and hold off on updates until you're ready.
:::
...
relayhelm profile info research-bot | grep Version
...
```yaml
# ~/.relayhelm/profiles/research-bot/local/my-overrides.yaml
...
relayhelm profile delete research-bot --yes
relayhelm profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
relayhelm profile update research-bot --force-config --yes
```
...
relayhelm profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.relayhelm/profiles/forked-research-bot/
...
relayhelm profile install ~/.relayhelm/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
relayhelm profile delete research-bot-test --yes
relayhelm profile install ~/.relayhelm/profiles/research-bot --name research-bot-test
```
...
Without `-o`, the CLI and TUI place the archive in Relayhelm's managed
`profile-exports/` directory under the default Relayhelm home, not in the current
...
source file. If the Relayhelm home itself lives inside a Git checkout (some
...
```bash
relayhelm profile export research-bot
relayhelm profile export research-bot -o ./research-bot.tar.gz
```
...
```bash
relayhelm profile import ./research-bot.tar.gz
relayhelm profile import ./research-bot.tar.gz --name research-bot-2
```

The profile name is inferred from the archive unless you pass `--name`. Importing over an existing profile is refused — rename or delete the old one first. A shell wrapper (`research-bot` → `relayhelm -p research-bot`) is created when the name doesn't collide with an existing command.
...
:::note
You cannot import as `default` — that name is the built-in root profile (`~/.relayhelm`). Pass `--name something-else`.
:::
...
**The default profile** (`~/.relayhelm`) is exported through an allow-list — only known Relayhelm artifacts, so an unrelated file sitting in your home directory can't get swept in:
...
**A named profile** (`~/.relayhelm/profiles/<name>`) copies the whole directory minus `auth.json` / `.env`. That's broader — if the profile has `state.db`, logs, or caches, they go in the archive too, and the file gets big.
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `relayhelm -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `relayhelm profile` — not a parallel command tree.
...
- [`relayhelm profile export` / `import`](../reference/profile-commands.md#hermes-profile-export) — the CLI form of [export files](#export-and-import-a-profile-file)
- [Slash Commands reference](../reference/slash-commands.md) — `/export`, `/import`, and every other in-chat command
- [Using SOUL with Relayhelm](../guides/use-soul-with-hermes.md) — authoring personalities
