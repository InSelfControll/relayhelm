...
# Relayhelm Web Dashboard

The web dashboard is a browser-based UI for managing your Relayhelm installation. Instead of editing YAML files or running CLI commands, you can configure settings, manage API keys, and monitor sessions from a clean web interface.

:::tip
Hosted-mode auth uses Nous Portal OAuth; if you also want the dashboard to talk to a real backend, `relayhelm setup --portal` wires up the model and tool gateway too. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
# Custom port
relayhelm dashboard --port 8080

# Bind to all interfaces (use with caution on shared networks)
relayhelm dashboard --host 0.0.0.0

# Start without opening browser
relayhelm dashboard --no-open
```
...
processes (manage them via `relayhelm -p <name> gateway …`), each profile's
session database, and cron schedulers (the Cron page already aggregates
across profiles with its own filter).

## Prerequisites

The default `relayhelm` install does not ship the HTTP stack or PTY helper — those are optional extras. The **web dashboard** needs FastAPI and Uvicorn (`web` extra). The **Chat** tab also needs `ptyprocess` to spawn the embedded TUI behind a pseudo-terminal (`pty` extra on POSIX). Install both with:

```bash
cd ~/.relayhelm/relayhelm && uv pip install -e ".[web,pty]"
```

The `web` extra pulls in FastAPI/Uvicorn; `pty` pulls in `ptyprocess` (POSIX) or `pywinpty` (native Windows — note that the embedded TUI itself still requires WSL). `cd ~/.relayhelm/relayhelm && uv pip install -e ".[all]"` includes both extras and is the easiest path if you also want messaging/voice/etc.

When you run `relayhelm dashboard` without the dependencies, it will tell you what to install. If the frontend hasn't been built yet and `npm` is available, it builds automatically on first launch.

The Chat tab is part of every `relayhelm dashboard` launch — the embedded browser chat pane (running the TUI over PTY/WebSocket) is always available, with no extra flag required.
...
- **Disk warnings** — the volume holding `~/.relayhelm` is nearly full
...
- Node.js (same requirement as `relayhelm --tui`; the TUI bundle is built on first launch)
- `ptyprocess` — installed by the `pty` extra (`cd ~/.relayhelm/relayhelm && uv pip install -e ".[web,pty]"`, or `[all]` covers both)
...
:::tip
Config changes take effect on the next agent session or gateway restart. The web dashboard edits the same `config.yaml` file that `relayhelm config set` and the gateway read from.
:::
...
Create and manage [profiles](../profiles.md) — isolated Relayhelm instances with their own config, skills, and sessions.
...
- **Set as active** — flips the sticky default that **future CLI/gateway runs** pick up (same as `relayhelm profile use`). This does *not* change what the dashboard manages — that's the profile switcher's job
- **Edit model / description / SOUL** — inline editors writing into that profile
- **Rename / Delete** — named profiles only

### Skills

Browse, search, and toggle installed skills and toolsets, and install new ones from the hub. Skills are loaded from `~/.relayhelm/skills/` and grouped by category.
...
- **Browse hub** — a third view searches the skill hub across all sources (the same as `relayhelm skills search`), installs any result by identifier with a live install log, and offers an "Update all" button to refresh installed skills.
...
Connect Relayhelm to any messaging platform from the browser — full parity with
`relayhelm setup gateway`. The page lists every supported channel (Telegram,
...
- **Restart gateway** — credentials are written to `~/.relayhelm/.env` and the enabled flag to `config.yaml`; the gateway connects each enabled channel on its next restart, which you can trigger right from the page.
...
- **Host** — live system stats: OS / kernel, architecture, hostname, Python and Relayhelm versions, CPU core count + utilization, memory, disk usage of the Relayhelm home, uptime, and load average. (CPU/memory/disk come from `psutil` when installed; identity fields are always shown.) The Relayhelm version shows an **update-status badge** (up to date / N commits behind) and a **Check for updates** button. When an update is available on a git install, an **Update now** button opens a confirmation dialog — showing how many commits you'll pull — before running `relayhelm update` in the background. On Docker/Nix installs the dashboard can't apply the update in place, so it shows the correct out-of-band command instead.
...
This re-reads `~/.relayhelm/.env` into the running process's environment. Useful when you've added a new provider key via the dashboard and want to use it immediately.
...
- **`disk`** — a live `shutil.disk_usage()` sample of the volume holding
  `~/.relayhelm`. Fields: `pressure`, `free_mb`, `total_mb`, `used_percent`,
...
When the dashboard is bound to a public or non-loopback address — anything other than `127.0.0.1` / `localhost` — Relayhelm engages an auth gate. Every request must carry a verified session cookie or it's bounced to the login page. Three providers ship in the box:
...
|-------|-----------|----------|
| `relayhelm dashboard` (default — binds to `127.0.0.1`) | OFF | Local development |
| `relayhelm dashboard --host 0.0.0.0` | **ON** | Remote / production — protect with the username/password provider or OAuth |
...
If the gate would engage but **no** `DashboardAuthProvider` is registered (no Nous plugin, no custom plugin), `relayhelm dashboard` refuses to bind with an explicit error message. There is no "default-deny but accept everything" fallback — a misconfigured gated dashboard never starts.

When you run `relayhelm dashboard --host 0.0.0.0` **interactively** (a real terminal) and no provider is configured yet, Relayhelm doesn't just fail — it offers to set one up on the spot: pick **username & password** (writes `dashboard.basic_auth` to `config.yaml` and you're running in seconds) or **OAuth** (points you at `relayhelm dashboard register`). Non-interactive callers — Docker/s6, CI, piped runs — skip the prompt and hit the fail-closed error above, so an unattended deploy still never starts without auth.
...
- **CLI — `relayhelm dashboard register`.** Run it on the host where the dashboard lives. It resolves your existing Nous login (run `relayhelm setup` first if you're not logged in), registers a self-hosted OAuth client with the Portal, and writes `HERMES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.relayhelm/.env` for you. Optional flags: `--name` (a human-readable label, otherwise auto-generated) and `--redirect-uri` (a public HTTPS callback URL for an internet-facing host).

  ```bash
  relayhelm dashboard register
  # ✓ Registered dashboard "swift_falcon"
  # …writes HERMES_DASHBOARD_OAUTH_CLIENT_ID to ~/.relayhelm/.env
  ```
...
|---------|-----------|--------|----------------|
| `HERMES_DASHBOARD_OAUTH_CLIENT_ID` | `dashboard.oauth.client_id` | `agent:{instance_id}` | `relayhelm dashboard register` |

Per the Relayhelm convention (`~/.relayhelm/.env` is for API keys / secrets only), **`config.yaml` is the recommended place to set these values** for local dev, on-prem, and any deployment you control directly. The environment-variable path exists so a hosting platform's secret injection can push per-deploy `client_id`s without anyone having to edit `config.yaml` inside the image — that's its primary purpose.
...
Configure an auth provider before exposing the dashboard:
  ...
  • OAuth: run `relayhelm dashboard register` (Nous Portal) or install a
...
From a logged-in Relayhelm install to a Nous-gated dashboard in three steps.

**1. Log in and register the dashboard.** `relayhelm dashboard register` uses your existing Nous login to provision an OAuth client and writes `HERMES_DASHBOARD_OAUTH_CLIENT_ID` into `~/.relayhelm/.env` for you:

```bash
relayhelm setup            # if you're not already logged into Nous Portal
relayhelm dashboard register
# ✓ Registered dashboard "swift_falcon"
# …writes HERMES_DASHBOARD_OAUTH_CLIENT_ID to ~/.relayhelm/.env
```
...
```bash
relayhelm dashboard --host 0.0.0.0 --port 9119 --no-open
```
...
**1. Set credentials in `~/.relayhelm/.env`.** Hash the password so no plaintext sits at rest, and set a stable signing secret so sessions survive restarts:
...
first; without one, Relayhelm fails closed at startup. This prevents the local SPA
...
Relayhelm always preserves loopback trust and rejects `*`, `0.0.0.0/0`, and
...
relayhelm dashboard --host 127.0.0.1 --port 9119 --no-open
```
...
```python
# ~/.relayhelm/plugins/dashboard-auth-myidp/__init__.py
...
Relayhelm Desktop can drive a Relayhelm backend running on another machine (a VPS, a home server, a Mini behind Tailscale). In the app this lives under **Settings → Gateways → Remote gateway**, which asks for a **Remote URL** and a way to **Sign in**. (For the desktop app itself — install, settings, chat — see the [Relayhelm Desktop](/user-guide/desktop) page.)

You protect the remote dashboard with one of the bundled auth providers, and the desktop app signs in against whichever one the backend advertises. For a backend reachable beyond your own machine — a VPS, a public host, anything internet-facing — the recommended provider is **OAuth (Nous Portal)** (register it with [`relayhelm dashboard register`](#registering-a-dashboard) and sign in with *Sign in with Nous Research*). The bundled [username/password provider](#usernamepassword-provider-no-oauth-idp) is the quickest option when the backend is on a trusted LAN or reachable only over a VPN, but is **not suitable for direct public-internet exposure**. Binding the dashboard to a non-loopback address engages its auth gate; once signed in, Desktop reuses the session for the chat WebSocket automatically — there is no token to copy or paste.
...
```bash
# 1. Set the dashboard login credentials in ~/.relayhelm/.env (secrets file, 0600).
cat >> ~/.relayhelm/.env <<'EOF'
HERMES_DASHBOARD_BASIC_AUTH_USERNAME=admin
HERMES_DASHBOARD_BASIC_AUTH_PASSWORD=choose-a-strong-password
...
relayhelm dashboard --no-open --host 0.0.0.0 --port 9119
```
...
If you run the dashboard as a systemd service, `~/.relayhelm/.env` is picked up automatically when the unit has `EnvironmentFile=%h/.relayhelm/.env`, so the credentials are in the environment at boot.
...
When you run `relayhelm update`, the web frontend is automatically rebuilt if `npm` is available. This keeps the dashboard in sync with code updates. If `npm` isn't installed, the update skips the frontend build and `relayhelm dashboard` will build it on first launch.
...
| Theme | Character |
|-------|-----------|
| **Relayhelm Teal** (`default`) | Dark teal + cream, system fonts, comfortable spacing |
| **Relayhelm Teal (Large)** (`default-large`) | Same as default with 18px text and roomier spacing |
