...
description: "Connect Relayhelm to external tool servers via MCP — and control exactly which MCP tools Relayhelm loads"
---

# MCP (Model Context Protocol)

MCP lets Relayhelm connect to external tool servers so the agent can use tools that live outside Relayhelm itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Relayhelm to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

:::tip Coming from Claude Code?
The `mcpServers` block in your `~/.claude.json` maps to `mcp_servers` in Relayhelm' `config.yaml` — and `relayhelm import-agent claude-code` migrates it (along with skills and instructions) automatically. See [Import from Other Agents](../import-from-other-agents.md).
:::

## What MCP gives you

- Access to external tool ecosystems without writing a native Relayhelm tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Relayhelm to see

## Quick start

1. MCP support ships with the standard install — no extra step needed.

2. Add an MCP server to `~/.relayhelm/config.yaml`:
...
4. Ask Relayhelm to use the MCP-backed capability.
...
Relayhelm will discover the MCP server's tools and use them like any other tool.

## Catalog: one-click install for Nous-approved MCPs

Relayhelm ships a curated catalog of MCP servers that Nous staff has reviewed
...
```
n8n          available              Manage and inspect n8n workflows from Relayhelm
...
`optional-mcps/` in the relayhelm repo — presence in that directory means
Nous approval. There is no community submission tier; entries are added by
merging a PR.

Catalog entries can require:

- **API key** — Relayhelm prompts at install time and writes the value to
  `~/.relayhelm/.env`. Non-secret values (base URLs) go to the same file.
- **OAuth** (remote MCP) — written as `auth: oauth` in your config; the MCP
  client opens a browser on first connection.
- **OAuth** (third-party provider like Google/GitHub) — Relayhelm points you at
  `relayhelm auth <provider>` if you haven't authenticated already.

### Tool selection at install time

After credentials are configured, Relayhelm probes the MCP server to list every
...
the relayhelm repo, so Nous has reviewed each entry before it shipped —
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/InSelfControll/relayhelm/tree/main/optional-mcps)
...
PR adds an entry with a newer `manifest_version` than your installed Relayhelm
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
Relayhelm`) for that entry instead of silently hiding it. Run `relayhelm update`
to install the latest Relayhelm when you see that.
...
from environment variables (which include everything in `~/.relayhelm/.env`).
...
after a Relayhelm update if a manifest version changed.

To add an MCP to the catalog, open a PR against
[`optional-mcps/`](https://github.com/InSelfControll/relayhelm/tree/main/optional-mcps).
...
rejected), and Relayhelm's bundled `github/*` skills driving the `gh` CLI are a
...
HTTP MCP servers are remote endpoints Relayhelm connects to directly.
...
- you do not want Relayhelm spawning a local subprocess for that integration

### OAuth-authenticated HTTP servers

Most hosted MCP servers (Cloudflare, Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Relayhelm handles discovery, client identification, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.

Relayhelm identifies itself with a [Client ID Metadata Document](../../reference/mcp-config-reference.md#client-identification-cimd-and-dcr) on servers that support one, and falls back to Dynamic Client Registration on those that don't. Both are automatic; there is nothing to configure.

:::tip Figma remote MCP
Figma's hosted endpoint (`https://mcp.figma.com/mcp`) allowlists Dynamic Client Registration by **exact `client_name`** — bare `"Relayhelm"` 403s, while `"Claude Code"` and `"Codex"` succeed. Relayhelm auto-sets `oauth.client_name: "Claude Code"` for `mcp.figma.com` so install/login works without a special trick:
...
On first connect, Relayhelm prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.relayhelm/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.

**Remote / headless hosts.** When Relayhelm runs on a different machine than your browser, the loopback callback can't reach your laptop. Ways to complete the flow:

- **Relayhelm Desktop (automatic):** when you run the OAuth sign-in from the Desktop app's MCP setup UI against a remote backend, Desktop hosts the callback listener on *your* machine and relays the authorization back to the gateway automatically — no tunnel, paste, or proxy needed. Requires both the Desktop app and the backend to be up to date.
- **Paste-back (no setup):** on an interactive terminal Relayhelm prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.
- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.
- **Proxied callback (`redirect_uri`):** when a public HTTPS endpoint forwards to the host (e.g. a Tailscale Funnel or reverse proxy pointed at the callback port), set `oauth.redirect_uri` and the browser redirect reaches Relayhelm on its own — no tunnel or paste needed:
...
For fully headless gateways (messaging bot, no interactive terminal at all), the optional [`mcp-oauth-remote-gateway` skill](../skills/optional/mcp/mcp-mcp-oauth-remote-gateway.md) walks the agent through completing the flow manually and writing tokens where Relayhelm expects them.
...
Then run `hermes mcp login googledrive` — with the pre-registered client, Relayhelm skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.relayhelm/config.yaml` from inside a running Relayhelm session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `hermes mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.

## mTLS / client certificates

Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. Relayhelm passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
- `value_from: profile` sends the active Relayhelm profile name, resolved once at connect time — useful when multiple profiles on one machine talk to the same server and it needs to tell them apart.
...
Relayhelm reads MCP config from `~/.relayhelm/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Relayhelm skips the server entirely |
...
## How Relayhelm registers MCP tools

Relayhelm prefixes MCP tools so they do not collide with built-in names:
...
- Relayhelm only registers resource utilities if the MCP session actually supports resource operations
- Relayhelm only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Relayhelm, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Relayhelm skips the server completely and does not even attempt a connection.
...
If your config filters out all callable tools and disables or omits all supported utilities, Relayhelm does not create an empty runtime MCP toolset for that server.
...
Relayhelm discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Relayhelm when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Relayhelm receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Relayhelm does not blindly pass your full shell environment.
...
Then verify your config and restart Relayhelm.
...
Because Relayhelm now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Relayhelm may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Relayhelm via the `sampling/createMessage` protocol. This allows an MCP server to ask Relayhelm to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Relayhelm as an MCP server

In addition to connecting **to** MCP servers, Relayhelm can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Relayhelm's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Relayhelm
- You want a single MCP server that bridges to all of Relayhelm's connected messaging platforms at once
- You already have a running Relayhelm gateway with connected platforms
...
Add Relayhelm to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Relayhelm in a specific location:

```json
{
  "mcpServers": {
    "hermes": {
      "command": "/home/user/.relayhelm/relayhelm/venv/bin/relayhelm",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server includes a live event bridge that polls Relayhelm's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
