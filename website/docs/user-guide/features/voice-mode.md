...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Relayhelm — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Relayhelm supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Relayhelm](../../guides/use-voice-mode-with-hermes.md).
...
1. **Relayhelm installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `relayhelm model` or set your preferred provider credentials in `~/.relayhelm/.env`
3. **A working base setup** — run `hermes` to verify the agent responds to text before enabling voice

:::tip
The `~/.relayhelm/` directory and default `config.yaml` are created automatically the first time you run `hermes`. You only need to create `~/.relayhelm/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `relayhelm setup --portal` wires both up at once.
:::
...
cd ~/.relayhelm/relayhelm && uv pip install -e ".[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
cd ~/.relayhelm/relayhelm && uv pip install -e ".[messaging]"

# Premium TTS (ElevenLabs)
cd ~/.relayhelm/relayhelm && uv pip install -e ".[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
cd ~/.relayhelm/relayhelm && uv pip install -e ".[all]"
```
...
Voice mode is available in both the **classic CLI** (`relayhelm chat`) and the **TUI** (`relayhelm --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.relayhelm/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
:::tip
The record key is configurable via `voice.record_key` in `~/.relayhelm/config.yaml` (default: `ctrl+b`).
:::
...
When Relayhelm Desktop is connected to a **remote gateway**, audio does not need to be relayed through the gateway at all. At voice-session start the desktop fetches the active profile's resolved STT/TTS settings (provider, model, language/voice, and credential) from the gateway over the authenticated REST channel (`GET /api/audio/voice-config`) and then calls the providers **directly**:
...
```bash
relayhelm gateway        # Start the gateway (connects to configured platforms)
relayhelm gateway setup  # Interactive setup wizard for first-time configuration
```
...
:::tip
To disable the mention requirement in server channels, add to `~/.relayhelm/.env`:
```bash
...
```bash
relayhelm gateway        # Start with existing configuration
```
...
If you are running Relayhelm inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.relayhelm/.env`
...
- Review gateway logs: `tail -f ~/.relayhelm/logs/gateway.log`
