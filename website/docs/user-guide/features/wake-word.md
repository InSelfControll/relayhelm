...
sidebar_position: 11
title: "Wake Word"
description: "Hands-free 'Hey Relayhelm' wake word — start a voice session by speaking, the 'Hey Siri' way"
---

# Wake Word ("Hey Relayhelm")

The wake word turns Relayhelm into a hands-free assistant across the CLI, TUI, and
desktop app: with one setting on, Relayhelm listens in the background for a spoken
trigger phrase. Say it, and Relayhelm starts a fresh session, opens the microphone,
...
When the desktop app connects to a **remote** Relayhelm backend (for example a
...
Relayhelm supports **client capture** for that case:
...
By default the phrase is **"hey hermes"** — a model for it ships with Relayhelm, so
...
```bash
cd ~/.relayhelm/relayhelm && uv pip install -e ".[wake]"
```
...
desktop ear button — also writes `wake_word.enabled` to `~/.relayhelm/config.yaml`,
...
default) to let Relayhelm choose per platform: **tflite on Apple Silicon**, onnx
...
fire. The tflite backend needs `ai-edge-litert` on macOS, which Relayhelm installs
on demand alongside the other wake-word deps.

### Surfaces (CLI, TUI, GUI)

The wake word works in all three Relayhelm surfaces, and `surface` picks which one
...
| `tui` | Only `relayhelm --tui`. |
| `gui` | Only the desktop app. |

The detector is on-device and single-mic, so only one surface listens at a time,
including when Relayhelm surfaces run in separate processes. Ownership is sticky:
the first eligible claimant keeps the listener until it stops, disconnects, or
its process exits. Relayhelm does not silently fail over to another open surface.
...
"Hey Relayhelm" works out of the box — the bundled openWakeWord model
...
command (`relayhelm -p <profile>`) instead of routing.
...
wake_word:
  ...
  phrase: "computer"
  openwakeword:
    model: ~/.relayhelm/wakewords/computer.onnx   # or a built-in name like hey_jarvis
```
...
Create a "Hey Relayhelm" keyword in the [Picovoice Console](https://console.picovoice.ai/),
...
wake_word:
  ...
  phrase: "hey hermes"
  porcupine:
    keyword: ~/.relayhelm/wakewords/hey_hermes.ppn
```

Set your access key in `~/.relayhelm/.env`:
...
- A TTS provider for speaking the reply (the default `edge-tts` works with no
  ...
  both STT and TTS are ready — `relayhelm tools` (Voice section) sets them up.
- The wake engine deps (auto-installed, or `relayhelm[wake]`).
...
but the phrase never fires. Relayhelm detects this (`/wake status` shows
"mic delivers only silence"; the desktop ear tooltip carries the same hint).
Fix: System Settings → Privacy & Security → Microphone → enable the Relayhelm
backend (it may appear as your terminal, `python`, or Relayhelm), then toggle the
...
```bash
relayhelm config set wake_word.input_device "Microphone Array"
```

Use `null` to return to the process default:

```bash
relayhelm config set wake_word.input_device null
```
