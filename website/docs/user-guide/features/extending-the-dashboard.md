...
description: "Build themes and plugins for the Relayhelm web dashboard — palettes, typography, layouts, custom tabs, shell slots, page-scoped slots, and backend API routes"
---

# Extending the Dashboard

The Relayhelm web dashboard (`relayhelm dashboard`) is built to be reskinned and extended without forking the codebase. Three layers are exposed:

1. **Themes** — YAML files that repaint the dashboard's palette, typography, layout, and per-component chrome. Drop a file in `~/.relayhelm/dashboard-themes/`; it appears in the theme switcher.
...
Themes are YAML files stored in `~/.relayhelm/dashboard-themes/`. The file name doesn't matter (the theme's `name:` field is what the system uses), but convention is `<name>.yaml`. Every field is optional — missing keys fall back to the built-in `default` theme, so a theme can be as small as one color.

### Quick start — your first theme

```bash
mkdir -p ~/.relayhelm/dashboard-themes
```

```yaml
# ~/.relayhelm/dashboard-themes/neon.yaml
name: neon
label: Neon
...
|-------|---------|------------|--------|
| **Relayhelm Teal** (`default`) | Dark teal + cream | System stack, 15px | 0.5rem radius, comfortable |
| **Relayhelm Teal (Large)** (`default-large`) | Same as default | System stack, 18px, line-height 1.65 | 0.5rem radius, spacious |
...
Themes that reference Google Fonts (all except Relayhelm Teal) load the stylesheet on demand — the first time you switch to them a `<link>` tag is injected into `<head>`.
...
```yaml
# ~/.relayhelm/dashboard-themes/ocean.yaml
name: ocean
label: Ocean Deep
...
A dashboard plugin is a directory with a `manifest.json`, a pre-built JS bundle, and optionally a CSS file and a Python file with FastAPI routes. Plugins live next to other Relayhelm plugins in `~/.relayhelm/plugins/<name>/` — the dashboard extension is a `dashboard/` subfolder inside that plugin directory, so one plugin can extend both the CLI/gateway and the dashboard from a single install.
...
```bash
mkdir -p ~/.relayhelm/plugins/my-plugin/dashboard/dist
```

Write the manifest:

```json
// ~/.relayhelm/plugins/my-plugin/dashboard/manifest.json
{
  "name": "my-plugin",
  "label": "My Plugin",
  "icon": "Sparkles",
...
```javascript
// ~/.relayhelm/plugins/my-plugin/dashboard/dist/index.js
(function () {
  "use strict";
...
// Relayhelm API client + raw fetcher
...
#### Calling built-in Relayhelm endpoints
...
| `header-left` | Before the Relayhelm brand in the top bar. |
...
```json
// ~/.relayhelm/plugins/session-notes/dashboard/manifest.json
{
  "name": "session-notes",
  "label": "Session Notes",
...
```javascript
// ~/.relayhelm/plugins/session-notes/dashboard/dist/index.js
(function () {
  const SDK = window.__HERMES_PLUGIN_SDK__;
  const { React } = SDK;
...
```python
# ~/.relayhelm/plugins/my-plugin/dashboard/plugin_api.py
...
#### Accessing Relayhelm internals

Backend routes run inside the dashboard process, so they can import from the relayhelm codebase directly:
...
|----------|-----------|--------------|
| 1 (wins on conflict) | `~/.relayhelm/plugins/<name>/dashboard/` | `user` |
| 2 | `<repo>/plugins/memory/<name>/dashboard/` | `bundled` |
| 2 | `<repo>/plugins/<name>/dashboard/` | `bundled` |
| 3 | `./.relayhelm/plugins/<name>/dashboard/` | `project` — only when `HERMES_ENABLE_PROJECT_PLUGINS` is set |
...
…or restart `relayhelm dashboard`.
...
cp hermes-example-plugins/strike-freedom-cockpit/theme/strike-freedom.yaml \
   ~/.relayhelm/dashboard-themes/

# Plugin
cp -r hermes-example-plugins/strike-freedom-cockpit ~/.relayhelm/plugins/
```

Open the dashboard, pick **Strike Freedom** from the theme switcher. The cockpit sidebar appears, the crest shows in the header, the tagline replaces the footer. Switch back to **Relayhelm Teal** and the plugin remains installed but invisible (the `sidebar` slot only renders under the `cockpit` layout variant).
...
Check that the file is in `~/.relayhelm/dashboard-themes/` and ends in `.yaml` or `.yml`. Refresh the page. Run `curl http://127.0.0.1:9119/api/dashboard/themes` — your theme should be in the response. If the YAML has a parse error, the dashboard logs to `errors.log` under `~/.relayhelm/logs/`.

**My plugin's tab doesn't show up.**
1. Check the manifest is at `~/.relayhelm/plugins/<name>/dashboard/manifest.json` (note the `dashboard/` subdirectory).
...
2. Restart `relayhelm dashboard` — plugin API routes are mounted once at startup, **not** on rescan.
3. Check that `plugin_api.py` exports a module-level `router = APIRouter()`. Other export names are not picked up.
4. Tail `~/.relayhelm/logs/errors.log` for `Failed to load plugin <name> API routes` — import errors are logged there.
...
Dashboard plugins are installed by directory layout, not by pip entry point. The cleanest distribution path today is a git repo the user clones into `~/.relayhelm/plugins/`. A pip-based installer for dashboard plugins is not currently wired up.
