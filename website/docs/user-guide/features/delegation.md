...
Top-level model calls run in the background automatically. Relayhelm returns a handle immediately so the conversation can continue, then posts the result back as a new message. An orchestrator subagent waits for its own workers so it can synthesize their results before returning.
...
When a top-level agent provides a `tasks` array, Relayhelm returns one background handle, runs the subagents in parallel, and posts one consolidated result after every child finishes. An orchestrator subagent waits for its batch in the current turn so it can synthesize the results.
...
When a background delegation finishes, Relayhelm stores its completion event in
the active profile's `state.db` before publishing it to the normal fresh-turn
queue. If Relayhelm restarts after completion but before delivery, the pending
...
Relayhelm cannot prove whether its external side effects happened. Pending and
...
```bash
tail -f ~/.relayhelm/cache/delegation/live/deleg_ab12cd34/task-0.log
```
...
- A Relayhelm process restart does **not** resume a running child. Its attempt becomes `unknown` because Relayhelm cannot prove which side effects happened.
