...
When using [OpenRouter](https://openrouter.ai) or [Nous Portal](/integrations/nous-portal) as your LLM provider, Relayhelm supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.relayhelm/config.yaml`:
...
- **CLI mode** — configured in `~/.relayhelm/config.yaml`, loaded at startup
