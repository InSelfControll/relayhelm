...
Relayhelm ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
relayhelm memory setup      # interactive picker + configuration
relayhelm memory status     # check what's active
relayhelm memory off        # disable external provider
```

You can also select the active memory provider via `relayhelm plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.relayhelm/config.yaml`:
...
When a memory provider is active, Relayhelm automatically:
...
**Setup Wizard:**
```bash
relayhelm memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `hermes honcho setup` command still works (it now redirects to `relayhelm memory setup`), but is only registered after Honcho is selected as the active memory provider.
...
**Config:** `$HERMES_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$HERMES_HOME/honcho.json` > `~/.relayhelm/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/InSelfControll/relayhelm/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Relayhelm profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Relayhelm profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Relayhelm profile. Host key `hermes` → default; `hermes.<profile>` for others. |
...
```bash
relayhelm profile create coder --clone
```
...
Scans every Relayhelm profile, creates host blocks for any profile without one, inherits settings from the default `hermes` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `relayhelm memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/InSelfControll/relayhelm/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/hermes).
...
relayhelm memory setup    # select "openviking"
# Or manually:
relayhelm config set memory.provider openviking
```

`relayhelm memory setup` can reuse or copy connection values from
`~/.openviking/ovcli.conf`. Manual setup uses the active profile's `.env` file;
for the default profile that is `~/.relayhelm/.env`, and for named profiles use
`~/.relayhelm/profiles/<profile>/.env`.
...
Peer identity is optional. By default, Relayhelm sends no peer ID and writes
...
Relayhelm sends `User-Agent: openviking-memory-hermes/<version>` on OpenViking
...
**Setup (Platform):**
```bash
relayhelm memory setup    # select "mem0" → "Platform"
# Or manually:
relayhelm config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.relayhelm/.env
```

**Setup (OSS):**
```bash
relayhelm memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
relayhelm memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
relayhelm memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
relayhelm memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
relayhelm memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.relayhelm/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.relayhelm/.env
```
...
**Config:** `$HERMES_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.relayhelm/.env`.
...
**Switching modes:** Re-run `relayhelm memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
relayhelm memory setup    # select "hindsight"
# Or manually:
relayhelm config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.relayhelm/.env
```
...
| `retain_context` | `conversation between Relayhelm and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/InSelfControll/relayhelm/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
relayhelm memory setup    # select "holographic"
# Or manually:
relayhelm config set memory.provider holographic
```
...
**Setup:**
```bash
relayhelm memory setup    # select "retaindb"
# Or manually:
relayhelm config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.relayhelm/.env
```
...
# Then configure Relayhelm
relayhelm memory setup    # select "byterover"
# Or manually:
relayhelm config set memory.provider byterover
```
...
**Setup:**
```bash
relayhelm memory setup    # select "supermemory"
# Or manually:
relayhelm config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.relayhelm/.env
```
...
Before running `relayhelm memory setup`, set `base_url` in
...
Then run `relayhelm memory setup` and enter the API key printed by the local
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `hermes-{identity}` → `hermes-coder`) to isolate memories per Relayhelm profile
...
relayhelm config set memory.provider memori
relayhelm memory setup
```
