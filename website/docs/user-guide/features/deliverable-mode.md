...
When Relayhelm runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.relayhelm/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.relayhelm/config.yaml` (switchable per session
via `/personality`).
...
If you use Relayhelm' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.relayhelm/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Relayhelm's deliverable
