...
description: "Durable SQLite-backed task board for coordinating multiple Relayhelm profiles"
---
...
Relayhelm Kanban is a durable task board, shared across all your Relayhelm profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.relayhelm/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.relayhelm/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_request_review`, `kanban_request_changes`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_attach`, `kanban_attach_url`, `kanban_attachments`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `relayhelm kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `relayhelm kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
called `default` (DB at `~/.relayhelm/kanban.db` for back-compat). Users who
only want one stream of work never need to know about boards; the feature
is opt-in.

Per-board isolation is absolute:

- Separate SQLite DB per board (`~/.relayhelm/kanban/boards/<slug>/kanban.db`).
...
relayhelm kanban boards list

# Create a new board.
relayhelm kanban boards create atm10-server \
    --name "ATM10 Server" \
...
relayhelm kanban --board atm10-server list
relayhelm kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
relayhelm kanban boards switch atm10-server
relayhelm kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
relayhelm kanban boards rename atm10-server "ATM10 (Prod)"
...
3. `~/.relayhelm/kanban/current` — the slug persisted by `relayhelm kanban
   boards switch`.
4. `default`.
...
`relayhelm dashboard` → Kanban tab shows a board switcher at the top as soon
...
relayhelm kanban create "research AI funding landscape" --assignee researcher

# 4. Watch activity live (you)
relayhelm kanban watch

# 5. See the board (you)
relayhelm kanban list
relayhelm kanban stats
```

When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `relayhelm kanban show t_abcd`.
...
for debugging. Standard gateway supervision applies: run `relayhelm gateway
...
until one comes up — `relayhelm kanban create` warns about this at creation
time.

Running `relayhelm kanban daemon` as a separate process is **deprecated**;
...
relayhelm kanban create "nightly ops review" \
    --assignee ops \
...
```bash
relayhelm kanban complete t_abc t_def t_hij --result "batch wrap"
relayhelm kanban archive  t_abc t_def t_hij
relayhelm kanban unblock  t_abc t_def
relayhelm kanban block    t_abc "need input" --ids t_def t_hij
```
...
**Workers do not shell out to `relayhelm kanban`.** When the dispatcher spawns a worker it sets `HERMES_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `relayhelm kanban` CLI.
...
### Why tools instead of shelling to `relayhelm kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `relayhelm kanban complete` *inside* the container, where `hermes` isn't installed and `~/.relayhelm/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.relayhelm/kanban.db` regardless of terminal backend.
...
**Agent-side prevention:** Before the worker exits, Relayhelm injects up to two
...
```bash
relayhelm kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

relayhelm kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `relayhelm skills list` to see what's available); there's no runtime install.
...
```bash
# At creation
relayhelm kanban create "hard refactor" --assignee coder \
    --model claude-opus-4.6 --provider anthropic

# Or later — takes effect on the next dispatch
relayhelm kanban set-model t_abcd claude-opus-4.6 --provider anthropic
relayhelm kanban set-model t_abcd none    # clear the override
```
...
Kanban's per-profile configs make the planner/worker cost split natural. Decomposing a project into well-scoped cards takes frontier-level judgment; executing a card that already carries a clear goal, context, and handoff evidence usually doesn't — and the workers are where the vast majority of tokens are spent, so the worker model is where the cost lives. Run your orchestrator/dispatcher profile on a frontier model and point worker profiles at inexpensive models. Each profile has its own `config.yaml` under `~/.relayhelm/profiles/<name>/`, and the dispatcher injects the profile-scoped `HERMES_HOME` when it spawns `relayhelm -p <assignee>`, so each worker reads its own profile's model settings:

```yaml
# ~/.relayhelm/config.yaml (orchestrator / dispatcher profile)
model:
  default: "your-frontier-model"

# ~/.relayhelm/profiles/coder/config.yaml (worker profile)
model:
  default: "your-inexpensive-model"

# ~/.relayhelm/profiles/researcher/config.yaml (another worker profile)
model:
  default: "your-inexpensive-model"
```

For the occasional quality-sensitive card, pin just that task back to a stronger model with the [per-task model override](#per-task-model-override) (`--model`/`--provider` at create time, `relayhelm kanban set-model` later, or the dashboard's model dropdown) — no profile edits needed.
...
```bash
relayhelm kanban create "Translate the docs site to French" \
...
Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `relayhelm kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `relayhelm profile create --description "..."`, `relayhelm profile describe <name> --text "..."`, `relayhelm profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
Config knobs (all under `kanban:` in `~/.relayhelm/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `relayhelm profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.relayhelm/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
The plugin uses the standard Relayhelm dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
relayhelm kanban init                                     # create kanban.db + print daemon hint
relayhelm kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
relayhelm kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
relayhelm kanban show <id> [--json]
relayhelm kanban assign <id> <profile>                    # or 'none' to unassign
relayhelm kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
relayhelm kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
relayhelm kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
relayhelm kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
relayhelm kanban diagnostics [--json]                     # board health snapshot (alias: diag)
relayhelm kanban link <parent_id> <child_id>
relayhelm kanban unlink <parent_id> <child_id>
relayhelm kanban claim <id> [--ttl SECONDS]
relayhelm kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
relayhelm kanban complete <id>... [--result "..."]
relayhelm kanban block <id> "<reason>" [--ids <id>...]
relayhelm kanban unblock <id>...
relayhelm kanban archive <id>...

relayhelm kanban request-review <id> [--summary "..."] [--metadata JSON] [--reviewer PROFILE]
relayhelm kanban request-changes <id> "<required changes>"               # active reviewer -> implementer
relayhelm kanban reopen-review  <id>... [--reason "..."]                 # changes requested: 'review' -> ready/todo

relayhelm kanban tail <id>                                # follow a single task's event stream
relayhelm kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
relayhelm kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
relayhelm kanban runs <id> [--json]                       # attempt history (one row per run)
relayhelm kanban assignees [--json]                       # profiles on disk + per-assignee task counts
relayhelm kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
relayhelm kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `relayhelm gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
relayhelm kanban stats [--json]                           # per-status + per-assignee counts
relayhelm kanban log <id> [--tail BYTES]                  # worker log from ~/.relayhelm/kanban/logs/
relayhelm kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
        [--chat-type dm|group|channel|thread] [--delivery-mode notify|notify+wake|wake]
relayhelm kanban notify-list [<id>] [--json]
relayhelm kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
relayhelm kanban context <id>                             # what a worker sees
relayhelm kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
relayhelm kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
relayhelm kanban create "nightly backup audit" \
...
Every `relayhelm kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `relayhelm chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `hermes_cli.kanban.run_slash()` entry point that reuses the `relayhelm kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `relayhelm kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.relayhelm/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`relayhelm kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `relayhelm kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |
...
relayhelm kanban create "Fix CI failure from t_impl: test_retry flakes on 3.11" \
    --assignee coder \
    --parent t_impl \
    --body "$(cat <<'EOF'
...
[`agent-merge-conflict-arbiter` optional skill](https://github.com/InSelfControll/relayhelm/blob/main/optional-skills/autonomous-ai-agents/agent-merge-conflict-arbiter/SKILL.md)
...
```bash
relayhelm kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
...
```bash
relayhelm kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7 \
    --chat-type group --delivery-mode notify+wake
relayhelm kanban notify-list
relayhelm kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
guide](https://github.com/InSelfControll/relayhelm/blob/main/docs/kanban/multi-gateway.md)),
...
```bash
relayhelm kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
relayhelm kanban runs t_abcd
...
**Bulk close caveat.** `relayhelm kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`relayhelm kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `relayhelm kanban heartbeat $TASK` to signal liveness during long operations. |
...
`relayhelm kanban tail <id>` shows these for a single task. `relayhelm kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.relayhelm/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.
