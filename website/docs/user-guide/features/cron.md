...
Schedule tasks to run automatically with natural language or cron expressions. Relayhelm exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Relayhelm itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
**Which model does a cron job run on?** Resolution at fire time is: per-job pin → `cron.model` in `config.yaml` → the global default from `relayhelm model`.

- **Per-job pin** — set by *you* via the dashboard, `relayhelm cron create/edit --model … --provider …`, or by editing `~/.relayhelm/cron/jobs.json`. Once set, it sticks until you change it. The agent's `cronjob` tool cannot set or change per-job models — inference pins are user-owned.
- **`cron.model` / `cron.model_provider`** — a cron-fleet default: every unpinned job runs on this model, independent of your chat model. Set it once (`relayhelm config set cron.model <name>`) and switching your chat model with `relayhelm model` or `/model` never touches your cron fleet.
- **Global default** — only when neither of the above is set does a job follow `relayhelm model`. In this case Relayhelm **snapshots** the provider and model at creation, and if the global default later changes the job **fails closed**: it skips the run, makes no inference call, and alerts you **once** — the job stays skipped (and silent) on subsequent ticks until you act or the config is restored (#44585). For recurring or otherwise repeatable jobs, pin the provider/model explicitly (`relayhelm cron edit <job_id> --provider <provider> --model <model>`) to proceed. A consumed finite one-shot cannot be updated; create a new future one-shot with an explicit provider and model instead. This prevents an unattended job from silently inheriting a switch to a paid provider/model. Setting `cron.model` (or a per-job pin) is the deliberate way to route cron spend, and the drift guard does not engage for an axis covered by it. Operators who instead want unpinned jobs to track the changing global default can [disable the drift guard](#letting-unpinned-jobs-track-global-defaults).
...
`relayhelm setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
**Per-job reasoning effort.** A job can pin its own thinking level, independent of the model pin: one of `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`, `ultra`. When set, it overrides both the global `agent.reasoning_effort` and per-model `agent.reasoning_overrides` for that job's runs (`none` disables thinking). Set it via `relayhelm cron create/edit --reasoning-effort high`; pass an empty string on edit to clear the pin and follow config again. (It is deliberately not exposed on the agent's `cronjob` tool — model configuration stays a user decision.) Levels a model doesn't support are clamped or omitted by the provider at request time — pinning `xhigh` on a model that caps at `high` runs at `high`. The pin has no effect on `no_agent` jobs (there is no LLM call to tune). Use it to run heavy scheduled analyses at `high` while cheap recurring jobs run at `minimal`, without touching your global default.
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Relayhelm disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
relayhelm cron create "every 2h" "Check server status"
relayhelm cron create "every 1h" "Summarize new feed items" --skill blogwatcher
relayhelm cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Ask Relayhelm normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

Relayhelm will use the unified `cronjob` tool internally.
...
Or: `relayhelm config set cron.preflight false`
...
```bash
relayhelm config set cron.model_drift_guard false
```
...
relayhelm cron create "every 1d at 09:00" \
...
```bash
relayhelm cron edit <job_id> --schedule "every 4h"
relayhelm cron edit <job_id> --prompt "Use the revised task"
relayhelm cron edit <job_id> --skill blogwatcher --skill maps
relayhelm cron edit <job_id> --add-skill maps
relayhelm cron edit <job_id> --remove-skill blogwatcher
relayhelm cron edit <job_id> --clear-skills
```
...
```bash
relayhelm cron list
relayhelm cron pause <job_id_or_name>
relayhelm cron resume <job_id_or_name>
relayhelm cron run <job_id_or_name>
relayhelm cron remove <job_id_or_name>
relayhelm cron edit <job_id_or_name> [...flags]
relayhelm cron status
relayhelm cron tick
```
...
```bash
relayhelm gateway install     # Install as a user service
sudo relayhelm gateway install --system   # Linux: boot-time system service for servers
relayhelm gateway             # Or run in foreground
...
On each tick Relayhelm:

1. loads jobs from `~/.relayhelm/cron/jobs.json`
...
A file lock at `~/.relayhelm/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.

### Execution history

Relayhelm records each claimed cron attempt in the profile-local
`~/.relayhelm/cron/executions.db` before executor or provider dispatch. Attempts
move through `claimed`, `running`, and one immutable terminal state:
`completed`, `failed`, or `unknown`. After restart, Relayhelm marks an abandoned
...
Inspect recent attempts with `relayhelm cron runs [job-id] --limit 20` (alias:
...
in a row and suggesting you fix, pause (`relayhelm cron pause <job>`), or remove
it. Any successful run resets the streak, and `relayhelm cron list` shows the
...
```bash
relayhelm cron incidents                 # list incidents (newest activity first)
relayhelm cron incidents --state alerted # filter: detected | alerted | closed
relayhelm cron incidents ack <id>        # acknowledge — stop re-pinging
```
...
### Fleet health check: `relayhelm cron doctor`

`relayhelm cron doctor` is a read-only health check over every active job. It
...
`relayhelm cron incidents` (durable failure records) and `relayhelm cron runs`
...
| `"local"` | Save to local files only (`~/.relayhelm/cron/output/`) | Default on CLI |
...
`last_delivery_error`. `relayhelm cron list` shows it in yellow as
`delivery_failed: <reason>`, `relayhelm cron doctor` reports it as a delivery
...
- `bot-chat:<profile>` targets another profile **on the same machine**. Names are validated against `relayhelm profile list` when the job is created; profiles on other gateways or machines can never be targeted, so same-named profiles across machines are unambiguous.
...
in `relayhelm cron list`:

```
⚠ Delivery UNVERIFIED: adapter acked slack:C0123456 without message_id/raw_response
```

and in `relayhelm cron doctor` as `last delivery unverified (...)`. The marker is
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.relayhelm/cron/output/`), but no message is sent to the delivery target.
...
```bash
relayhelm cron create "every 5m" \
  --no-agent \
...
The `cronjob` tool's schema exposes `no_agent` to Relayhelm directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Relayhelm will write the check script to `~/.relayhelm/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.relayhelm/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.relayhelm/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.relayhelm/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.relayhelm/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Relayhelm reads Job 1's most recent output from `~/.relayhelm/cron/output/{job1_id}/*.md`
...
From the CLI: `relayhelm cron create "every 6h" "Scan for news" --continuity`, and `relayhelm cron edit <job_id> --continuity` / `--no-continuity` to toggle it on an existing job. The same toggle appears in the dashboard's cron editor and the desktop Bot Mode routine dialog.
...
- `relayhelm cron list` — a red `⚠ Missed scheduled fire:` line under the job
- The dashboard job view

The stamp always reflects **current** auto-fire health: it is overwritten by newer misses and cleared automatically by the next successful run. If you see it, the job and its schedule are fine — the gateway side of the fire path needs attention (most commonly, restart the gateway through its supervisor so it loads the full profile environment: `relayhelm gateway restart`).
...
Runtimes that can't receive detached results (one-shot `relayhelm -z`, `hermes
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `relayhelm tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `relayhelm tools` cron-platform config wins; otherwise Relayhelm falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Relayhelm should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.relayhelm/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.relayhelm/scripts/.feed-changed.last"
...
```bash
#!/bin/bash
# ~/.relayhelm/scripts/flag-ready.sh
...
:::tip
Relayhelm's own `~/.relayhelm/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/InSelfControll/relayhelm/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.relayhelm/cron/jobs.json`. Output from job runs is saved to `~/.relayhelm/cron/output/{job_id}/{timestamp}.md`.

Job definitions are plain JSON on disk: they survive `relayhelm update`, gateway restarts, and machine reboots. A job that was mid-run during a restart is marked `unknown` in the execution ledger — it is not automatically retried, but the job's next scheduled tick fires normally. See [Execution history](#execution-history) for details.

:::tip
Ask the agent to manage jobs through the `cronjob` tool, `relayhelm cron edit`, or `/cron` — not by patching `jobs.json` directly. Direct edits can fail silently when [file write safety](../security.md#file-write-safety) blocks the path (for example when `HERMES_WRITE_SAFE_ROOT` is set), and the [file-mutation verifier](../configuration.md#file-mutation-verifier) footer is the authoritative signal that nothing was saved.
:::

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Relayhelm resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
