...
title: "Plugins"
description: "Extend Relayhelm with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Relayhelm has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Relayhelm
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Relayhelm Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.relayhelm/plugins/` with a `plugin.yaml` and Python code:

```
~/.relayhelm/plugins/my-plugin/
...
Start Relayhelm — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.relayhelm/plugins/hello-world/plugin.yaml`**
...
**`~/.relayhelm/plugins/hello-world/__init__.py`**

```python
"""Minimal Relayhelm plugin — registers a tool and a hook."""
...
Drop both files into `~/.relayhelm/plugins/hello-world/`, restart Relayhelm, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

The model-facing tool description belongs in `schema["description"]`. The optional `ctx.register_tool(description=...)` value is separate `ToolEntry` registry metadata: when omitted, it defaults to the schema description, but Relayhelm does not copy it back into a schema that lacks `description`. Prefer defining the text once in the schema. If you provide both values, keep them synchronized; the model sees the schema value.

Project-local plugins under `./.relayhelm/plugins/` are disabled by default. Enable them only for trusted repositories by setting `HERMES_ENABLE_PROJECT_PLUGINS=true` before starting Relayhelm.
...
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `relayhelm plugins install` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Relayhelm — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.relayhelm/plugins/` | Personal plugins |
| Project | `.relayhelm/plugins/` | Project-specific plugins (requires `HERMES_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `hermes_agent.plugins` entry_points | Distributed packages |
| Nix | `services.relayhelm.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Relayhelm also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.relayhelm/plugins/model-providers/<name>/` and `~/.relayhelm/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `relayhelm plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.relayhelm/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
relayhelm plugins                    # interactive toggle (space to check/uncheck)
relayhelm plugins enable <name>      # add to allow-list
relayhelm plugins disable <name>     # remove from allow-list + add to disabled
```

After `relayhelm plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

For a reproducible install, pin a full immutable commit (tags, branches, and
abbreviated SHAs are not accepted):

```bash
relayhelm plugins install owner/repo --ref 0123456789abcdef0123456789abcdef01234567
```

Relayhelm checks out the commit detached, verifies that `HEAD` exactly matches the
requested SHA, and records the canonical source, installed revision, and pin
status in the current profile. `relayhelm plugins update` refuses to move a pinned
plugin; choose a new exact commit explicitly with
`relayhelm plugins install <source> --force --ref <new-commit>`. The
...
Several categories of plugin bypass `plugins.enabled` — they're part of Relayhelm' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.relayhelm/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.relayhelm/plugins/`.

### Approval transports

An approval transport changes **where a human sees and answers** an existing
Relayhelm tool-approval request. It does not decide whether a command needs
...
`present` may be synchronous or async. Relayhelm runs it on a bounded worker and
...
`transport_fallback: builtin`. Without that exact opt-in, Relayhelm never
materializes the prompt on another surface.

Relayhelm still owns hardline blocks, sudo-stdin protection, user deny rules,
...
Relayhelm has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.relayhelm/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Relayhelm also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Relayhelm Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Relayhelm Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Relayhelm Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
...
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Relayhelm auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `relayhelm skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.relayhelm/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `relayhelm plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
relayhelm plugins                               # unified interactive UI
relayhelm plugins list                          # table: enabled / disabled / not enabled
relayhelm plugins search <term>                 # search the community plugin index
relayhelm plugins install <name>                # install by index name (resolved to repo @ pinned ref)
relayhelm plugins install user/repo             # install from Git, then prompt Enable? [y/N]
relayhelm plugins install user/repo --enable    # install AND enable (no prompt)
relayhelm plugins install user/repo --no-enable # install but leave disabled (no prompt)
relayhelm plugins update my-plugin              # pull latest (local edits are autostashed and re-applied)
relayhelm plugins remove my-plugin              # uninstall
relayhelm plugins enable my-plugin              # add to allow-list
relayhelm plugins disable my-plugin             # remove from allow-list + add to disabled
relayhelm plugins capabilities [my-plugin]      # declared vs granted capabilities
```

### One-click install links (Desktop)

Relayhelm Desktop registers the `hermes://` URL scheme, so a website, README, or
...
Clicking one opens Relayhelm and shows a **confirmation dialog** — the repo id,
...
[Add to Relayhelm link](/reference/mcp-config-reference#add-to-hermes-link).
...
When a plugin declares capabilities, `relayhelm plugins install` (and
`relayhelm plugins enable`) shows the list with one-line risk descriptions and
...
granted, `relayhelm plugins update` surfaces the additions and asks again. New
...
`relayhelm plugins enable <id>` interactively to grant them later.

Inspect the state at any time:

```bash
relayhelm plugins capabilities             # all plugins with declared/granted capabilities
relayhelm plugins capabilities my-plugin   # one plugin, declared vs granted
```
...
not a code audit, and Relayhelm has not reviewed the plugin's code. Only install
...
`relayhelm plugins search <term>` searches the **community plugin index** — a
static, machine-readable JSON catalog of community plugins. Matching is fuzzy
across name, description, and tags:

```bash
relayhelm plugins search telegram               # fuzzy search
relayhelm plugins search                        # browse the whole index
relayhelm plugins search --capability platform  # filter by declared capability
relayhelm plugins search media --json           # machine-readable output
relayhelm plugins search --refresh              # bypass the 24h local cache
```
...
```bash
relayhelm plugins install hermes-media-studio
```
...
overridable via `relayhelm config set plugins.index_url <url>`). Fetches are
cached under `~/.relayhelm/cache/plugin_index.json` for 24 hours; when the
remote is unreachable the stale cache is used, and when there is no cache at
all a bundled seed copy ships with Relayhelm — so search works fully offline.
...
```bash
relayhelm plugins pack show ./hermes-pack.yaml     # dry-run review
relayhelm plugins pack install ./hermes-pack.yaml  # review → confirm → install
relayhelm plugins pack export > hermes-pack.yaml   # snapshot the current install
relayhelm plugins pack export --enabled-only       # only plugins.enabled
```
...
same pinned install path as `relayhelm plugins install --ref <sha>` and record
...
capability-consent prompt — identical to a single `relayhelm plugins install`.
...
provenance (installed via `relayhelm plugins install`). Local-only plugins are
...
auto-installed — install those manually for now (`relayhelm skills`). Wiring
skill-hub ids into pack install is a documented follow-up seam.

### Install-time security scanning

Every `relayhelm plugins install` and `relayhelm plugins update` runs a static
...
On `relayhelm plugins update`, a dangerous verdict on the updated tree
...
Running `relayhelm plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `relayhelm plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
- Relayhelm reuses that session's stored platform, chat, thread, profile, and conversation history. Plugins cannot supply a new chat route through this API.
- Relayhelm rechecks the stored route against the gateway's current authorisation rules before dispatch.
- Routes that relied only on an adapter-time or upstream authorisation decision are rejected unless Relayhelm can revalidate them from current core allowlists, pairing, or explicit allow-all configuration.
...
- The route and conversation are pinned while dispatch is pending. Relayhelm drops the request if topic recovery changes the route or the session rotates before handling starts.
...
:::warning
Only grant gateway injection to plugins you trust. Relayhelm checks this host API permission and restricts it to existing session routes, but Python plugins run in-process and this setting is not a sandbox.
:::
...
`ctx.call_mcp()` lets a plugin call a tool on one of the user's configured MCP servers — synchronously, from any hook or tool handler — routing through Relayhelm' existing native MCP client (same connections, trust-tier gates, circuit breaker, and reconnect logic as model-invoked MCP tools; never a parallel client).
