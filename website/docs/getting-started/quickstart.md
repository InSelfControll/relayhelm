---
sidebar_position: 1
title: "Relayhelm Quickstart"
description: "Your first conversation with Relayhelm — from install to chatting in under 5 minutes"
---

# Relayhelm Quickstart

This guide gets you from zero to a working Relayhelm setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Relayhelm Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Relayhelm Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Relayhelm for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Relayhelm working on my machine | `relayhelm setup` | Run a real chat and verify it responds |
| I already know my provider | `relayhelm model` | Save the config, then start chatting |
| I want a bot or always-on setup | `relayhelm gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `relayhelm model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `relayhelm model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Relayhelm cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Relayhelm
### With the Relayhelm Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Relayhelm Desktop installer](https://hermes-agent.nousresearch.com/) from our website and run it.

### Without Relayhelm Desktop:
For a command-line only install without Relayhelm Desktop, run:
...
The single most important setup step. Use `relayhelm model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `relayhelm setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `relayhelm update`. Re-enable anything later with `relayhelm tools`, seed skills with `relayhelm skills opt-in --sync`, or tune settings with `relayhelm setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `relayhelm model` |
| **OpenAI Codex** | ChatGPT or Codex subscription, uses Codex models | Device code auth via `relayhelm model` → **ChatGPT or Codex Subscription** |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `relayhelm model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hermes_cli/models.py` may change between releases) | `relayhelm model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `relayhelm model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `relayhelm model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `relayhelm model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `relayhelm model` (requires `copilot` CLI + `copilot login`) |
...
Relayhelm requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `relayhelm model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Relayhelm separates secrets from normal config:

- **Secrets and tokens** → `~/.relayhelm/.env`
- **Non-secret settings** → `~/.relayhelm/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
relayhelm config set model anthropic/claude-opus-4.6
relayhelm config set terminal.backend docker
relayhelm config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
hermes            # classic CLI
relayhelm --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Relayhelm ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `hermes` vs `relayhelm --tui`.
:::
...
- Relayhelm replies without error
...
```bash
relayhelm --continue    # Resume the most recent session
relayhelm -c            # Short form
```
...
```bash
relayhelm gateway setup    # Interactive platform configuration
```
...
- `relayhelm tools` — tune tool access per platform
- `relayhelm skills` — browse and install reusable workflows
...
```bash
relayhelm config set terminal.backend docker    # Docker isolation
relayhelm config set terminal.backend ssh       # Remote server
```
...
```bash
# From the Relayhelm install directory (the curl installer placed it at
# ~/.relayhelm/relayhelm on Linux/macOS or %LOCALAPPDATA%\hermes\relayhelm on Windows):
...
Skills are on-demand instruction documents that teach Relayhelm how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Relayhelm ships with a catalog of bundled skills already installed in `~/.relayhelm/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
relayhelm skills browse                      # list everything available
relayhelm skills search kubernetes           # find skills by keyword
relayhelm skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `relayhelm skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Relayhelm ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.relayhelm/relayhelm && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Relayhelm opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `relayhelm model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `relayhelm gateway setup` and check `relayhelm gateway status` |
| `relayhelm --continue` can't find old session | Switched profiles or session never saved | Check `hermes sessions list` and confirm you're in the right profile |
...
| `relayhelm doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
5. `relayhelm --continue`
6. `relayhelm gateway status`
...
| `relayhelm model` | Choose your LLM provider and model |
| `relayhelm tools` | Configure which tools are enabled per platform |
| `relayhelm setup` | Full setup wizard (configures everything at once) |
| `relayhelm doctor` | Diagnose issues |
| `relayhelm update` | Update to latest version |
| `relayhelm gateway` | Start the messaging gateway |
| `relayhelm --continue` | Resume last session |
...
- **[Moving to another machine](/reference/faq#exporting-hermes-to-another-machine)** — `relayhelm backup` migrates your whole setup (or [a single profile](/reference/faq#moving-a-single-profile-to-another-machine)); no need to rebuild from scratch
