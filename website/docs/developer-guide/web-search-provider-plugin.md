...
description: "How to build a web-search/extract/crawl backend plugin for Relayhelm"
---
...
:::tip
Web search is one of several **backend plugins** Relayhelm supports. The others (with their own ABCs) are [Image Generation Provider Plugins](/developer-guide/image-gen-provider-plugin), [Video Generation Provider Plugins](/developer-guide/video-gen-provider-plugin), [Memory Provider Plugins](/developer-guide/memory-provider-plugin), [Context Engine Plugins](/developer-guide/context-engine-plugin), and [Model Provider Plugins](/developer-guide/model-provider-plugin). General tool/hook/CLI plugins live in [Build a Relayhelm Plugin](/developer-guide/plugins).
:::

## How discovery works

Relayhelm scans for web-search backends in three places:

1. **Bundled** — `<repo>/plugins/web/<name>/` (auto-loaded with `kind: backend`, always available)
2. **User** — `~/.relayhelm/plugins/web/<name>/` (opt-in via `plugins.enabled` or `relayhelm plugins enable <name>`)
...
When neither key is set, Relayhelm auto-detects the backend from whichever API key/URL is present in the environment. `relayhelm tools` walks users through selection.
...
class MyBackendWebSearchProvider(WebSearchProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `relayhelm tools`. Defaults to `name`.
        return "My Backend"

    def is_available(self) -> bool:
        # Cheap check — env var present, optional dep importable, etc.
        # MUST NOT make network calls (runs on every `relayhelm tools` paint).
...
| `provides_web_providers` | List of provider `name`s this plugin registers — used by the loader to advertise the plugin in `relayhelm tools` even before `register()` runs |
| `requires_env` | Interactive credential prompt during `relayhelm plugins install` (see [Build a Relayhelm Plugin](/developer-guide/plugins#gate-on-environment-variables) for the rich format) |
...
| `display_name` | — | `name` | Label shown in `relayhelm tools` |
...
Relayhelm routes calls to the right provider based on the `supports_*` flags. A common multi-provider setup:
...
When `web.search_backend` or `web.extract_backend` aren't set, both fall through to `web.backend`. When that's also unset, Relayhelm picks the first available provider that supports the requested capability based on env-var presence.
...
## How Relayhelm wires it into the tools
...
Errors surface as the tool result; the LLM decides how to explain them. If no provider is registered (or every available one fails the capability gate), the tool returns a helpful error pointing at `relayhelm tools`.

## Lazy-installing optional dependencies

If your provider wraps a third-party SDK (like DDGS does with the `ddgs` package), don't `import` it at module top level. Use `tools.lazy_deps.ensure(...)` inside `is_available()` or `search()` — Relayhelm will install the package on first use, gated by `security.allow_lazy_installs`. See [Build a Relayhelm Plugin → Lazy-install](/developer-guide/plugins#lazy-install-optional-python-dependencies) for the security model.
...
- [Build a Relayhelm Plugin](/developer-guide/plugins) — general tools/hooks/slash commands guide
