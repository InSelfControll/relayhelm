...
description: "How to build a memory provider plugin for Relayhelm"
---

# Building a Memory Provider Plugin

Memory provider plugins give Relayhelm persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `relayhelm plugins`.
:::

## Installation Layouts

Relayhelm discovers memory providers from four sources, in this precedence order:

| Source | Location | Notes |
|---|---|---|
| Bundled | `plugins/memory/<name>/` | Ships with Relayhelm. Closed to new providers — see [CONTRIBUTING](https://github.com/InSelfControll/relayhelm/blob/main/CONTRIBUTING.md). |
| User | `$HERMES_HOME/plugins/<name>/` | Dropped in by the user, per profile. |
| Project | `./.relayhelm/plugins/<name>/` | Opt-in via `HERMES_ENABLE_PROJECT_PLUGINS=1`. |
...
Relayhelm, in `$HERMES_HOME/plugins/<name>/` when installed by a user, or in
`./.relayhelm/plugins/<name>/` for a project-local one:
...
two files Relayhelm reads from disk rather than importing — `config_schema.py`
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `relayhelm memory setup` | **Yes** |
...
The gate binds to every compaction authority, not just the Relayhelm
...
required checkpoint cannot be guaranteed there. The checkpoint-aware Relayhelm
...
`get_config_schema()` returns a list of field descriptors used by `relayhelm memory setup`:
...
the `messages` parameter; Relayhelm will continue calling them with the legacy
signature.
...
All storage paths **must** use the `hermes_home` kwarg from `initialize()`, not hardcoded `~/.relayhelm`:
...
data_dir = Path("~/.relayhelm/my-provider").expanduser()
```
...
**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `relayhelm --help`.
