...
The ACP adapter wraps Relayhelm' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
relayhelm acp / hermes-acp / python -m acp_adapter
...
- `allow_once` -> Relayhelm `once`
- `allow_always` -> Relayhelm `always`
- reject options -> Relayhelm `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Relayhelm tools to ACP tool kinds and builds editor-facing content.
...
Instead it reuses Relayhelm' runtime resolver:

- `acp_adapter/auth.py`
- `hermes_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Relayhelm provider/credentials. It also always advertises a terminal setup auth method (`hermes-setup`, args `--setup`) so first-run ACP clients can open Relayhelm' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.relayhelm/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `hermes_cli/main.py` — `relayhelm acp` CLI subcommand
