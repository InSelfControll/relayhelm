...
description: "How to build an image-generation backend plugin for Relayhelm"
---
...
:::tip
Image-gen is one of several **backend plugins** Relayhelm supports. The others (with more specialized ABCs) are [Memory Provider Plugins](/developer-guide/memory-provider-plugin), [Context Engine Plugins](/developer-guide/context-engine-plugin), and [Model Provider Plugins](/developer-guide/model-provider-plugin). General tool/hook/CLI plugins live in [Build a Relayhelm Plugin](/developer-guide/plugins).
:::

## How discovery works

Relayhelm scans for image-gen backends in three places:

1. **Bundled** — `<repo>/plugins/image_gen/<name>/` (auto-loaded with `kind: backend`, always available)
2. **User** — `~/.relayhelm/plugins/image_gen/<name>/` (opt-in via `plugins.enabled`)
3. **Pip** — packages declaring a `hermes_agent.plugins` entry point

Each plugin's `register(ctx)` function calls `ctx.register_image_gen_provider(...)` — that puts it into the registry in `agent/image_gen_registry.py`. The active provider is picked by `image_gen.provider` in `config.yaml`; `relayhelm tools` walks users through selection.

The `image_generate` tool wrapper asks the registry for the active provider and dispatches there. If no provider is registered, the tool surfaces a helpful error pointing at `relayhelm tools`.
...
A bundled plugin is complete at this point. User plugins at `~/.relayhelm/plugins/image_gen/<name>/` need to be added to `plugins.enabled` in `config.yaml` (or run `relayhelm plugins enable <name>`).
...
class MyBackendImageGenProvider(ImageGenProvider):
    ...
    def display_name(self) -> str:
        # Human label shown in `relayhelm tools`. Defaults to name.title() if omitted.
        return "My Backend"
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        # Catalog shown in `relayhelm tools` model picker.
        return [
            {
                "id": "my-model-fast",
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        # Metadata for the `relayhelm tools` picker — keys to prompt for at setup.
        return {
            "name": "My Backend",
...
`kind: backend` is what routes the plugin to the image-gen registration path. `requires_env` is prompted during `relayhelm plugins install`.
...
| `display_name` | — | `name.title()` | Label shown in `relayhelm tools` |
| `is_available()` | — | `True` | Gate for missing creds/deps |
| `list_models()` | — | `[]` | Catalog for `relayhelm tools` model picker |
...
Drop a user plugin at `~/.relayhelm/plugins/image_gen/<name>/` with the same `name` property as a bundled one and enable it via `relayhelm plugins enable <name>` — the registry is last-writer-wins, so your version replaces the built-in. Useful for pointing an `openai` plugin at a private proxy, or swapping in a custom model catalog.
...
relayhelm plugins enable my-backend
...
# Exercise it
relayhelm -z "Generate an image of a corgi in a spacesuit"
```

Or interactively: `relayhelm tools` → "Image Generation" → select `my-backend` → enter API key if prompted.
