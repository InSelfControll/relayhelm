...
Relayhelm runs shell commands through a pluggable set of **terminal backends**.
...
installed under `~/.relayhelm/plugins/`, registering a backend the user selects
...
| `relayhelm setup` backend picker | `display_name`, `description`, `setup_instructions()`, `post_setup()` |
| Dashboard terminal-backend picker (probe status) | `probe()` |
| `relayhelm status` / `relayhelm doctor` | `doctor_checks()` |
...
```python title="~/.relayhelm/plugins/acmebox/__init__.py"
...
class AcmeBoxProvider(TerminalEnvironmentProvider):
    ...
    def cache_path_base(self):
        return "~/.relayhelm"    # where synced cache files land, or None
...
```yaml title="~/.relayhelm/plugins/acmebox/plugin.yaml"
name: acmebox
version: 0.1.0
...
```bash
relayhelm plugins enable acmebox
relayhelm config set terminal.backend acmebox
```
