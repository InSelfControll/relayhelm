...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Relayhelm already has applies. The plugin never sees a
...
relayhelm itself). For the async surface (`acomplete()` /
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Relayhelm provider id (e.g. "openrouter")
...
using Relayhelm built-in auxiliary tasks; it does not permit another plugin's
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Relayhelm provider
        # (must be one Relayhelm already knows about — same names as
        # `relayhelm model` and config.yaml model.provider).
        ...
        # plugin sends — Relayhelm does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.relayhelm/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Relayhelm' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Relayhelm subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/InSelfControll/relayhelm/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/InSelfControll/relayhelm/blob/main/tests/agent/test_plugin_llm.py)
