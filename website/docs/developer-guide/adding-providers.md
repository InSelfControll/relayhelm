...
description: "How to add a new inference provider to Relayhelm — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Relayhelm can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `relayhelm model` menu entries
...
:::tip
`hermes_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `relayhelm setup`.
:::
...
7. `relayhelm model` menu includes the provider
8. `relayhelm setup` wizard delegates to `main.py` automatically
...
- Provider-specific `relayhelm model` menu entries with bespoke auth flows
...
- What env vars should Relayhelm check, and in what priority order?
...
Be careful with API-key precedence. Relayhelm already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `hermes_cli/main.py`

A provider is not discoverable until it shows up in the interactive `relayhelm model` flow.
...
:::tip
`hermes_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `relayhelm model` and `relayhelm setup` automatically.
:::
...
When you add a native provider, double-check that Relayhelm is only sending fields that provider actually understands.
...
### 7. Updating `relayhelm model` but not `relayhelm setup`
