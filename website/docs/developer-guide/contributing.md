...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Relayhelm — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Relayhelm! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Relayhelm core? Start with [Build a Relayhelm Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Relayhelm itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated Relayhelm instance (throwaway HERMES_HOME, separate Electron
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.relayhelm/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.relayhelm` — use `get_hermes_home()` from `hermes_constants` for code paths and `display_hermes_home()` for user-facing messages. See [AGENTS.md](https://github.com/InSelfControll/relayhelm/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
Projects built on (or reviewed by) Relayhelm can keep reviewer checklists inside the repository under `.agents/checks/`. Each file is a focused, plain-markdown checklist that an agent loads before reviewing a change touching the matching area:
...
When you ask Relayhelm to review a PR in a repository that has `.agents/checks/`, tell it (or teach it via a skill) to read the relevant checklists first and report against them. This gives review agents the project-specific bar that generic review prompts miss.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/InSelfControll/relayhelm/blob/main/LICENSE).
