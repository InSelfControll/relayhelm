...
description: "Build wrapper CLIs that extend the Relayhelm TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

Relayhelm exposes protected extension hooks on `HermesCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the `run()` method or the TUI construction in `hermes_cli/cli_tui_mixin.py` (where these hooks are defined; `HermesCLI` in `cli.py` mixes it in). This keeps your extension decoupled from internal changes.
...
```python
#!/usr/bin/env python3
"""my_cli.py — Example wrapper CLI that extends Relayhelm."""
...
Called after Relayhelm registers its own keybindings and before the layout is built. Add your keybindings to `kb`.
