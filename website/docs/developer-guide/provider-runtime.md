...
description: "How Relayhelm resolves providers, credentials, API modes, and auxiliary models at runtime"
---

# Provider Runtime Resolution

Relayhelm has a shared provider runtime resolver used across:
...
That ordering matters because Relayhelm treats the saved model/provider choice as the source of truth for normal runs. This prevents a stale shell export from silently overriding the endpoint a user last selected in `relayhelm model`.
...
This resolver is the main reason Relayhelm can share auth/runtime logic between:
...
Set `AI_GATEWAY_API_KEY` in `~/.relayhelm/.env` and run with `--provider ai-gateway`. Relayhelm fetches available models from the gateway's `/models` endpoint, filtering to language models with tool-use support.

## OpenRouter, AI Gateway, and custom OpenAI-compatible base URLs

Relayhelm contains logic to avoid leaking the wrong API key to a custom endpoint when multiple provider keys exist (e.g. `OPENROUTER_API_KEY`, `AI_GATEWAY_API_KEY`, and `OPENAI_API_KEY`).
...
Relayhelm also distinguishes between:
...
When provider resolution selects `anthropic`, Relayhelm uses:
...
- Relayhelm preflights Anthropic credential refresh before native Messages API calls
- Relayhelm still retries once on a 401 after rebuilding the Anthropic client, as a fallback path
...
When an auxiliary task is configured with provider `main`, Relayhelm resolves that through the same shared runtime path as normal chat. In practice that means:

- env-driven custom endpoints still work
- custom endpoints saved via `relayhelm model` / `config.yaml` also work
- auxiliary routing can tell the difference between a real saved custom endpoint and the OpenRouter fallback

## Fallback models

Relayhelm supports a configured fallback provider chain — a list of `(provider, model)` entries tried in order when the primary model encounters errors. The legacy single-pair `fallback_model` dict is still accepted for back-compat (and migrated on first write).
