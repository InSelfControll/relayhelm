...
Plugins can launch and supervise fresh Relayhelm child sessions without importing
...
active agent turn fails closed with `No active Relayhelm parent session`.
...
overrides, and per-launch timeouts are explicitly rejected until Relayhelm can
support them without weakening isolation. Use `allowed_toolsets` to narrow a
child; Relayhelm's existing unsafe-tool block remains enforced.
