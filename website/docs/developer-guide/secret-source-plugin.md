...
description: "How to build a secret-manager backend plugin for Relayhelm"
---

# Building a Secret Source Plugin

Secret sources resolve provider credentials from an external secret manager (a vault, a password manager, an OS keystore, a custom script) into environment variables at process startup — after `~/.relayhelm/.env` loads, before Relayhelm reads credentials. Bitwarden, 1Password, and a generic command-helper source ship in-tree; **every other backend is a plugin**. This guide covers building one.
...
Relayhelm then re-pulls secrets after plugin discovery when any **enabled**
...
Subclass the kit from the Relayhelm repo (`tests/secret_sources/conformance.py`) in your plugin's tests:
