...
Relayhelm 使用 SQLite 数据库（`~/.relayhelm/state.db`）跨 CLI 和 gateway 会话持久化会话元数据、完整消息历史及模型配置。这替代了早期的逐会话 JSONL 文件方案。
...
```
~/.relayhelm/state.db (SQLite, WAL mode)
...
db = SessionDB()                           # 默认：~/.relayhelm/state.db
...
默认路径：`~/.relayhelm/state.db`

该路径由 `hermes_constants.get_hermes_home()` 推导，默认解析为 `~/.relayhelm/`，或 `HERMES_HOME` 环境变量的值。
