...
本指南介绍如何向 Relayhelm gateway 添加新的消息平台。平台适配器将 Relayhelm 连接到外部消息服务（Telegram、Discord、WeCom 等），使用户可以通过该服务与 agent 交互。

:::tip
添加平台有两种方式：
- **Plugin**（推荐用于社区/第三方）：将 plugin 目录放入 `~/.relayhelm/plugins/` — 无需修改任何核心代码。参见下方 [Plugin 路径](#plugin-path-recommended)。
...
Plugin 系统允许你在不修改任何 Relayhelm 核心代码的情况下添加平台适配器。你的 plugin 是一个包含两个文件的目录：
...
Plugin 元数据。`requires_env` 和 `optional_env` 块会自动填充 `relayhelm config` UI 条目（参见下方[在 relayhelm config 中暴露环境变量](#surfacing-env-vars-in-hermes-config)）。
...
def register(ctx):
    """Plugin 入口点 — 由 Relayhelm plugin 系统调用。"""
...
| `relayhelm config` UI 条目 | `plugin.yaml` 中的 `requires_env` / `optional_env` 自动填充 |
...
| PII 脱敏 | `pii_safe` 标志 |
| `relayhelm status` | 显示带 `(plugin)` 标签的 plugin 平台 |
| `relayhelm gateway setup` | Plugin 平台出现在设置菜单中 |
| `relayhelm tools` / `relayhelm skills` | Plugin 平台出现在每平台配置中 |
| Token 锁（多配置文件） | 在 `connect()` 中使用 `acquire_scoped_lock()` |
| 孤立配置警告 | Plugin 缺失时输出描述性日志 |

## 环境变量驱动的自动配置

大多数用户通过将环境变量写入 `~/.relayhelm/.env` 来配置平台，而不是编辑 `config.yaml`。`env_enablement_fn` hook 允许你的 plugin 在适配器构建**之前**读取这些环境变量，使 `relayhelm gateway status`、`get_connected_platforms()` 和 cron 投递无需实例化平台 SDK 即可看到正确状态。
...
`cron_deliver_env_var` 使你的平台成为可识别的 `deliver=` 目标。要在 cron 任务运行于独立进程（即 `relayhelm cron run` 与 `relayhelm gateway` 分离）时使实际发送成功，需注册 `standalone_sender_fn`：
...
## 在 `relayhelm config` 中暴露环境变量 {#surfacing-env-vars-in-hermes-config}
...
:::note
此清单用于将平台直接添加到 Relayhelm 核心代码库 — 通常由核心贡献者为官方支持的平台执行。社区/第三方平台应使用上方的 [Plugin 路径](#plugin-path-recommended)。
:::
...
2. **`toolsets.py`** — 将 `"hermes-newplat"` 添加到 `"relayhelm-gateway"` 的 includes 列表
