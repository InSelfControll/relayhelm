...
description: "使用 Nix 安装和部署 Relayhelm——从快速 `nix run` 到完全声明式的 NixOS 模块（含容器模式）"
---

# Nix & NixOS 安装配置

Relayhelm 提供了一个 Nix flake，支持三个层级的集成：
...
**对于非 NixOS 用户**，这只影响安装步骤。之后的操作（`relayhelm setup`、`relayhelm gateway install`、编辑配置）与标准安装完全相同。
...
nix run github:InSelfControll/relayhelm -- chat
...
执行 `nix profile install` 后，`hermes`、`relayhelm` 和 `hermes-acp` 将出现在你的 PATH 中。之后的工作流与[标准安装](./installation.md)完全相同——`relayhelm setup` 引导你完成提供商选择，`relayhelm gateway install` 设置 launchd（macOS）或 systemd 用户服务，配置存放在 `~/.relayhelm/`。
...
设置 `addToSystemPackages = true` 有两个作用：将 `hermes` CLI 添加到系统 PATH，**并**在系统范围内设置 `HERMES_HOME`，使交互式 CLI 与 gateway 服务共享状态（会话、技能、cron）。不设置此项时，在 shell 中运行 `hermes` 会创建独立的 `~/.relayhelm/` 目录。
:::
...
- 路由是透明的：`relayhelm chat`、`hermes sessions list`、`relayhelm --version` 等命令都会在底层 exec 进容器
- 所有 CLI 参数原样转发
...
设置 `container.hostUsers` 可创建 `~/.relayhelm` 到服务状态目录的符号链接，使主机 CLI 和容器共享会话、配置和记忆：
...
CLI 会自动检测何时需要 sudo 并透明地使用它。没有此配置，你需要手动运行 `sudo relayhelm chat`。
:::
...
:::note 模型命名
`settings.model.default` 使用你的提供商所期望的模型标识符。使用 [OpenRouter](https://openrouter.ai)（默认）时，格式如 `"anthropic/claude-sonnet-4"` 或 `"google/gemini-3-flash"`。如果直接使用提供商（Anthropic、OpenAI），请将 `settings.model.base_url` 指向其 API，并使用其原生模型 ID（例如 `"claude-sonnet-4-20250514"`）。未设置 `base_url` 时，Relayhelm 默认使用 OpenRouter。
:::
...
| 给 Agent 设置个性 | `${services.relayhelm.stateDir}/.relayhelm/SOUL.md` | 直接管理该文件 |
...
| 覆盖 hermes 包 | `package` | `inputs.relayhelm.packages.${system}.default.override { ... }` |
...
`environment`（非密钥变量）和 `environmentFiles`（密钥文件）在激活时（`nixos-rebuild switch`）都会合并到 `$HERMES_HOME/.env` 中。Relayhelm 在每次启动时读取此文件，因此更改在 `systemctl restart relayhelm` 后生效——无需重建容器。
...
`documents` 选项将文件安装到 Agent 的工作目录（即 `workingDirectory`，Agent 将其作为工作区读取）。Relayhelm 按约定查找特定文件名：

- **`USER.md`** — 关于 Agent 正在交互的用户的上下文信息。
- 你放置在此处的任何其他文件对 Agent 都可见，作为工作区文件。

Agent 身份文件是独立的：Relayhelm 从 `$HERMES_HOME/SOUL.md` 加载其主要 `SOUL.md`，在 NixOS 模块中对应 `${services.relayhelm.stateDir}/.relayhelm/SOUL.md`。将 `SOUL.md` 放入 `documents` 只会创建一个工作区文件，不会替换主角色文件。

```nix
{
  services.relayhelm.documents = {
...
对于使用 OAuth 2.1 的服务器，设置 `auth = "oauth"`。Relayhelm 实现了完整的 PKCE 流程——元数据发现、动态客户端注册、token 交换和自动刷新。
...
首次 OAuth 授权需要基于浏览器的同意流程。在无头部署中，Relayhelm 将授权 URL 打印到 stdout/日志，而不是打开浏览器。
...
| 被屏蔽的命令 | 原因 |
|---|---|
| `relayhelm setup` | 配置是声明式的——请在 Nix 配置中编辑 `settings` |
| `relayhelm config edit` | 配置由 `settings` 生成 |
| `relayhelm config set <key> <value>` | 配置由 `settings` 生成 |
| `relayhelm gateway install` | systemd 服务由 NixOS 管理 |
| `relayhelm gateway uninstall` | systemd 服务由 NixOS 管理 |

这可以防止 Nix 声明的内容与磁盘上实际内容之间产生漂移。检测使用两个信号：

1. **`HERMES_MANAGED=true`** 环境变量——由 systemd 服务设置，对 gateway 进程可见
2. **`.managed` 标记文件**，位于 `HERMES_HOME` 中——由激活脚本设置，对交互式 shell 可见（例如 `docker exec -it relayhelm relayhelm config set ...` 也会被屏蔽）
...
~/.relayhelm -> /var/lib/hermes/.relayhelm       （符号链接桥接，按 hostUsers）
...
Nix 构建的二进制文件能在 Ubuntu 容器内运行，是因为 `/nix/store` 被绑定挂载——它携带自己的解释器和所有依赖，不依赖容器的系统库。容器入口点通过 `current-package` 符号链接解析：`/data/current-package/bin/relayhelm gateway run --replace`。执行 `nixos-rebuild switch` 时，只更新符号链接——容器继续运行。

### 各事件的持久性

| 事件 | 容器重建？ | `/data`（状态） | `/home/hermes` | 可写层（`apt`/`pip`/`npm`） |
|---|---|---|---|---|
| `systemctl restart relayhelm` | 否 | 保留 | 保留 | 保留 |
...
NixOS 模块支持声明式插件安装——无需命令式的 `relayhelm plugins install`。
...
插件在激活时以符号链接方式安装到 `$HERMES_HOME/plugins/`。Relayhelm 通过其正常的目录扫描发现它们。从列表中移除插件并运行 `nixos-rebuild switch` 会删除符号链接。
...
对于已在 relayhelm 的 `pyproject.toml` 中声明的可选 extras（例如 `hindsight` 或 `honcho` 等记忆提供商），使用 `extraDependencyGroups` 在构建时将其包含到封闭的 venv 中：
...
| 检查 | 测试内容 |
|---|---|
| `package-contents` | `hermes` 和 `relayhelm` 二进制文件存在且 `relayhelm --version` 可运行 |
| `entry-points-sync` | `pyproject.toml` 中 `[project.scripts]` 的每个条目在 Nix 包中都有对应的封装二进制文件 |
| `cli-commands` | `relayhelm --help` 暴露 `gateway` 和 `config` 子命令 |
| `managed-guard` | `HERMES_MANAGED=true relayhelm config set ...` 打印 NixOS 错误 |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `enable` | `bool` | `false` | 启用 relayhelm 服务 |
| `package` | `package` | `relayhelm` | 使用的 relayhelm 包 |
...
| 选项 | 类型 | 默认值 | 描述 |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | `relayhelm gateway` 的额外参数 |
...
| `container.hostUsers` | `listOf str` | `[]` | 获得 `~/.relayhelm` 符号链接（指向服务 stateDir）的交互式用户，自动加入 `hermes` 组 |
...
| 容器路径 | 主机路径 | 模式 | 说明 |
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Relayhelm 二进制文件 + 所有 Nix 依赖 |
...
| `relayhelm --version` 显示旧版本 | 容器未重启 | `systemctl restart relayhelm` |
...
| `no container with name or ID "relayhelm"`（Podman） | Podman rootful 容器对普通用户不可见 | 为 podman 添加免密 sudo（参见[容器模式](#container-mode)章节） |
| `unable to find user hermes` | 容器仍在启动中（入口点尚未创建用户） | 等待几秒后重试——CLI 会自动重试 |
| 通过 `extraPackages` 添加的工具在终端中找不到 | 需要 `nixos-rebuild switch` 更新每用户 profile | 重建并重启：`nixos-rebuild switch && systemctl restart relayhelm` |