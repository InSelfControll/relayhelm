...
Relayhelm 自动将每次对话保存为一个 session。Session 支持对话恢复、跨 session 搜索以及完整的对话历史管理。

## Session 的工作原理

每次对话——无论来自 CLI、Telegram、Discord、Slack、WhatsApp、Signal、Matrix、Teams 还是其他任何消息平台——都会以完整消息历史的形式存储为一个 session。Session 记录在：

1. **SQLite 数据库**（`~/.relayhelm/state.db`）——包含 FTS5 全文搜索的结构化 session 元数据，以及完整消息历史
...
Relayhelm 存储 session 历史以便恢复对话，但不会在每次对话时重新发送所有历史字节。每轮对话中，模型看到的是：所选系统 prompt、当前对话窗口，以及 Relayhelm 为该轮显式注入的内容。
...
例如，如果用户发送一张图片并要求 Relayhelm 制作表情包，Relayhelm 可能会用视觉能力检查该图片一次并运行图像处理脚本。后续轮次不会自动将原始 JPEG 带入上下文，只携带写入对话的内容，例如用户的请求、简短的图片描述、本地缓存路径或最终的助手回复。
...
| 来源 | 描述 |
|--------|-------------|
| `cli` | 交互式 CLI（`hermes` 或 `relayhelm chat`） |
...
# 会自动恢复最新的一个
relayhelm -c "my project"   # → 恢复 "my project #3"
```
...
恢复 session 时，Relayhelm 会在输入提示符前以样式化面板显示之前对话的紧凑摘要：

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="恢复 Relayhelm session 时显示的「上次对话」摘要面板的样式化预览。" />
...
要禁用摘要并保留最简单的单行行为，在 `~/.relayhelm/config.yaml` 中设置：
...
**恢复到 CLI：** 当你想回到桌面时，只需运行 `/resume <title>`（或在 shell 中运行 `relayhelm -r "<title>"`），从平台停止的地方继续。
...
Relayhelm 在第一次交换后自动为每个 session 生成简短的描述性标题（3–7 个词）。这在后台线程中使用快速辅助模型运行，不增加延迟。浏览 `hermes sessions list` 或 `hermes sessions browse` 时可以看到自动生成的标题。
...
当 session 的上下文被压缩（通过 `/compress` 手动或自动触发）时，Relayhelm 会创建一个新的续接 session。如果原 session 有标题，新 session 会自动获得带编号的标题：

```
"my project" → "my project #2" → "my project #3"
```

按名称恢复时（`relayhelm -c "my project"`），会自动选取谱系中最新的 session。
...
Relayhelm 通过 `hermes sessions` 提供完整的 session 管理命令集：
...
当你想在隐藏或删除旧 session 之前保留一份可读的文件归档时，传入 `--format md` 或 `--format qmd`。Markdown/QMD 导出会为每个 session 写入一个文件到目录中（默认：`~/.relayhelm/session-exports`）。
...
当 Relayhelm 无法获取共享聊天的参与者标识符时，回退为该房间共享一个 session。

### 共享与隔离的群组 Session

默认情况下，Relayhelm 在 `config.yaml` 中使用 `group_sessions_per_user: true`。这意味着：

- Alice 和 Bob 可以在同一个 Discord 频道中与 Relayhelm 对话，而不共享对话历史
...
| 内容 | 路径 | 描述 |
|------|------|-------------|
| SQLite 数据库 | `~/.relayhelm/state.db` | 所有 session 元数据 + 带 FTS5 的消息 |
| Gateway 消息 | `~/.relayhelm/state.db` | SQLite——所有 session 消息的权威存储 |
| Gateway 路由索引 | `~/.relayhelm/sessions/sessions.json` | 将 session 键映射到活跃 session ID（来源元数据、过期标志） |

SQLite 数据库使用 WAL 模式支持并发读取和单写入，非常适合 gateway 的多平台架构。

:::note 遗留 JSONL 对话记录
在 state.db 成为权威存储之前创建的 session 可能在 `~/.relayhelm/sessions/` 中留有
`*.jsonl` 文件。Relayhelm 不再写入或读取这些文件。在确认对应 session 存在于
state.db 后可安全删除。
:::
...
- 清理最多每 `sessions.min_interval_hours`（默认 24）小时运行一次；上次运行时间戳记录在 `state.db` 内部，因此在同一 `HERMES_HOME` 下的所有 Relayhelm 进程间共享

默认为**关闭**——session 历史对 `session_search` 召回很有价值，静默删除可能会让用户感到意外。在 `~/.relayhelm/config.yaml` 中启用：
