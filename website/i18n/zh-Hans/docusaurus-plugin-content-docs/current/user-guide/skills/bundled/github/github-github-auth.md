...
uv run python3 "${HERMES_HOME:-$HOME/.relayhelm}/skills/github/github-auth/scripts/git-credential-token.py"
```
...
elif [ -f ~/.relayhelm/.env ] && grep -q "^GITHUB_TOKEN=" ~/.relayhelm/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.relayhelm/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
elif grep -q "github.com" ~/.git-credentials 2>/dev/null; then
  export GITHUB_TOKEN=$(uv run python3 "${HERMES_HOME:-$HOME/.relayhelm}/skills/github/github-auth/scripts/git-credential-token.py")
  echo "AUTH_METHOD=curl"
else
