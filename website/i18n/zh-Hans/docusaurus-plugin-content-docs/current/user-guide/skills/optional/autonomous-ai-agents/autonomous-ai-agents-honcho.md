...
description: "配置并使用 Honcho 记忆功能与 Relayhelm -- 跨会话用户建模、多配置文件 peer 隔离、观察配置、辩证推理、会话摘要及上下文预算控制。"
---
...
配置并使用 Honcho 记忆功能与 Relayhelm -- 跨会话用户建模、多配置文件 peer 隔离、观察配置、辩证推理、会话摘要及上下文预算控制。适用于设置 Honcho、排查记忆问题、通过 Honcho peers 管理配置文件，或调整观察、召回和辩证设置。

## Skill 元数据

| | |
|---|---|
| 来源 | 可选 — 通过 `relayhelm skills install official/autonomous-ai-agents/honcho` 安装 |
...
| 相关 skills | [`relayhelm`](/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-relayhelm) |
...
# Relayhelm 的 Honcho 记忆

Honcho 提供 AI 原生的跨会话用户建模。它在多次对话中学习用户特征，并为每个 Relayhelm 配置文件提供独立的 peer 身份，同时共享统一的用户视图。
...
3. **AI peer 卡片** -- 此 Relayhelm 配置文件的 AI peer 身份卡片
...
Honcho 将对话建模为 **peers** 之间的交互。Relayhelm 每个会话创建两个 peers：

- **用户 peer**（`peerName`）：代表人类用户。Honcho 从观察到的消息中构建用户表示。
- **AI peer**（`aiPeer`）：代表此 Relayhelm 实例。每个配置文件拥有自己的 AI peer，使 agents 形成独立视角。
...
| `per-session` | 每次 Relayhelm 运行创建新的 Honcho 会话 |
| `global` | 跨所有目录使用单一会话 |
...
每个 Relayhelm 配置文件拥有自己的 Honcho AI peer，同时共享同一工作区（用户上下文）。这意味着：
...
运行 `hermes honcho setup`。确保 `~/.relayhelm/config.yaml` 中包含 `memory.provider: honcho`。

### 记忆未跨会话持久化
检查 `hermes honcho status` -- 验证 `saveMessages: true` 且 `writeFrequency` 不是 `session`（该选项仅在退出时写入）。

### 配置文件未获得自己的 peer
创建时使用 `--clone`：`relayhelm profile create <name> --clone`。对于现有配置文件：`hermes honcho sync`。
...
| `hermes honcho sync` | 为所有尚未拥有 host 块的 Relayhelm 配置文件创建 host 块 |
| `hermes honcho migrate` | 从 OpenClaw 原生记忆迁移到 Relayhelm + Honcho 的分步指南 |
| `relayhelm memory setup` | 通用记忆提供商选择器（选择 "honcho" 运行相同向导） |
| `relayhelm memory status` | 显示当前活跃的记忆提供商及配置 |
| `relayhelm memory off` | 禁用外部记忆提供商 |