...
| | |
|---|---|
| 来源 | 可选 — 通过 `relayhelm skills install official/devops/watchers` 安装 |
...
监控 GitHub 仓库（在 `~/.relayhelm/.env` 中设置 `GITHUB_TOKEN` 以避免匿名请求限制 60 次/小时）：

```bash
python $HERMES_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name hermes-issues --repo InSelfControll/relayhelm --scope issues
```
