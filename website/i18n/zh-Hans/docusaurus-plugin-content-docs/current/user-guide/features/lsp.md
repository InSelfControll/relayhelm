...
Relayhelm 以后台子进程方式运行完整的语言服务器——pyright、gopls、rust-analyzer、
...
这与顶级编码 agent 所采用的架构相同。Relayhelm 将其作为自包含组件提供：
...
Relayhelm 会自动检测 PATH 上或 `<HERMES_HOME>/lsp/bin/` 中的二进制文件。
...
当 `install_strategy: auto` 时，Relayhelm 将二进制文件安装到 `<HERMES_HOME>/lsp/bin/`。
...
服务器在 Relayhelm 进程的整个生命周期内保持运行。没有空闲超时回收机制——每次写入都
...
同样的警告会在服务器启动时记录一次到 `~/.relayhelm/logs/agent.log`。

**服务器已启动但从不返回诊断结果**

检查 `~/.relayhelm/logs/agent.log` 中的 `[agent.lsp.client]` 条目——语言服务器的
