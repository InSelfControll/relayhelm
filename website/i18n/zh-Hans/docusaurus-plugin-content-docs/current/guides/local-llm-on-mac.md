...
两者均暴露兼容 OpenAI 的 `/v1/chat/completions` 端点。Relayhelm 支持任意一个——只需将其指向 `http://localhost:8080` 或 `http://localhost:8000`。
...
Relayhelm 会自动检测本地端点（localhost、局域网 IP）并放宽其流式传输超时限制。大多数情况下无需额外配置。
