...
完成本指南需要**租户管理员权限**（或由管理员代为授予同意）。请记录收集到的值——最终需要填入 `~/.relayhelm/.env`。
...
4. 填写以下内容：
   - **Name：**`Relayhelm Teams Meeting Pipeline`（或任何你能识别的名称）。
...
```powershell
# Create a policy scoped to the Relayhelm app
New-CsApplicationAccessPolicy `
  -Identity "Hermes-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict Relayhelm meeting pipeline to allow-listed users"
...
Relayhelm 内置了 Graph 身份验证冒烟测试。在 Relayhelm 安装目录下执行：
...
2. 用新值更新 `~/.relayhelm/.env` 中的 `MSGRAPH_CLIENT_SECRET`。
3. 重启 gateway 以使新密钥生效：`relayhelm gateway restart`。
4. 使用上述冒烟测试进行验证。
5. 在 Azure 门户中删除旧密钥。
