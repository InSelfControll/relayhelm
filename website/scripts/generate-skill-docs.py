...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/InSelfControll/relayhelm/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `relayhelm skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Relayhelm"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Relayhelm ships with a large built-in skill library copied into `~/.relayhelm/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Relayhelm also syncs bundled skills on `relayhelm update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.relayhelm/skills/` tree, it is still shipped with Relayhelm; restore it with `relayhelm skills reset <name> --restore`.",
        "",
...
def build_catalog_md_optional(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Official optional skills shipped with relayhelm — install via relayhelm skills install official/<category>/<skill>"',
        "---",
        "",
        "# Optional Skills Catalog",
        "",
        "Optional skills ship with relayhelm under `optional-skills/` but are **not active by default**. Install them explicitly:",
        "",
        "```bash",
