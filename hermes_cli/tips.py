...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or relayhelm -c.",
    ...
    "Ctrl+Z suspends Relayhelm to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "relayhelm -c resumes your most recent CLI session. relayhelm -c \"project name\" resumes by title.",
    "relayhelm -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "relayhelm -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "relayhelm chat -t web,terminal enables only specific toolsets for a focused session.",
    "relayhelm chat -s github-pr-workflow preloads a skill at launch.",
    "relayhelm chat -q \"query\" runs a single non-interactive query and exits.",
    "relayhelm chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "relayhelm chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "relayhelm --yolo bypasses all dangerous command approval prompts for the entire session.",
    "relayhelm chat --source telegram tags the session for filtering in hermes sessions list.",
    "relayhelm -p work chat runs under a specific profile without changing your default.",
    # --- CLI Subcommands ---
    "relayhelm doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "hermes dump outputs a compact setup summary — great for bug reports.",
    "relayhelm config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "relayhelm config edit opens config.yaml in your default editor.",
    "relayhelm config check scans for missing or stale configuration options.",
    ...
    "relayhelm skills search react --source skills-sh searches the skills.sh public directory.",
    "relayhelm skills check scans installed hub skills for upstream updates.",
    "relayhelm skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "relayhelm skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "hermes mcp add github --command npx adds MCP servers from the command line.",
    "hermes mcp serve runs Relayhelm itself as an MCP server for other agents.",
    "relayhelm auth add lets you add multiple API keys for credential pool rotation.",
    ...
    "relayhelm backup creates a zip backup of your entire Relayhelm home directory.",
    "relayhelm profile create coder creates an isolated profile that becomes its own command.",
    "relayhelm profile create work --clone copies your current config and keys to a new profile.",
    "relayhelm update syncs new bundled skills to ALL profiles automatically.",
    "relayhelm gateway install sets up Relayhelm as a system service (systemd/launchd).",
    "relayhelm memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "hermes webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: relayhelm tools disables unused tools, relayhelm skills config trims skills down.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as relayhelm -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Relayhelm tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'relayhelm profile create coder' creates the 'coder' command.",
    "relayhelm profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Relayhelm prints a resume command with session ID and stats.",
    "hermes sessions export backup.jsonl exports all sessions for backup or analysis.",
    "relayhelm -r SESSION_ID resumes any specific past session by its ID.",
    # --- Memory ---
    ...
    "relayhelm skills install official/security/1password installs optional skills from the repo.",
    ...
    "/plan writes a markdown implementation plan to .relayhelm/plans/ without executing anything.",
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: relayhelm cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Relayhelm runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "relayhelm gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Relayhelm via a local macOS server.",
    ...
    "Relayhelm loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — relayhelm tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.relayhelm/checkpoints/ — your project's .git is never touched.",
    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "relayhelm chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "relayhelm plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "relayhelm acp runs Relayhelm as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "relayhelm auth add nous or relayhelm auth add openai-codex sets up OAuth-based providers.",
    ...
    "relayhelm status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.relayhelm/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Relayhelm your own.",
    ...
    "Before a gateway session reset, Relayhelm auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.relayhelm/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Relayhelm state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    "Every interrupt during an agent run is logged to ~/.relayhelm/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'relayhelm profile create ls' would be rejected.",
    "relayhelm profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via relayhelm tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.relayhelm/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.relayhelm/skills/.hub/quarantine/ holds skills pending security review.",
    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Relayhelm auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Relayhelm config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Relayhelm is working.',
    ...
    '/reload reloads ~/.relayhelm/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.relayhelm/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.relayhelm/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    # --- Curator ---
    ...
    'relayhelm auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'relayhelm dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'relayhelm dashboard embeds the full Relayhelm TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.relayhelm/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.relayhelm/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.relayhelm/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.relayhelm/skills/ so drop-in skills appear without restarting the session.',
    ...
    'relayhelm -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'relayhelm chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'relayhelm chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'relayhelm chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "relayhelm chat --source tool tags programmatic chats so they don't clutter hermes sessions list.",
    ...
    'relayhelm import restores a full Relayhelm backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    ...
    'relayhelm setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'relayhelm status --deep runs the full health sweep across every component; plain relayhelm status is the quick view.',
    ...
    'HERMES_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.relayhelm/plugins/ — trust-gated by design.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Relayhelm block commands when the tirith scanner itself errors out.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.relayhelm/dashboard-plugins/.',
]
...
# Task-oriented example prompts for the empty composer. Kept generic — Relayhelm is
