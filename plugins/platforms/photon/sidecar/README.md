...
Small Node helper that bridges Relayhelm to Photon's Spectrum SDK
(`spectrum-ts`).  Relayhelm is Python; Photon has no public HTTP
...
- drains the inbound message stream so `spectrum-ts` keeps its
  reconnect/heartbeat machinery alive and Relayhelm can receive inbound messages
...
The Relayhelm plugin's `hermes photon setup` command runs `npm install`
here automatically.
...
`Space.send(...)` API. Relayhelm is Python, so replies go through this sidecar
