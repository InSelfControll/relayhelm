// Relayhelm — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Relayhelm): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Relayhelm -> gRPC): `/send` drives `space.send(...)`; `/typing`
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Relayhelm can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Relayhelm'
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Relayhelm allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Relayhelm, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Relayhelm supplied them so a known-good
