# A2A — Agent-to-Agent protocol for Relayhelm
...
peer (another Relayhelm, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
relayhelm gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Relayhelm serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.relayhelm/a2a_audit.jsonl`.
- Conversations persist to `~/.relayhelm/a2a_conversations/` — they survive context
