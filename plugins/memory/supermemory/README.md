...
Before running `relayhelm memory setup`, add the local endpoint to
...
Then run `relayhelm memory setup` and enter the local server's API key. Configuring
...
`https://api.supermemory.ai`. Relayhelm resolves it once and uses the same endpoint
...
it groups Hermes-written memories into a dedicated "Relayhelm" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Relayhelm profile:
