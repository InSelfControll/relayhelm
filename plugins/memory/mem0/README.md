...
```bash
relayhelm memory setup    # select "mem0"
```
...
Behavioral settings live in `$HERMES_HOME/mem0.json` (set them via `relayhelm memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.relayhelm/.env`.
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Relayhelm session and call `mem0_search` — it connects to your server.
...
```bash
relayhelm memory setup mem0 --mode oss \
...
```bash
relayhelm memory setup mem0 --mode oss --oss-llm-key sk-...
```
...
```json
{
  ...
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.relayhelm/mem0_qdrant"}}
  }
}
```
...
```bash
relayhelm memory setup mem0 --mode platform --api-key sk-...
```
