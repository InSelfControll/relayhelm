...
```bash
relayhelm plugins enable security-guidance
# or edit ~/.relayhelm/config.yaml manually:
plugins:
  enabled:
    - security-guidance
```
...
* **No project-local rules file.** Anthropic's `.claude/claude-security-guidance.md`
  ...
  add an analogous `.relayhelm/security-guidance.md` once layer 2 lands.
...
* `__init__.py`, `plugin.yaml`, `README.md`, and tests are original work by
  NousResearch, MIT-licensed alongside the rest of relayhelm.
