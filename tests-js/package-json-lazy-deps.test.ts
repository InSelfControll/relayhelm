 ...
 *
 * The root ``package.json`` is installed by ``relayhelm update`` on every user,
 ...
 *   workspace graph entirely. ``relayhelm update`` and ``relayhelm doctor --fix``
 ...
 *   ``relayhelm tools`` → Browser Automation → Camofox, and only used at
 *   runtime when ``CAMOFOX_URL`` is set. Its postinstall fetches a ~300MB
 *   Firefox-fork binary, which silently blocked ``relayhelm update`` for
...
test('camofox is not in root dependencies (must stay opt-in)', () => {
  ...
  assert.ok(
    ...
    'Camofox is a ~300MB binary-postinstall backend that must stay ' +
      ...
      '`relayhelm tools` → Browser Automation → Camofox.'
  )
})
