 ...
 *
 * Relayhelm Desktop signs with ``hardenedRuntime: true`` and points
...
test('bootstrap installer carries microphone entitlement for launcher attribution', () => {
  ...
  assert.equal(
    ...
    'the macOS bootstrap installer must sign with its entitlements.plist. ' +
      'When /Applications/Relayhelm.app is the setup launcher, macOS TCC treats ' +
      'io.github.inselfcontroll.relayhelm.setup as the responsible process for the desktop ' +
