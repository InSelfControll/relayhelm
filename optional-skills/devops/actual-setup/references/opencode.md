...
is typically only in `~/.relayhelm/.env`, not a shell profile, so the env form
...
```
OpenCode DOES use the `provider/model` slash form on the CLI (unlike Relayhelm,
...
`hermes-custom-providers` skill, pitfall 2). Relayhelm hits this because it forwards
