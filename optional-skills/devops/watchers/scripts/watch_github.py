...
Usage (via cron with --no-agent):
    ...
    relayhelm cron create hermes-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$HERMES_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name hermes-issues --repo InSelfControll/relayhelm --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Relayhelm .env file
(``${HERMES_HOME:-~/.relayhelm}/.env``) to avoid the 60 req/hr
