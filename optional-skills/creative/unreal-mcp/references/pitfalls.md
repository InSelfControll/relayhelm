...
### 1. Start order: editor first, Relayhelm second

Relayhelm probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Relayhelm session.
...
restart the editor, restart the Relayhelm session.
...
server fails to bind (Output Log) or Relayhelm' probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.relayhelm/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Relayhelm

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Relayhelm' connection
lives in `~/.relayhelm/config.yaml` via `hermes mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Relayhelm — just don't
mistake it for the Relayhelm setup step.
...
explicitly warns against overlapping calls. Relayhelm executes same-turn tool
...
### 9. Timeouts: Relayhelm gives up before Unreal does

Relayhelm' default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Relayhelm runs elsewhere (SSH backend,
