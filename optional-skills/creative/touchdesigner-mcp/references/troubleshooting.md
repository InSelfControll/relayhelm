...
## 5. Relayhelm Configuration

### Config location

`$HERMES_HOME/config.yaml` (defaults to `~/.relayhelm/config.yaml` when `HERMES_HOME` is unset)
...
Restart the Relayhelm session for changes to take effect. The MCP connection is
