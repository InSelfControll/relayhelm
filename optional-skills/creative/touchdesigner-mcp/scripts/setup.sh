...
HERMES_HOME_DIR="${HERMES_HOME:-$HOME/.relayhelm}"
...
# ── 3. Ensure Relayhelm config has twozero_td MCP entry ──
if [[ ! -f "$HERMES_CFG" ]]; then
    echo -e " ${FAIL} Relayhelm config not found at ${HERMES_CFG}"
    manual_steps+=("Create ${HERMES_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$HERMES_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Relayhelm config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Relayhelm config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
    ...
    manual_steps+=("Restart Relayhelm session to pick up config change")
fi
