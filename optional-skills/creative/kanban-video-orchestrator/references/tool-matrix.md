...
Maps each role archetype to the Relayhelm skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public relayhelm
...
## Relayhelm skills relevant to video production

### Visual / rendering skills (`relayhelm/skills/creative/`)
...
### Design / pre-production skills (`relayhelm/skills/creative/`)
...
### Audio / media skills (`relayhelm/skills/creative/` + `skills/media/`)
...
These are **not** Relayhelm skills but external CLIs / APIs that profiles invoke.
...
## Built-in Relayhelm tools for media review

These are native Relayhelm tools — not invoked via terminal but through their own
...
key is present in `${HERMES_HOME:-~/.relayhelm}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Relayhelm profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${HERMES_HOME:-~/.relayhelm}/.env` → environment variable.
