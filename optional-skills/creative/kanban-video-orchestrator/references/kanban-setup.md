...
the actual `setup.sh` that creates the project workspace, configures Relayhelm
...
2. **Create profiles** — `relayhelm profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.relayhelm/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `relayhelm kanban create` assigned to the director
...
```bash
relayhelm profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.relayhelm/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.relayhelm/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Relayhelm
...
Each profile gets a `SOUL.md` at `~/.relayhelm/profiles/<name>/SOUL.md` that
...
```bash
relayhelm kanban create "Direct production of <video title>" \
    --assignee director \
...
the Relayhelm `.env` (`${HERMES_HOME:-$HOME/.relayhelm}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _hermes_env="${HERMES_HOME:-$HOME/.relayhelm}/.env"
