...
| User config | EXTEND.md (project/user/XDG paths) + first-time-setup | Removed — not part of Relayhelm infra |
...
| File operations | Bash commands | Relayhelm file tools: `write_file`/`read_file` for text, `terminal` for binaries and URL downloads, `vision_analyze` for reading images |
...
- Core reference files (workflow, prompt-construction, styles, style-presets) — adapted for Relayhelm tooling
