...
The Relayhelm agent orchestrates scanning and opt-out submission with native tools
...
def cmd_doctor(args) -> None:
    ...
    if cfg["email_mode"] == "draft_only":
        ...
        L.append("  Email mode: browser (no password) - the agent sends opt-outs and opens verify "
                 "links via the operator's logged-in webmail. This needs Relayhelm pointed at the "
                 "operator's OWN Chrome over CDP (launch with --remote-debugging-port=9222 "
                 "--user-data-dir=~/.relayhelm/chrome-debug, signed into the webmail once); else it falls "
...
def cmd_cdp(args) -> None:
    ...
    Relayhelm at the operator's real Chrome on a dedicated debug profile (see methods.md).
    """
    import shlex
    import time
    ...
    if live:
        _out({"running": True, "endpoint": f"127.0.0.1:{port}",
              ...
              "note": "a debuggable browser is already listening; point Relayhelm's browser tools at "
    ...
    _out({"running": bool(live), "launched_pid": pid, "browser": browser,
          ...
          "next": ([f"point Relayhelm's browser tools at 127.0.0.1:{port} (CDP)",
