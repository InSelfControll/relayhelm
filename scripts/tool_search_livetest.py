#!/usr/bin/env python3
"""Live test harness for Relayhelm's Tool Search feature.
...
ORIGINAL_AUTH = Path.home() / ".relayhelm" / "auth.json"
...
def setup_isolated_home(enabled: bool, listing: str = "off",
    ...
    """Create a fresh ~/.relayhelm/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.relayhelm/.env`` so
    ...
    hermes_home = home_dir / ".relayhelm"
    ...
    real_env_file = Path.home() / ".relayhelm" / ".env"
    if real_env_file.exists():
        ...
        load_hermes_dotenv(hermes_home=str(Path.home() / ".relayhelm"))
