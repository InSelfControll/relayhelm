...
batch (tracker: [#77056](https://github.com/InSelfControll/relayhelm/issues/77056)).
...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Relayhelm home, same model, same tasks, same reps.
...
1. Create a dedicated Relayhelm home with credentials for the models under test:
   ...
   integration at it; no Relayhelm observability plugin needs to be enabled.
