...
Options:
  ...
  --root                Install as uid 0 with the root FHS layout: code in
                        /usr/local/lib/relayhelm, command in
...
$HERMES_HOME/relayhelm plus a ~/.local/bin launcher. Pass --root for the FHS
...
Examples:
  ...
  # skipping `relayhelm setup` & the browser tools for speed.
...
if [ "$INSTALL_SHORTCUT" = true ]; then
  ...
  set -- bash -c '
    set +e
    curl -fsSL https://raw.githubusercontent.com/InSelfControll/relayhelm/main/scripts/install.sh | bash -s -- "$@"
    install_status=$?
    if [ "$install_status" -eq 0 ] && [ -f /work/promote-main ]; then
      next_main=$(cat /work/promote-main)
      if git --git-dir=/work/repos/relayhelm.git update-ref refs/heads/main "$next_main"; then
...
SOURCE_REF="$COMMIT"
SNAPSHOT_REPO=""
FAKE_REPO="$SANDBOX_ROOT/root/repos/relayhelm.git"
git -C "$SANDBOX_ROOT/root/repos" init --bare -q relayhelm.git
if [ -n "$INSTALL_REF" ]; then
...
if [ -n "$(git -C "$GIT_ROOT" status --porcelain)" ]; then
  ...
  git -C "$SNAPSHOT_REPO" config user.name 'Relayhelm sandbox'
...
if [ ! -f "$SANDBOX_ROOT/root/certs/ca.pem" ]; then
  if ! ca_error="$(OPENSSL_CONF="$SANDBOX_ROOT/root/certs/openssl.cnf" \
    ...
    -subj '/CN=Relayhelm dev sandbox CA' \
