 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.relayhelm/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.relayhelm', 'whatsapp', 'session'));
...
const IMAGE_CACHE_DIR = process.env.HERMES_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.relayhelm', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.HERMES_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.relayhelm', 'document_cache');
const AUDIO_CACHE_DIR = process.env.HERMES_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.relayhelm', 'audio_cache');
...
// `relayhelm update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *Relayhelm*\n────────────\n';
...
function enqueuePollUpdateEvent({ key, update, selectedOptions, aggregation }) {
  ...
  const pollId = key?.id
    ...
    || '';
  // Only surface votes on polls Relayhelm itself created (tracked when
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Relayhelm', 'Chrome', '120.0'],
    ...
    for (const msg of messages) {
      ...
      if (msg.key.fromMe) {
        ...
        if (WHATSAPP_MODE === 'bot') {
          ...
          // the owner replied to would leak into Relayhelm and trigger
      ...
      // Ignore Relayhelm' own reply messages in self-chat mode to avoid loops.
