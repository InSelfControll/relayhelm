...
relayhelm model        # اپنا LLM پرووائیڈر اور ماڈل منتخب کریں
relayhelm tools        # کنفیگر کریں کہ کون سے ٹولز ایکٹو ہیں
relayhelm config set   # انفرادی کنفگ (config) ویلیوز سیٹ کریں
relayhelm gateway      # میسجنگ گیٹ وے شروع کریں (ٹیلی گرام، ڈسکارڈ، وغیرہ)
relayhelm setup        # مکمل سیٹ اپ وزرڈ چلائیں (یہ سب کچھ ایک ساتھ کنفیگر کر دے گا)
hermes claw migrate # OpenClaw سے مائیگریٹ کریں (اگر آپ OpenClaw سے آ رہے ہیں)
relayhelm update       # لیٹسٹ ورژن پر اپ ڈیٹ کریں
relayhelm doctor       # کسی بھی مسئلے کی تشخیص کریں
```
...
| --------------------------------------- | --------------------------------------------- | -------------------------------------------------------------------------------- |
| بات چیت شروع کریں                       | `hermes`                                      | `relayhelm gateway setup` اور `relayhelm gateway start` چلائیں، پھر بوٹ کو میسج بھیجیں |
...
**پہلی بار سیٹ اپ کے دوران:** سیٹ اپ وزرڈ (`relayhelm setup`) خود بخود `~/.openclaw` کو پہچان لیتا ہے اور کنفیگریشن شروع ہونے سے پہلے مائیگریٹ (migrate) کرنے کا آپشن دیتا ہے۔
...
- **مہارتیں (Skills)** — صارف کی بنائی گئی مہارتیں → `~/.relayhelm/skills/openclaw-imports/`
...
cd relayhelm
./setup-hermes.sh     # uv کو انسٹال کرتا ہے، venv بناتا ہے، .[all] کو انسٹال کرتا ہے، اور ~/.local/bin/relayhelm کا سیم لنک (symlink) بناتا ہے
...
- 🐛 [مسائل (Issues)](https://github.com/InSelfControll/relayhelm/issues)
