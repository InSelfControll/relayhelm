...
/**
 * In production the Python `relayhelm dashboard` server injects a one-shot
      ...
      try {
        ...
        if (!match) {
          console.warn(
            `[hermes] Could not find session token in ${BACKEND} — ` +
              `is \`relayhelm dashboard\` running? /api calls will 401.`,
          );
          return;
        }
        ...
        console.warn(
          `[hermes] Dashboard at ${BACKEND} unreachable — ` +
            `start it with \`relayhelm dashboard\` or set HERMES_DASHBOARD_URL. ` +
...
export default defineConfig({
  ...
  server: {
    proxy: {
      ...
      // Same host as `relayhelm dashboard` must serve these; Vite has no
