...
Browser-based dashboard for managing Relayhelm configuration, API keys, and monitoring active sessions.
...
`relayhelm dashboard` on port 9119 serves the **built** bundle from `hermes_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `npm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
- **Do not introduce new `uppercase`** (the literal Tailwind class) in `relayhelm/web/src`. Prefer `text-display` for new brand chrome. Legacy `uppercase` call sites (e.g. `components/ui/label.tsx`, `card.tsx`) remain until migrated.
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Relayhelm” — mixed case, no Mondwest, no `text-display` |
